...
| 영역 | 변경 내용 |
|------|----------|
| **핸드 장애 유형** | `kHandTimeout`, `kHandMotorDataFault`, `kHandSensorDataFault`, `kHandRateLow` 추가 (총 13개 장애 유형) |
| **핸드 경고 유형** | `kHandRateDegraded` 추가 (총 4개 경고 유형) |
| **컨트롤러 통계** | 컨트롤러별 패킷 수, 활성 시간, 장애 수 수집 및 JSON 저장 |
| **장애 로깅** | `FailureContext` 구조체: 장애 시점 관절 상태/추적 오차/모드 통합 기록 |

### v5.16.1

| 영역 | 변경 내용 |
|------|----------|
| **코드 검증** | C++20, `[[nodiscard]]`, `noexcept` 확인 완료 |
