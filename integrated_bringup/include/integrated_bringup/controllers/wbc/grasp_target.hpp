...
enum class GraspCommand : int {
  ...
  kRelease = 2,   ///< HOLD → RELEASE, then RELEASE → IDLE — open the hand.
  kAbort = 3,     ///< Any phase → IDLE — user cancellation / fault.
};
...
/// @var pregrasp_pose  TCP pose APPROACH drives to — typically a local-frame
///                     offset above @c grasp_pose (e.g. +Z by 5–10 cm).
///                     Computed by the caller; the FSM treats it as an opaque
///                     target. Reaching it (within approach_tolerance) enters
///                     CLOSURE, which then closes the residual gap to
///                     @c grasp_pose under the contact_rich OCP.
/// @var approach_start TCP pose recorded on APPROACH entry; RELEASE drives
///                     back toward this pose while opening the hand.
struct GraspTarget {
