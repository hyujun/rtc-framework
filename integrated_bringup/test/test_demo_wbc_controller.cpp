...
TEST_F(WbcFSMTest, WbcStateAggregatesContactFromFingertipForce) {
  // Inject contact on 2 fingertips → wbc_state should report
  // num_active_contacts = 2 and grasp_detected = true (>= min_fingertips=2).
  // Arm forward kinematics is unavailable (empty urdf_path), but fingertip
  // sensor parsing in ReadState is independent of FK.
  // Single fingertip force: 5 N
...
// Phase 2 lifecycle prereq (P-1): controller must tolerate repeated
// re-initialisation as part of activate/deactivate cycling. With the
// MPC-disabled construction used in this fixture, the on_activate logger
// path is never entered, so this test specifically exercises the
// InitializeHoldPosition path that the lifecycle hooks call into. A full
// LifecycleNode-based activate/deactivate cycle test belongs in an
// integration suite (requires rclcpp::init + create_lifecycle_node).
...
static void InjectFingertipForce(ControllerState& s, int f, float fx, float fy, float fz,
                                 float contact = 1.0f) {
...
TEST_F(WbcFSMTest, ReleaseReturnsToIdleOnTrajectoryComplete) {
  ...
  for (int i = 0; i < 1000; ++i) {
    state_.iteration = static_cast<uint64_t>(i);
    (void)ctrl_.Compute(state_);
    if (ctrl_.GetPhaseForTesting() == WbcPhase::kIdle) {
      break;
    }
  }
  EXPECT_EQ(ctrl_.GetPhaseForTesting(), WbcPhase::kIdle);
...
TEST_F(WbcFSMTest, ForceRateEMASmoothsImpulse) {
  ...
  (void)ctrl_.Compute(state_);
  const float rate_after_step = ctrl_.GetFingertipReportForTesting(0).force_rate;
  ...
  const float rate_after_decay = ctrl_.GetFingertipReportForTesting(0).force_rate;
