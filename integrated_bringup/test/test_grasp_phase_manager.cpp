...
using integrated_bringup::phase::GraspTarget;
...
TEST_F(GraspPhaseManagerTest, LoadValidYamlSucceeds) {
  ...
  EXPECT_EQ(manager_->Load(cfg), integrated_bringup::phase::GraspPhaseInitError::kNoError);
...
TEST_F(GraspPhaseManagerTest, LoadRejectsMissingPhase) {
  ...
  EXPECT_EQ(manager_->Load(cfg), integrated_bringup::phase::GraspPhaseInitError::kMissingPhase);
...
TEST_F(GraspPhaseManagerTest, LoadRejectsUnknownOcpType) {
  ...
  EXPECT_EQ(manager_->Load(cfg), integrated_bringup::phase::GraspPhaseInitError::kInvalidOcpType);
}

TEST_F(GraspPhaseManagerTest, LoadRejectsNonPositiveThreshold) {
  auto cfg = YAML::Load(kPandaPhaseConfig);
  cfg["transition"]["approach_tolerance"] = 0.0;
  EXPECT_EQ(manager_->Load(cfg), integrated_bringup::phase::GraspPhaseInitError::kInvalidThreshold);
}
