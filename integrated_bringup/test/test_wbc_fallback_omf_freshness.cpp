...
rub::ModelConfig MakeModelConfig(const FreshnessConfig& fc) {
  rub::ModelConfig cfg;
  cfg.urdf_path = ament_index_cpp::get_package_share_directory(fc.urdf_pkg) + "/" + fc.urdf_rel;
  cfg.root_joint_type = "fixed";
  if (fc.extended) {
    cfg.closure_yaml_path =
        ament_index_cpp::get_package_share_directory(fc.urdf_pkg) + "/" + fc.closure_rel;
  }
