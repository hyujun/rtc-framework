...
- **plan 원천 (S6-A)**: 계획기 스레드 (`catching.planner.enabled`) 또는 `diagnostic.oracle_plan` (사용자가 ground-truth 로 잰 고정 포구점 하나 — 테스트·sim 용) 이 **같은 plan box** 에 쓰고, RT 는 매 tick 한 번 읽어 `JudgePlan` (L3 §5.2 (a)~(f): 유효 · 현재 activation · 같은 트랙 · 새 `plan_id` · 게시 나이 ≤ `io.t_stale` · 마지막 리셋 이후) 을 통과한 plan 만 TRACKING 에서 받는다. 두 writer 를 함께 켜면 park. S6-A 의 계획기 탐색은 stub 이라 "plan 없음" 만 게시한다 — 출하 `ur5e_p1b` 는 oracle 을, `iiwa7_leap` 은 계획기를 켠다 (S6-B 에서 p1b 도 계획기로)
