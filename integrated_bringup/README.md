...
- **WBC (#123 Phase 2)**: DemoWbcController 도 동일 `ClosedChainHandFk` 를 배선하되 **관찰/publish 표면 전용**이다 — `InitHandModel`(secondary hand-only tree = `p1b`) + `ConfigureClosedChainHandFk`(OnDeviceConfigsSet) + per-tick `ComputeHandFingertipFk` 가 fingertip 을 arm TCP(`tcp.act`)로 base 합성해 `task_link_poses`(kHandTip) 로 publish 한다. TSID/CLIK/MPC 제어 동역학은 여전히 actuated control model(frozen-loop, `full_model_ptr_`)을 쓰며 이 FK 와 **분리**된다 (접촉 동역학의 loop-consistency 는 Phase 3, 별건). arm TCP FK(`tip_frame_id_`)는 종전대로. serial `hand_handle_` 은 closure 활성 시 non-null gate 겸 비-extended fallback 이며, `SetJointOrder` 는 `!closed_hand_fk_.active()` 일 때만 적용한다(loop-locked DoF 를 뺀 reduced serial tree 는 device 관절 전체와 매핑되지 않으므로).
