...
| `pull_estimator.*` | In-plane pull-force estimator (#167) — 필터/유효성 파라미터, plane normal source (`fixed` · `pinch_geometry` 화이트리스트), baseline subtraction, `required_roles`, per-tip contact model (`tips.<role>.link/contact_normal_local/friction_coeff/…`). 키 이름은 `rtc::grasp::{PullEstimatorParams, PullContactConfig}` 필드와 1:1 |
