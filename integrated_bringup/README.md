...
│       ├── compliance_diag_log_pod.hpp <- §7 task-admittance 진단 (#469 S4) — 소비된 wrench + source id/quality/invalid_reason + FSM/α + x̃·ν_c + K_p·K_d·Λ_d·§7.5 bound 스냅샷, 단일 compliance_diag.csv (DemoCompliance 전용)
│       ├── momentum_observer_log_pod.hpp <- 일반화 운동량 관측기 잔차 (#135 Layer 1b) + payload 추정 (Layer 2A) — device order r + ‖r‖∞ + valid/invalid_reason/ticks_since_seed + payload wrench/mass/σ_min/λ²/fit_error/valid/reason, 단일 momentum_observer.csv (3 데모 컨트롤러 공용)
...
| `payload_estimate` (3 데모 컨트롤러 공용) | 컨트롤러 | **SeqLock 직접 소유** — `SeqLock<MomentumObserverLogPod>` + `SetupPayloadEstimatePublisher()`. `PublishRole` 미사용 (#135 D12). 발행 조건은 **관측기** 활성이지 payload 추정기 활성이 아니다 — 잔차 절반만으로도 메시지가 성립하고, 출하 config 는 `payload_estimator.enabled: false` 라 추정기 기준으로 걸면 잔차까지 wire 에서 사라진다. `joint_names` 를 함께 실어 잔차의 **device order** 를 메시지가 자기기술한다 (pinocchio order 와 짝지으면 유한하고 매끄럽고 틀린 답이 나온다) |
