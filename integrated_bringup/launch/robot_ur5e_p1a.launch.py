# robot_ur5e_p1a.launch.py - UR5e real robot bringup
#
...
from rtc_tools.launch.pinning import pin_dds_threads_to_slot
...
def _launch_setup(context, *, session_dir):
    ...
    cm_rt_params_file, cm_rt_params_log = _build_cm_rt_params(context, session_dir, "ur5e_p1a")
...
def generate_launch_description():
    ...
    # Fingertip F/T inferencer config -- p1a-specific overlay in integrated_bringup
    # (symmetric with the p1a hand yaml). p1b intentionally ships none, so the
    # node's ft_inferencer.enabled=false default auto-disables inference there.
    ft_inferencer_config = PathJoinSubstitution(
        [
            FindPackageShare("integrated_bringup"),
            "config",
            "ur5e_p1a",
            "fingertip_ft_inferencer_p1a.yaml",
        ]
    )

    # SIL mode: default from the p1a hand yaml (SSoT); CLI `sil_mode:=` overrides.
    ...
    _p1a_hand_params = _load_hand_params(
        "integrated_bringup", "config", "ur5e_p1a", "udp_hand_node_p1a.yaml"
    )
    sil_mode_arg = DeclareLaunchArgument(
        "sil_mode",
        default_value=str(_p1a_hand_params.get("sil_mode", "off")),
        choices=["off", "loopmodel", "firmware"],
        description="SIL mode (default from udp_hand_node_p1a.yaml). "
    ...
    enable_cpu_shield = shield.enable_cpu_shield(
        "--robot",
        log_prefix="[RT]",
        # Passed as a substitution, not a performed string: this launch builds its
        # description without an OpaqueFunction, so there is no context to resolve
        # against here (#350).
    ...
    # robot_ur5e_p1a.launch.py does not use OpaqueFunction, and nested YAML overrides
    ...
    fake_hand_firmware_node = Node(
        ...
        parameters=[
            ...
            {
                "port": int(_p1a_hand_params.get("target_port", 55151)),
                "protocol_version": str(_p1a_hand_params.get("protocol_version", "1a")),
            },
        ],
        emulate_tty=True,
    )
