"""
sim_ur5e_p1b.launch.py — MuJoCo simulation launch file for UR5e + proto_1b
======================================================
...
Usage:
  ...
  ros2 launch integrated_bringup sim_ur5e_p1b.launch.py enable_viewer:=false
...
def launch_setup(context, *args, **kwargs):
    ...
    # names, topics, model_path (integrated_bringup/config/ur5e_p1b/
    # mujoco_simulator.yaml — overlaid on top).
    sim_default = PathJoinSubstitution([pkg_sim, "config", "solver_param.yaml"])
    sim_config = PathJoinSubstitution([pkg_bringup, "config", "ur5e_p1b", "mujoco_simulator.yaml"])
    # Mode-agnostic base (URDF/model topology, rosters, limits, control_rate);
    # sim.yaml overlays only the sim-specific delta on top.
    base_config = PathJoinSubstitution([pkg_bringup, "config", "ur5e_p1b", "_base.yaml"])
    ctrl_config = PathJoinSubstitution([pkg_bringup, "config", "ur5e_p1b", "sim.yaml"])
    ...
    try:
        ...
        ctrl_yaml_path = os.path.join(
            get_package_share_directory("integrated_bringup"), "config", "ur5e_p1b", "_base.yaml"
        )
...
def generate_launch_description():
    ...
    initial_controller_arg = DeclareLaunchArgument(
        ...
        description=(
            ...
            "Empty = use value from config/ur5e_p1b/sim.yaml."
        ),
    )
