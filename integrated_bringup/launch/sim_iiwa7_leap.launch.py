"""
sim_iiwa7_leap.launch.py — MuJoCo simulation launch for iiwa7 + LEAP
=========================================================================

Usage:
  # Default
  ros2 launch integrated_bringup sim_iiwa7_leap.launch.py
...
Hand sensor stack (baro/ToF/contact) is intentionally absent on this robot;
grasp controllers are deactivated via demo_shared.yaml (grasp_controller_type:
none).  MPC is off by default in the WBC YAML — enable explicitly after
...
def launch_setup(context, *args, **kwargs):
    ...
    ctrl_config = PathJoinSubstitution([pkg_bringup, "config", "iiwa7_leap", "sim.yaml"])
    ...
    if use_affinity.lower() in ("true", "1", "yes"):
        ...
        # before invoking it: launch's stdin isn't a tty, so an interactive sudo
        # prompt would silently hang and leave the cores un-isolated — better to
        # warn loudly and skip.
...
def generate_launch_description():
    ...
    # on the command line moves both (#402). iiwa7_leap has no _base.yaml — its
    # node parameters come from sim.yaml.
    max_log_sessions_arg = DeclareLaunchArgument(
        ...
        description=(
            ...
            "comes from config/iiwa7_leap/sim.yaml, which is also the SSoT for "
    ...
    initial_controller_arg = DeclareLaunchArgument(
        ...
        description=(
            ...
            "Empty = use value from config/iiwa7_leap/sim.yaml."
        ),
    )
