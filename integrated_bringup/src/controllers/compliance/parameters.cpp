...
void DemoComplianceController::DeclareGainParameters() noexcept {
  if (!node_) {
    return;
  }
...
rcl_interfaces::msg::SetParametersResult DemoComplianceController::OnGainParametersSet(
  ...
  for (const auto& p : params) {
    ...
    try {
      if (name == "kp_translation") {
        ...
        // ComputeSecondary carries the re-seed race closure for the window
