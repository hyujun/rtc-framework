...
DemoComplianceController::DemoComplianceController(std::string_view urdf_path, Gains gains)
...
void DemoComplianceController::InitFingertipForceFilters(double cutoff_hz, double grasp_cutoff_hz) {
...
void DemoComplianceController::InitArmModel(const rtc_urdf_bridge::ModelConfig& config) {
...
void DemoComplianceController::InitHandModel(const rtc_urdf_bridge::ModelConfig& /*config*/) {
  namespace rub = rtc_urdf_bridge;
  ...
  try {
    ...
    RCLCPP_WARN(logger_,
                "DemoComplianceController: secondary device '%s' has no matching tree_model (%s) — "
...
void DemoComplianceController::ConfigureClosedChainHandFk() {
  if (!builder_) {
    return;
  }
  const auto primary = GetPrimaryDeviceName();
  ...
  LogHandFkWiring(logger_, "[compliance]", res, closed_hand_fk_.missing_joint());
}

bool DemoComplianceController::ComputeHandForwardKinematics(const ControllerState& state) noexcept {
...
void DemoComplianceController::OnDeviceConfigsSet() {
  ...
  if (auto* cfg = GetDeviceNameConfig(primary); cfg) {
    ...
    if (js_size > 0 && arm_dof_ > 0 && js_size != arm_dof_) {
      RCLCPP_ERROR(logger_,
                   "[compliance] arm DoF mismatch: arm_dof=%d but primary device "
  ...
  if (hand_dof_ < 0 || hand_dof_ > kDemoComplianceMaxHandDof) {
    RCLCPP_ERROR(
        logger_,
        "[compliance] hand DoF %d exceeds capacity kDemoComplianceMaxHandDof=%d — clamping",
        hand_dof_, kDemoComplianceMaxHandDof);
    hand_dof_ = std::min(std::max(hand_dof_, 0), kDemoComplianceMaxHandDof);
  }
  ...
  if (combined_cache_.model() && tip_frame_id_ != 0) {
    ...
    if (task_tcp_frame_idx_ < 0) {
      RCLCPP_ERROR(logger_, "[compliance] arm TCP frame registration failed (cache locked)");
      ...
      if (task_base_frame_idx_ < 0) {
        RCLCPP_ERROR(logger_, "[compliance] arm base frame registration failed (cache locked)");
  ...
  RCLCPP_INFO(logger_,
              "[compliance] fingertip sensor capability: has_native_contact=%d "
...
ControllerOutput DemoComplianceController::Compute(const ControllerState& state) noexcept {
...
void DemoComplianceController::SetDeviceTarget(int device_idx,
...
void DemoComplianceController::DrainTargetSlot(const ControllerState& state) noexcept {
  RTC_TRACE_SCOPE("DemoComplianceController::DrainTargetSlot");
  ...
  if (!arm_target_initialized_.load(std::memory_order_acquire) && !estop_active_ && arm_readable_ &&
    ...
    if (arm_dof_ == 0 && state.num_devices > 0) {
      arm_dof_ = rtc::SelfReportedChannelBound(state.devices[0], kDemoComplianceMaxArmDof);
    }
  ...
  if (!hand_target_initialized_.load(std::memory_order_acquire)) {
    if (state.num_devices <= 1) {
      ...
      if (hand_dof_ == 0) {
        hand_dof_ = rtc::SelfReportedChannelBound(state.devices[1], kDemoComplianceMaxHandDof);
      }
      for (std::size_t d = 1; d < static_cast<std::size_t>(state.num_devices); ++d) {
        ...
        if (d == 1) {
          trajectory::JointSpaceTrajectory<kDemoComplianceMaxHandDof>::State hold_state;
...
void DemoComplianceController::SeedHoldTarget(const pinocchio::SE3& pose,
                                              const rtc::DeviceState& dev0,
...
void DemoComplianceController::ApplyPendingTarget(int device_idx, std::span<const double> values,
  ...
  const auto cap = (arm_dof_ > 0) ? static_cast<std::size_t>(arm_dof_)
                                  : static_cast<std::size_t>(kDemoComplianceMaxArmDof);
...
std::string_view DemoComplianceController::Name() const noexcept {
  return "DemoComplianceController";
}

void DemoComplianceController::TriggerEstop() noexcept {
  estopped_.store(true, std::memory_order_release);
}

void DemoComplianceController::ClearEstop() noexcept {
...
void DemoComplianceController::LoadConfig(const YAML::Node& cfg) {
  ...
  if (builder_) {
    (void)combined_cache_.InitModel(*builder_, /*contact_frame_ids=*/{}, "[compliance]", logger_);
  }
  ...
  {
    arm_dof_ = ParseArmDof(cfg, kDemoComplianceMaxArmDof, "demo_compliance_controller");
    const auto sp =
        ParseArmSafePosition(cfg, static_cast<std::size_t>(arm_dof_), "demo_compliance_controller");
  ...
  auto load3 = [&cfg](const char* key, std::array<double, 3>& arr) {
    ...
    if (!n.IsSequence() || n.size() != 3) {
      throw std::runtime_error(std::string("demo_compliance_controller: '") + key +
  ...
  if (!cfg["fsm"] || !cfg["fsm"].IsMap()) {
    throw std::runtime_error("demo_compliance_controller: required 'fsm' section is missing");
  }
  {
    const auto& fsm = cfg["fsm"];
    if (!fsm["pi_rotation_margin"]) {
      throw std::runtime_error(
          "demo_compliance_controller: required 'fsm.pi_rotation_margin' is missing");
    }
    const double pi_margin = fsm["pi_rotation_margin"].as<double>();
    if (!(pi_margin >= 0.0 && pi_margin <= M_PI_2)) {
      throw std::runtime_error(
          "demo_compliance_controller: 'fsm.pi_rotation_margin' out of range "
          "[0, pi/2]");
    }
    ...
    if (!fsm["contact_stop_release_eps"]) {
      throw std::runtime_error(
          "demo_compliance_controller: required 'fsm.contact_stop_release_eps' "
          "is missing");
    }
    const double eps = fsm["contact_stop_release_eps"].as<double>();
    if (!(eps >= 0.0 && eps <= 0.1)) {
      throw std::runtime_error(
          "demo_compliance_controller: 'fsm.contact_stop_release_eps' out of range "
          "[0, 0.1]");
    }
    ...
    if (fsm["contact_stop_lpf_cutoff_hz"]) {
      ...
      if (!(cutoff > 0.0 && cutoff < nyquist)) {
        throw std::runtime_error(
            "demo_compliance_controller: 'fsm.contact_stop_lpf_cutoff_hz' out of range "
            "(0, control_rate/2)");
      }
    ...
    if (fsm["contact_stop_force_lpf_cutoff_hz"]) {
      ...
      if (!(cutoff > 0.0 && cutoff < nyquist)) {
        throw std::runtime_error(
            "demo_compliance_controller: 'fsm.contact_stop_force_lpf_cutoff_hz' out of range "
            "(0, control_rate/2)");
      }
    ...
    if (fsm["contact_stop_force_guard_delta_n"]) {
      ...
      if (!(delta > 0.0 && delta <= 1000.0)) {
        throw std::runtime_error(
            "demo_compliance_controller: 'fsm.contact_stop_force_guard_delta_n' out of range "
            "(0, 1000] N");
      }
    ...
    if (fsm["contact_stop_force_guard_max_hold_ticks"]) {
      ...
      if (!(ticks >= 0 && ticks <= 1000)) {
        throw std::runtime_error(
            "demo_compliance_controller: 'fsm.contact_stop_force_guard_max_hold_ticks' out of "
            "range "
            "[0, 1000] ticks");
      }
  ...
  // BesselFilterN::Init so the failure names the key an operator has to edit.
  {
    ...
    if (!(shared.force_pi_lpf_cutoff_hz > 0.0 && shared.force_pi_lpf_cutoff_hz < nyquist)) {
      throw std::runtime_error(
          "demo_compliance_controller: 'force_pi_grasp.lpf_cutoff_hz' out of range "
          "(0, control_rate/2)");
    }
  }
  ...
  if (cfg["logs"]) {
    if (!cfg["logs"].IsSequence()) {
      throw std::runtime_error("DemoComplianceController: 'logs' must be a sequence");
    }
    for (const auto& entry : cfg["logs"]) {
      if (!entry.IsMap() || !entry["msg_type"]) {
        throw std::runtime_error("DemoComplianceController: each `logs` entry needs `msg_type`");
      }
      ParsedLogEntry e;
...
void DemoComplianceController::PublishNonRtSnapshot(const rtc::PublishSnapshot& snap) noexcept {
