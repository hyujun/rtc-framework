...
void DemoComplianceController::UpdateFingertipForceFilter(
...
void DemoComplianceController::UpdateVirtualTcp(const pinocchio::SE3& T_base_tcp,
                                                const Gains& gains) noexcept {
  RTC_TRACE_SCOPE("DemoComplianceController::UpdateVirtualTcp");
  vtcp_valid_ = false;
...
void DemoComplianceController::ComputeControl(const ControllerState& state, double dt,
                                              const Gains& gains) noexcept {
  RTC_TRACE_SCOPE("DemoComplianceController::ComputeControl");
  ...
  switch (ClassifyFrameTransition(
      ...
      frame_wait_ticks_.load(std::memory_order_acquire), kComplianceVtcpFrameWaitTicks)) {
    ...
    case FrameTransition::kExpireExternal:
      ...
      RCLCPP_WARN_THROTTLE(logger_, log_clock_, ::integrated_bringup::logging::kThrottleSlowMs,
                           ...
                           kComplianceVtcpFrameWaitTicks,
                           static_cast<int>(target_frame_id_.is_vtcp),
...
void DemoComplianceController::ComputeSecondary(const ControllerState& state, double dt,
                                                const Gains& gains) noexcept {
  RTC_TRACE_SCOPE("DemoComplianceController::ComputeSecondary");
  ...
  if (state.num_devices > 1 && state.devices[1].valid) {
    ...
    if (hand_new_target_pending_ && hand_readable_) {
      trajectory::JointSpaceTrajectory<kDemoComplianceMaxHandDof>::State start_state;
      trajectory::JointSpaceTrajectory<kDemoComplianceMaxHandDof>::State goal_state;
  ...
  // ── Grasp detection + ContactStopHand (500Hz) ────────────────────────
  {
    ...
    if (run_force_pi) {
      ...
      if (hand_readable_) {
        ...
        // transient.
        std::array<double, kDemoComplianceMaxHandDof> hand_meas{};
        for (int i = 0; i < hand_dof_; ++i) {
          hand_meas[static_cast<std::size_t>(i)] = dev1.positions[static_cast<std::size_t>(i)];
        }
        const std::array<double, kDemoComplianceMaxHandDof> hand_filt =
    ...
    if (pull_wiring_.enabled()) {
      ...
      RTC_TRACE_SCOPE("DemoComplianceController::UpdatePullEstimate");
...
ControllerOutput DemoComplianceController::WriteJointCommand(const ControllerState& state,
                                                             double dt) noexcept {
  RTC_TRACE_SCOPE("DemoComplianceController::WriteJointCommand");
  if (estop_active_) {
...
void DemoComplianceController::WriteArmJointCommand(const ControllerState& state,
                                                    rtc::DeviceOutput& out0, double dt) noexcept {
  const auto& dev0 = state.devices[0];
  const int nc0 = dev0.num_channels;
...
void DemoComplianceController::FillLogOutput(const ControllerState& state, ControllerOutput& output,
                                             double dt) noexcept {
  RTC_TRACE_SCOPE("DemoComplianceController::FillLogOutput");
  if (estop_active_) {
...
void DemoComplianceController::FillGraspSensorAggregates(const Gains& gains) noexcept {
...
void DemoComplianceController::FillEstopPublishState(double dt) noexcept {
  RTC_TRACE_SCOPE("DemoComplianceController::FillEstopPublishState");
...
void DemoComplianceController::FillPublishOutput(const ControllerState& state,
                                                 ControllerOutput& output, double /*dt*/) noexcept {
  RTC_TRACE_SCOPE("DemoComplianceController::FillPublishOutput");
  if (estop_active_) {
    return;
  }
  const auto& dev0 = state.devices[0];
...
ControllerOutput DemoComplianceController::ComputeEstop(const ControllerState& state) noexcept {
  RTC_TRACE_SCOPE("DemoComplianceController::ComputeEstop");
  const auto& dev0 = state.devices[0];
  ControllerOutput output;
  output.num_devices = state.num_devices;
  auto& out0 = output.devices[0];
  ...
  // safe_position_ is kDemoComplianceMaxArmDof (32) wide and only [0, arm_dof_) is
