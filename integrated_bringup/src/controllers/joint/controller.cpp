...
void DemoJointController::InitFingertipForceFilters(double cutoff_hz, double grasp_cutoff_hz) {
  ...
  if (estop_active_) {
    ...
    if (secondary_sensor_log_handle_) {
      ...
      FillDeviceSensorLogPod(state, /*device_idx=*/1, num_active_fingertips_,
                             ForceFilterLogView{fingertip_force_filt_, fingertip_force_guarded_,
                                                force_guard_rejected_},
                             pod);
...
void DemoJointController::LoadConfig(const YAML::Node& cfg) {
  ...
  }
  {
    ...
    if (fsm["contact_stop_force_guard_delta_n"]) {
      ...
      if (!(delta > 0.0 && delta <= 1000.0)) {
        throw std::runtime_error(
            "demo_joint_controller: 'fsm.contact_stop_force_guard_delta_n' out of range "
            "(0, 1000] N");
      }
    ...
    if (fsm["contact_stop_force_guard_max_hold_ticks"]) {
      ...
      if (!(ticks >= 0 && ticks <= 1000)) {
        throw std::runtime_error(
            "demo_joint_controller: 'fsm.contact_stop_force_guard_max_hold_ticks' out of range "
            "[0, 1000] ticks");
      }
  ...
  // BesselFilterN::Init so the failure names the key an operator has to edit.
  {
    ...
    if (!(shared.force_pi_lpf_cutoff_hz > 0.0 && shared.force_pi_lpf_cutoff_hz < nyquist)) {
      throw std::runtime_error(
          "demo_joint_controller: 'force_pi_grasp.lpf_cutoff_hz' out of range "
          "(0, control_rate/2)");
    }
  }
