...
**여기 없는 것 — "기존 test assertion 수정"**: 이는 timing-safety 가 아니라 **process 규칙** (회귀 은폐 방지) 이므로 RT 목록에서 분리했다 → [invariants.md](../../agent_docs/invariants.md) PROC-6 / AGENTS.md §6 E-6. Test 가 진짜 틀렸거나 spec 이 바뀐 경우의 정당한 변경까지 막지 않는다 (근거 + 별도 commit).

세부 규칙·grep 패턴·복구 절차: [../../agent_docs/invariants.md](../../agent_docs/invariants.md). Architecture / Process / Numerical invariants (ARCH·PROC·NUM) 는 RT 코드 밖에서도 적용되므로 AGENTS.md §3 에 상주.
