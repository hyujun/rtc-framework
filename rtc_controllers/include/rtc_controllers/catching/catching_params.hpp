...
struct CatchingParams {
  ...
  /// for a margin above those. Since S8-B every shipped robot profile sets it
  /// explicitly (value and measurement in that profile's controller YAML), so
  /// this default only covers a config without the key.
