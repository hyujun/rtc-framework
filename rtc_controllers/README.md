...
│   │   ├── catching_params.hpp               -- 파라미터 검증기 (`ParseCatchingParams` + `ValidateCatchingParams` → `armable`): 활성 구성 TBD, D-9·a_dec 교차제약, ω·dt 안정 경계, ζ≠1, provisional 은 실기에서 차단 (invented key 4종: `core.ball.provisional`·`planner.catchability.manipulability_min.provisional`·`robot.hand.provisional`·`reference.provisional` — 마지막은 S5.4 신설로, 블록 안에서 `v_max` 는 확정이고 `a_max` 는 미결이라 둘을 한 TBD 로 묶을 수 없었기 때문이다. 소비 키의 TBD 는 sim configure 를 **거부**하고 CM 은 그 한 건으로 전체 컨트롤러를 거부한다). `CheckCatchFrameProvisional` 은 robot config 의 `urdf.extra_frames.<catch_frame>.provisional` 에 같은 규칙 적용. 손 프로파일 `HandProfile` 은 S4.1 에서 `q_open`·`eta_close`(0.5–1)·`T_close_e2e`(≥0) 를 더했다 — `q_open` 은 §4.2 caging 쌍이 아니라 따로 세므로 (`q_open_tbd`) 쌍을 준 채 그것만 빠진 프로파일은 parse 되고 검증기가 그 키를 지목한다
...
│   ├── params/                               -- 위 스키마 파서 구현 (`catch_pose_ik_params.cpp` 포함 — S1.9 IK 옵션의 YAML 반쪽). `catching_yaml_read.hpp` 는 `catching:` 트리를 읽는 두 파서 (`catching_params.cpp`·`catch_pose_ik_params.cpp`) 가 공유하는 **private** 읽기 helper (섹션 접근·TBD double·값 표기) 다 — 노드를 어떻게 *읽는가* 만 공유하고, 어떻게 *보고하는가* (검증 report vs 예외) 는 각 파서가 갖는다
│   ├── catching/
