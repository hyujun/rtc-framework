...
│   │   └── reach_gate.hpp                    -- reach gate 코어 (header-only, RT). `gate = max(exp(-(d/σ)²), hold)` — d = tip 들과 물체 pose 에 실린 기록 접촉점 사이 평균 거리, hold = 대향 digit + `min_fingers` 개가 threshold 초과인 상태가 `hold_on_steps` 연속이면 ON, 미접촉 `hold_off_steps` 연속이면 OFF 인 Schmitt trigger (**policy step 단위**, ratchet 없음). `IsGraspedByForce` / `UpdateReachHold` / `RotateByQuatXyzw` (x,y,z,w · 정규화 · 영 쿼터니언 → NaN) / `MeanTipDistance` / `ReachProximity` / `ReachGate` (NaN 근접도는 hold 로 세탁하지 않는다)
│   ├── joint/
...
│   │   ├── hand_capture.hpp                  -- 판정의 손 관절 증거 (S8-C, D-S8-8 (b)) `EvaluateHandCapture`: caging 관절별로 ρ 가 `[rho_min, rho_max]` 에서 멈추고 (\|q̇\| ≤ `qd_tol`) q_close 쪽으로 밀면 (s·τ/τ_max ≥ `effort_frac_min`) stalled, `min_joints` 이상이면 blocked. τ_max 는 손 device 의 `joint_limits.max_torque` (로봇 상수 없음), 비유한 입력·좁은 lane·무효 config 는 "not blocked". Hold 여부·lane freshness·지속 시간은 바인딩 몫
...
│   │   ├── catching_params.hpp               -- 파라미터 검증기 (`ParseCatchingParams` + `ValidateCatchingParams` → `armable`): 활성 구성 TBD, D-9·a_dec 교차제약, ω·dt 안정 경계, ζ≠1, provisional 은 실기에서 차단 (invented key 5종: `core.ball.provisional`·`planner.catchability.manipulability_min.provisional`·`robot.hand.provisional`·`reference.provisional`·`robot.hand.capture.provisional` (S8-C) — 마지막은 S5.4 신설로, 블록 안에서 `v_max` 는 확정이고 `a_max` 는 미결이라 둘을 한 TBD 로 묶을 수 없었기 때문이다. 소비 키의 TBD 는 sim 에서도 이제 configure 거부가 아니라 park 다 — A-S5-12, 바인딩이 판정). `CheckCatchFrameProvisional` 은 robot config 의 `urdf.extra_frames.<catch_frame>.provisional` 에 같은 규칙 적용. 손 프로파일 `HandProfile` 은 S4.1 에서 `q_open`·`eta_close`(0.5–1)·`T_close_e2e`(≥0) 를 더했다 — S8-C 에서 `T_release_timeout` (없으면 `ReleaseTimeoutPerE2e` × T_close_e2e 로 유도, > T_close_e2e) 과 `capture.*` (`HandCaptureParams`, 섹션이 있을 때만 활성, 자체 provisional 이 실기를 막는다) 를 더했다 — `q_open` 은 §4.2 caging 쌍이 아니라 따로 세므로 (`q_open_tbd`) 쌍을 준 채 그것만 빠진 프로파일은 parse 되고 검증기가 그 키를 지목한다
│   │   ├── catch_pose_ik_params.hpp          -- `CatchPoseIkOptions` 의 YAML 스키마 (`ParseCatchPoseIkParams` → `CatchPoseIkConfig`, S3.5a). L3 §6 의 `planner.ik.*` + `planner.catchability.definition`·`manipulability_min.*` 를 읽고, `definition` 이 고른 행을 `options.manipulability_min` 에 담는다 — 두 행은 단위가 달라 (C-3) 한 호출로 묶어 짝이 어긋날 수 없게 한다. 없는 키는 구조체 기본값, **`planner.ik` 의 모르는 키는 거부** (그 섹션을 이 파서가 통째로 소유하므로 오타 외 가능성이 없다), 은퇴 키 (`lambda`·`manip_min`) 는 `CatchPoseIkRetiredKeys` 로 보고. `alpha_max` 는 L3 §6 과 코드가 같은 provisional 기본값을 갖지만 config 의 명시적 `TBD` 가 표현 가능해야 하므로 `TbdDouble` 로 개방 여부를 남기고 (TBD·부재면 값은 그 provisional 기본값), 반대로 **활성 정의의 문턱이 TBD 면** `options.manipulability_min` 을 비유한값으로 남겨 `Solve` 가 `kOptionsInvalid` 로 막는다 (다른 정의의 수치로 게이트하지 않는다). `fd_step` 은 L3 §6 에 키가 없어 YAML 로 도달할 수 없는 유일한 필드다. `ParseCatchingParams` 와 겹치는 키 `manipulability_min.arm_5row` 는 **읽는 법만** 공유한다 — 범위 밖 값 (`-0.1`) 은 이 파서가 던지고, 저쪽은 parse 한 뒤 `ValidateCatchingParams` 가 `kRangeViolation` 으로 보고한다
│   │   ├── catch_pose_ik_batch.hpp           -- 오프라인 catchability 지도(S3.5a)의 배치 계층: 후보/seed CSV I/O (`ParseCandidateCsv`·`ParseSeedCsv`), 결과 CSV (`BatchCsvHeader`·`BatchCsvRow`), 사유 이름 (`CatchPoseReasonName`), 배치 루프 (`RunBatch`), 그리고 실행파일의 fail-closed 진입 검사 둘 — catch frame 이름 해석 (`ResolveCatchFrame`: `RtModelHandle::GetFrameId` 는 모르는 이름에 universe(0) 를 돌려주므로, 오타나 frame 이 빠진 sub-model 이 전부 `model_invalid` 인 멀쩡한 CSV 로 끝나지 않게 먼저 던진다) 와 `--params` 파일 안의 `catching:` 트리 위치 (`ResolveCatchingTree`: 최상위 `catching:` · 출하 컨트롤러 config `<controller>: {catching: …}` · 트리 자체(최상위 `planner:`) **세 형태만** 받고 나머지는 거부한다 — root 로 폴백하면 파서가 모든 키를 조용히 기본값으로 채운다). `FormatCatchPoseIkOptions` 는 `--print-options` 가 찍는 해석 결과다. `BatchCsvRow(row, nv)` 의 행 폭은 **헤더의 nv** 다 — `Solve` 는 `kOptionsInvalid`·`kModelInvalid`·`kJointOrderMismatch` 를 `result.nv` 기록 전에 반환하므로 결과의 nv 로 폭을 잡으면 바로 그 fail-closed 행이 짧아져 reader 가 파일을 거부한다. 실행파일이 아니라 라이브러리에 있는 이유는 두 계약이 gtest 로 닿아야 하기 때문이다 — 결과 열이 solver 의 double 을 **bit-exact** 로 싣고 (S6.2 의 G3-I 동치 비교가 반올림된 열로는 성립하지 않는다), 후보 **순서를 바꿔도 판정이 같다** (python 이 격자를 샤딩·재개한다). `p_c`·`v_ball` 은 `Solve` 와 같이 **모델 world** 좌표이고 base→world 변환은 호출자 몫이다. 후보 CSV 의 열 순서는 헤더에서 읽고 (가정하면 p_c 와 v 가 바뀌어도 그럴듯한 지도가 나온다), 비유한 값은 거부한다 (`kTargetNonFinite` 로 사유 분포에 섞이면 깨진 생성기가 물리로 보인다)
│   │   ├── catch_gate_batch.hpp              -- 오프라인 gate-catchable 지도(S3.5b)의 배치 계층: 후보 CSV (q\*·q̇ᵘ 포함, 열 순서는 헤더가 정한다) → `time_feasibility.hpp` 의 런타임 함수 그대로 도달시간·γ 창·정지점 판정 (`JudgeGates`·`RunGateBatch`), 게이트별 결과와 첫 탈락 사유 (`GateReason`) 를 round-trip 정밀도 CSV 로. 모델을 로드하지 않는다 — 한계·상수는 전부 `GateSettings` (기본값 없음, `ValidateGateSettings`)
│   │   ├── planner_io.hpp                    -- 계획기 ↔ RT 데이터 계약 (S6-A): `PlannerRtState` (RT → 계획기, 매 tick) · `ActivityFor(Mode)` (TRACKING/APPROACH 탐색, COMMITTED/CLOSING monitor — enum 순서가 아닌 술어) · `JudgePlan` (L3 §5.2 (a)~(f) RT 수락 규칙 — 새 plan 판정은 궤적 token 이 아니라 writer 의 단조 `plan_id`, E-STOP 처럼 generation 을 안 올리는 리셋은 reset floor 로)
│   │   ├── planner_params.hpp                -- `planner.*` 파서 (`ParsePlannerParams`): 스레드 키 (S6-A) + 탐색 키 (S6-B: `sub_model` · `max_ik` · `slice.*` · `time.margin` · `n_settle` · `unc.kappa_sigma` · `gamma.margin` · `budget.*` · `hand.d_eff/r_cap` · `switch.*` · `freeze.T_freeze` · `score.*` (+ 발명 키 `penalty`) · `workspace.catch_box` · `provisional`). **결정값** (`T_freeze`·`catch_box`·`d_eff`·`r_cap`) 은 기본값이 없어 부재·TBD 면 unset (바인딩이 park), 기형 값은 거부
...
│   ├── params/                               -- 위 스키마 파서 구현 (`catch_pose_ik_params.cpp` 포함 — S1.9 IK 옵션의 YAML 반쪽). `catching_yaml_read.hpp` 는 `catching:` 트리를 읽는 파서들 (`catching_params.cpp`·`catch_pose_ik_params.cpp`·`planner_params.cpp`) 이 공유하는 **private** 읽기 helper (섹션 접근·TBD double·값 표기) 다 — 노드를 어떻게 *읽는가* 만 공유하고, 어떻게 *보고하는가* (검증 report vs 예외) 는 각 파서가 갖는다
│   ├── catching/
