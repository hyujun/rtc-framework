...
경계 조건: q(0) = q₀, q̇(0) = v₀, q̈(0) = a₀
          q(T) = qf, q̇(T) = vf = 0, q̈(T) = af = 0
```
...
**Duration 계산:** `max(0.01, max_joint_distance / trajectory_speed)`
...
**Duration 계산:**
- CLIK: `max(0.01, position_distance / trajectory_speed)`
- OSC: `max(0.01, max(trans_dist / speed, angular_dist / angular_speed))`
...
| **영공간** | N/A | N/A | 관절 센터링 (3-DOF만) | N/A (전체 사용) |
| **E-STOP** | **미구현** | 안전 위치 이동 | 안전 위치 이동 | 안전 위치 이동 |
| **역행렬** | N/A | N/A | LDLT (3×3/6×6) | PartialPivLU (6×6) |
| **토크 제한** | N/A | ±150 Nm | N/A | (속도 제한 경유) |
| **관절 속도 제한** | ±2.0 rad/s | ±2.0 rad/s | ±2.0 rad/s | ±2.0 rad/s |
| **계산량** | 최소 | 중간 | 중간 | 높음 |
