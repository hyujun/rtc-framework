...
### 6.8 실기 실측 (2026-08-14, p1b) — S2 답 + 추정기 결함의 구조
...
**따라서 "적응이 진동을 잡는다" 는 #426 의 동기는 성립하지 않는다.** 잡을 진동이 없다.
...
- **단일 tick 차분 추정은 상수 조정으로 살릴 수 없다** (구조 문제)
- **진동이 없으므로 적응의 원래 동기도 사라졌다** — "제거" 가 실제 후보다
- 살린다면 차분이 아니라 **레벨 회귀**, 창은 **접촉 → settle**
...
> **S1 · S2 는 2026-08-14 런이 답했다 (§6.8).** S2 = 진동 없음(0.1 Hz 공통모드), S1 = σ 7 mN
> (무부하) ~ 37 mN (부하 HF), 그리고 추정기가 실제로 먹는 `σ(Δf)` = 3.2–5.6 mN. **S3 는 그
> 결과로 무의미해졌다** — `K_est_max` 를 400 으로 올려도 수렴 지점 SNR 이 0.24 라 `k_est` 가
> 물체 강성으로 수렴할 수 없다 (§6.8). 다시 재려면 `K_est_max` 가 아니라 **측정 지점**을 바꾼
> 설계여야 한다. 아래 세 시나리오는 그 재설계를 검증할 때 재사용하는 절차로 남긴다.
...
1·2 는 §6.7–6.8 이 종결했다. **남은 것은 3 뿐이고, 그것도 현행 추정기로는 답이 정해져 있다** —
`k_est` 는 pin 을 안 벗어나고(#424 AC 충족) 풀어도 물체 강성으로 수렴하지 않는다(§6.8). 다음
계측은 이 runbook 을 **#426 의 재설계안**에 적용하는 것이지 같은 추정기를 다시 재는 것이 아니다.
