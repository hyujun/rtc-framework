...
### 6.8 실기 실측 (2026-08-14, p1b) — 추정기 결함의 구조 (S2 는 *무른* 물체만)
...
#### S2 — 이 물체에서는 진동이 없다. 그런데 이 물체는 무르다
...
> ⚠ **그러나 이것으로 S2 가 닫히지 않는다.** 아래에서 이 물체의 강성이 **K ≈ 2–11** 로
> 측정되는데, §6.6·#426 의 설계 범위는 **K ∈ [10, 200]** 이다. 즉 이 런은 그 범위의 **바닥**을
> 쟀다 — 1.4 N 을 내는 데 손가락 행정 **0.094** 가 들어간, 상당히 무른 물체다. §6.6 이 진동을
> 걱정한 것은 **단단한 쪽**이므로, S2 가 물었던 조건은 아직 측정되지 않았다.
>
> 그러므로 이 런이 정하는 것은 "**무른 물체에서는 적응 없이도 안정**" 까지다. `K ≳ 100` 인
> 물체(금속·유리 블록 등)를 한 번 더 재야 "적응 제거" 가 안전한지 알 수 있다. 그 판정은
> 아래 레벨 회귀로 **런 안에서 바로** 확인된다 — 회귀 K 가 100 을 넘지 않으면 그 세션은
> 다시 무른 쪽을 잰 것이다.
...
> 단일 런·단일 물체(작은 사각 상자)·손가락 2개다. K 가 O(10) 인 것이 이 물체 고유인지 이 손
> 전반인지는 물체를 바꿔 재야 갈린다. 다만 **진동 부재와 차분의 정보 손실은 K 값에 의존하지
> 않는 관찰**이다.
...
**아직 열려 있는 것**: "적응 제거" 가 안전한가. 이 런은 무른 물체(K ≈ 2–11)에서만 진동 부재를
보였고, §6.6 이 걱정한 단단한 쪽은 미측정이다. **제거를 결정하기 전에 `K ≳ 100` 물체로 한 번 더
재야 한다** — 그 한 세션이 없으면 관측된 적 없는 실패 모드를 근거 없이 제거하는 것이 된다.
...
>
> **S2 는 아직 열려 있다.** 그 런의 물체가 **K ≈ 2–11** 로, 설계 범위 [10, 200] 의 바닥이었다.
> 다시 잴 때는 **`K ≳ 100` 인 물체**(금속·유리 블록 등)를 쓰고, §8.4 의 레벨 회귀로 **그 런
> 안에서** 강성부터 확인한다 — K 가 100 을 안 넘으면 무른 쪽을 또 잰 것이므로 물체를 바꾼다.
...
1 은 §6.7–6.8 이 종결했고 3 은 현행 추정기로는 답이 정해져 있다 — `k_est` 는 pin 을 안 벗어나고
(#424 AC 충족) 풀어도 물체 강성으로 수렴하지 않는다 (§6.8). **남은 것은 2 (S2) 이고, 단단한
물체로 한 세션이 더 필요하다** (§6.8 의 ⚠). 그 세션이 #426 의 "적응 제거" 선택지를 가른다.
