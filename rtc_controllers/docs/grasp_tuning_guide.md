...
> **읽기 전에 — 적응 이득 스케줄링은 은퇴했다 (2026-08-14, §6.9).** `K_est_max` 가 추정치의
> seed 와 같아 `gain_scale` 은 상수 `1/(1 + β)` = 0.769 이며, 이 pin 은 임시 조치가 아니라
> **최종 처분**이다. 아래 §3.2 · §4.1 · §4.2 · §6.3 · §6.4 의 β=0.03 분석과 실측표는 **적응이
> 켜진 상태를 가정한 설계·역사 기록**이지 실기 거동이 아니고, 그 상태를 배포할 계획도 없다.
> 지금 로봇이 도는 값을 찾는다면 §4 의 "배포" 열과 §6.9 를 본다.
...
빨라지지 않고, 최단 시상수는 `τ_min = β / Kp_base` 로 β 혼자 결정한다. β=0.03 · Kp_base=0.02
에서 `τ_min = 1.5 s`. β 를 올리는 것은 "이득을 조금 낮추는" 것이 아니라 **모든 파지의 속도 상한을
낮추는** 행위다 (§6.3).

> **이 절은 적응이 켜진 상태를 가정한 설계 분석이고, 그 상태는 배포된 적이 없으며 배포될 계획도
> 없다 (§6.9).** 배포는 `β = 0.3` · `K_est_max = 1.0` (pin) 이라 `gain_scale` 이 상수 0.769 이고
> 위 λ·τ_min 식은 실기 거동을 서술하지 않는다. **`β = 0.03` 은 YAML 에 들어간 적이 없다** —
> #425 가 세웠다가 적응을 끄면서 취소한 계획값이고 (배포 `beta` 는 `1fc219e5` 이래 0.3 에서
> 움직이지 않았다), #426 이 그 계획 자체를 폐기했다.
...
> 유효성을 못박는다. 되살려 놓고 보니 실기 힘 노이즈에서 예산을 넘겨 **적응은 비활성으로
> 배포됐고** (#425), 2026-08-14 실기가 그 상태를 최종 처분으로 만들었다 (§6.9). 배포 `beta` 는
> 0.3 에서 한 번도 움직이지 않았다 — 되살린 추정을 그대로 켜면 `τ_min` 이 15 s 가 되어 BT
> 예산을 전부 초과하므로 0.03 으로 낮추는 계획이 있었으나, 적응이 은퇴하며 함께 폐기됐다.
...
| `beta` | 0.3 | 0.3 | 루프이득 상한 — `τ_min = β/Kp_base` (§3.2). pin 상태에서는 이 값 혼자 상수 `gain_scale` 을 정한다 |
| `K_est_max` | 1.0 | 1.0 | `K_contact_est` 상한. seed 와 같으므로 **적응 비활성 — 최종 처분** (§6.9) |

> **세 값 전부 고정이다 (§6.9).** 아래 튜닝 지침은 적응이 켜진 가상 상태를 다루는 설계 기록이며
> 배포에서 움직일 손잡이가 아니다. `K_est_max` 를 올려 적응을 켜는 계획(400 + `β` 0.03)은
> 폐기됐고, `β` 는 pin 상태에서 **모든 파지의 이득을 26% 바꾸는 값**이라 단독으로 움직이면 안 된다.

튜닝 (적응이 켜진 상태 가정 — 역사 기록):
...
> | 단조 상승 후 안 내려옴 = 부풀어오름 | §6.6 의 흡수 상태. **튜닝으로 못 고친다** | 없음 — pin 이 막는다 (§6.9) |
...
> **배포에서는 이 스텝을 돌지 않는다 (§6.9).** `beta` 는 적응이 켜져 있을 때만 손잡이이고,
> pin 상태에서는 상수 `gain_scale = 1/(1+β)` 를 혼자 정하므로 건드리면 물체와 무관하게 모든
> 파지의 이득이 바뀐다. 아래는 적응이 켜진 상태를 가정한 설계 기록이다.
...
### 6.6 추정기는 힘 노이즈에 구조적으로 취약하다 (σ 실측은 §6.7–6.8)
...
`β` 와 `K_est_max` 는 얽혀 있어 **어느 쪽도 단독으로 움직일 수 없다** — 그리고 §6.9 이후로는
둘 다 움직이지 않는다:

| | `K_est_max = 1.0` (pinned) | `K_est_max = 400` (활성) |
|---|---|---|
| **`β = 0.3`** | **← 현재 배포.** `gain_scale` 0.769 | τ_min 15 s — 전 구간 예산 초과 |
| **`β = 0.03`** | `gain_scale` 0.971 — 이득만 26% 상승, 적응 없음 | ~~#426 목표~~ **폐기** (§6.9) — 실측 상한에서 no-op |
...
> **어느 σ 인가.** 위 표의 σ 는 *레벨* 노이즈지만 추정기가 실제로 먹는 것은 **tick 당 차분**
> `Δf` 다. 둘은 다른 양이고 실기에서 자릿수가 다르다 (§6.8: 레벨 σ 7–235 mN 대 `σ(Δf)` 3.2–5.6 mN).
> 이후 논의에서 예산을 세울 때는 `σ(Δf)` 를 쓴다.

**결론 (2026-08-12 시점)**: 적응을 실기에서 켜려면 tick 차분을 그대로 둔 채 상수를 조정하는
것으로는 안 되고, 추정기 자체 (여기 조건을 반영한 가변 윈도우, 최소자승 누적, RLS/칼만 등) 를
다시 설계해야 한다 — #426.

> **이 결론은 §6.8 이 뒤집었다.** 재설계도 하지 않는다 — 적응이 풀려는 문제(단단한 물체의 진동)가
> **도달 가능한 최대 강성에서 존재하지 않음**이 실측됐기 때문이다. 최종 처분은 **§6.9** 를 읽는다.
> 아래 관측 수단 문단과 §3.2 · §4.1 · §6.3 의 지위는 그대로 유효하되, "그때까지" 가 아니라
> **영구**로 읽는다.

관측 수단은 #424 가 붙였다: `GraspState.finger_stiffness_est` 가 per-finger 추정치를
싣는다. pin 이 걸린 배포에서 이 필드가 계속 `1.000` 인 것이 위 `DeployedTuningIsPinnedAndNoiseImmune`
의 런타임 대응물이며, 값이 움직이기 시작하면 pin 이 풀렸다는 뜻이다. §3.2 · §4.1 · §4.2 · §6.3 의
β=0.03 분석은 **적응이 켜진 상태를 가정한 설계 문서**이며 배포 거동이 아니다.
...
손가락 0.0 이었으므로 §6.2(deformation guard 동결)는 원인에서 배제됐다. → **§6.8 에서 답이 났다.**

### 6.8 실기 실측 (2026-08-14, p1b) — S2 답 + 추정기 결함의 구조 + 강성 상한

§6.7 이 남긴 두 공백(부하 상태 σ · S2)을 메운 런이다 (`logging_data/260814_0855`, joint
컨트롤러, 적응 OFF, `f_target` 1.0 N, `f_contact_threshold` 0.5 N, `Kp_base` 0.02, `β` 0.3).
작은 사각 상자를 미끄러짐 없이 파지했고, **이번 런은 전제가 전부 성립한다**:

| 전제 | 실측 |
|---|---|
| 무손실 | tick gap **0** / 42385 행 |
| 기준 고정 (슬립 미발동) | `f_desired` = **1.0000 ± 0.0006 N** |
| 서보 여력 있음 (`s` 미포화) | thumb **0.74–0.81** · index **0.68–0.79** |
| deformation guard 미발동 | `integrator_frozen` **0%** |
| 유지 구간 | **2 블록, 13.4 s + 21.4 s** (합 17403 tick) |

#### S2 — 도달 가능한 최대 강성에서 진동이 없다

유지 구간 스펙트럼에서 서보하는 손가락의 파워는 **2 Hz 미만에 84–97%** 가 몰려 있고 피크는
**0.07–0.3 Hz** 다. 그리고 두 손가락이 **같이** 움직인다:

```
corr(LF thumb, LF index) = +0.904 / +0.992     (블록 #0 / #1)
```

게인 스케줄 불안정이면 두 손가락이 서로 밀어내며 **역상관**이 나와야 한다. 공통모드라는 것은
루프가 자기 자신이 아니라 **외부 교란을 느리게 따라갔다**는 뜻이다. 대역폭도 그와 일치한다 —
`Kp_eff = 0.02 × 0.769 = 0.0154 1/(N·s)` 는 그 자체로 0.01–0.1 Hz 짜리 루프다.

**파지 대상은 금속 박스였다** — 즉 이것이 **이 하드웨어가 도달할 수 있는 가장 단단한 조건**
이고, 더 단단한 경우는 존재하지 않는다. `gain_scale` 은 배포 상수 **0.769** 였다. 적응이 켜져
있었다면 아래 실측 K 에서 β=0.3 기준 **0.229** 를 골랐을 것이다 — 즉 **적응이 3.4배 깎았을
지점에서, 안 깎은 채로 진동이 없었다.** 이것이 S2 에 대한 답이다.

#### σ — 조건을 분리해야 한다

| 조건 | 손가락 (mean) | σ | LF < 2 Hz | HF > 2 Hz |
|---|---|---|---|---|
| 무부하, 법칙 미개입 | ring (0.02 N) | **7.4** | 2.2 | 7.1 |
| 미세 접촉, 법칙 미개입 | middle (0.10 N) | 16.8–26.5 | 6–21 | 16 |
| 부하 + **서보 중** | index (0.88 N) | 58–103 | 55–68 | 21–23 |
| 부하 + **서보 중** | thumb (1.08 N) | 147–235 | 145–206 | 21–37 |

부하 손가락 σ 의 대부분은 **LF 성분**이고 그것은 센서가 아니라 위 루프 응답이다. 순수 센서
바닥은 **7 mN (무부하) ~ 37 mN (부하 HF)**. §6.7 의 9–13 mN 은 이 범위 안이다.

#### 추정기가 먹는 것은 레벨 σ 가 아니다 — 그리고 여기서 결론이 난다

추정기는 tick 차분을 쓴다. 그것을 재면 **`σ(Δf) = 3.2–5.6 mN`, 부하·접촉 여부와 무관하게 평탄**
하다. 오차 5분위로 쪼개면 구조가 그대로 드러난다 (블록 #1, index):

| \|e\| [N] | \|Δf\| [mN] | \|Δs\| | `k_inst_raw` |
|---|---|---|---|
| 0.0232 | 4.68 | 1.15e-6 | **4048** |
| 0.0494 | 3.11 | 1.94e-6 | 1608 |
| 0.2056 | 2.99 | 6.75e-6 | **435** |

```
spearman(|e|, |Δf|) = -0.03 / -0.16     ← 분자는 오차와 무관 = 순수 노이즈
spearman(|e|, |Δs|) = +0.967 / +0.985   ← 분모는 P 법칙 그 자체
spearman(|e|, k_inst_raw) = -0.605 / -0.653
```

`Δs = Kp_eff · e · dt = 3.08e-5 · e` 를 대입하면 예측 9.5e-6 대 실측 9.87e-6 — 모델이 맞는다.
따라서

> **`K_inst = Δf/Δs = (센서 노이즈)/(Kp·dt·e) ∝ 1/e`**

**추정기는 물체 강성이 아니라 컨트롤러 자신의 추종오차의 역수를 재고 있다.** 물체 항이 식에
없다. 실제로 블록 간 비교에서 thumb 평균오차가 −0.28 → +0.04 N 으로 **좋아지자** `k_inst_raw`
중앙값이 207 → 832 로 **커졌다** — 방향이 거꾸로다. 이것은 통계 문제가 아니라 **분모가 오차라는
구조 문제**이므로 σ 를 줄이거나 `alpha_ema` 를 조정해서 해결되지 않는다.

**#426 제안값 `K_est_max = 400` 으로도 안 된다.** 수렴 지점 `|Δs| ≈ 2e-6` 에서 참 강성 400 이
만드는 신호는 **0.8 mN**, 노이즈는 **3.2 mN** → **SNR 0.24**. SNR 3 을 얻으려면 `|e| ≥ 0.78 N`
의 정상편차가 필요한데 목표가 1.0 N 이므로 유지 구간에서 원리적으로 불가능하다.

#### 참 강성은 O(2–11) 이다 — 차분을 안 쓰면 나온다

`s` 는 지령이라 노이즈가 없다. 그러면 차분할 이유가 없다 — `f = K·(s − s₀)` 를 **ramp 구간에서
레벨 회귀**하면 (같은 런, 같은 손가락):

| | K | bootstrap 5–95% | r | n |
|---|---|---|---|---|
| thumb | **11.21** | [10.42, 12.34] → **±8%** | +0.988 | 2174 |
| index | **1.91** | [1.58, 2.30] → ±19% | +0.865 | 2174 |

| 추정량 (같은 데이터) | thumb | index |
|---|---|---|
| **레벨 회귀 (ramp)** | **11.2** | **1.9** |
| #426 제안 LS 차분 `Σ(Δs·ΔF)/Σ(Δs²)` (ramp+hold) | 1.5 | 15.4 |
| 현행 tick 비율 (median) | 351.8 | 655.6 |

차분이 신호를 없앤다 — tick 당 `Δs` 2e-6 는 총 행정 0.094 보다 **5자릿수** 작다. 정보는
레벨에 있다. 2-파라미터(`K`, `s₀`) 회귀는 running sum 5개(`Σs Σf Σs² Σsf n`)로 tick 당 O(1)
· 할당 없음이므로 **RT 제약은 현행과 같다**. 차분은 `s₀` 를 소거하려는 장치였는데 그 대가가
신호 전부였다.

**창이 중요하다**: 유지 구간에서 같은 회귀를 하면 r 이 0.47 / **−0.71** 로 무너진다 (위 공통모드
교란이 지배). 유효한 창은 **접촉 → settle** 이고, 적응이 필요한 시점도 거기다.

§6.6 의 `K_est_max = 400` + `β = 0.03` 짝은 **존재하지 않는 강성에 맞춰 크기를 잡았다**. 실측
K 를 넣으면 `gain_scale` 은 pin 0.769 대 (K=11.2, β=0.03) **0.749** — 목표 짝에서 적응은 이
물체에 대해 **no-op** 이다. 효과가 나는 β=0.3 쪽은 0.229 로 이득을 4.4배 깎는데, 위에 따르면
**깎을 진동이 없다**.

#### 그 K 는 물체가 아니라 **손**이다 — 그래서 상한이다

파지 대상이 **금속 박스**였다. 강체를 물었는데 thumb 이 1 N 당 `s` 를 **0.089** (배포 자세에서
그 관절 45° 행정의 **4.0°**) 내줬다는 것은, 그 변형이 물체가 아니라 **핑거패드·건·센서의 직렬
컴플라이언스**라는 뜻이다. 직렬 스프링이므로

```
1/K_total = 1/K_object + 1/K_hand      (K_object -> 무한대 이면 K_total = K_hand)
```

즉 **`K ≈ 11` 은 이 손의 강성이고 K_total 의 상한**이다. 어떤 물체를 물어도 이보다 클 수 없고,
무를수록 작아진다. index 의 1.9 는 접촉이 스치듯 걸린 경우다 (1 N 에 폐쇄 행정의 52% — 물체를
사실상 안 누르고 있다).

**§6.6·#426 의 설계 범위 `K ∈ [10, 200]` 은 이 하드웨어에 존재하지 않는다.** 그 범위는 강체
손가락을 가정한 시뮬 플랜트에서 왔고, 실기는 상한 ~11 에서 포화한다. `K_est_max = 400` 은 물리
상한의 **약 36배**다. 적응이 실제로 고를 수 있는 폭은:

| | `gain_scale` 범위 (K_total 0 → 11.2) |
|---|---|
| β = 0.3 | 1.000 → **0.229** |
| β = 0.03 (#426 이 노리던 짝 — §6.9 에서 폐기) | 1.000 → **0.749** |

> 단일 런·손가락 2개(그중 index 는 스치는 접촉)다. 상한 값 자체는 접촉점·패드 면적에 따라
> 변할 수 있지만 **자릿수(10 이지 400 이 아니다)는 강체를 물어 얻은 것이라 견고**하다.

#### #426 에 대한 함의

**K 에 의존하지 않는 결론** (이 런으로 확정):

- **단일 tick 차분 추정은 상수 조정으로 살릴 수 없다** — `K_inst ∝ 1/e` 는 구조 문제다
- 살린다면 차분이 아니라 **레벨 회귀**, 창은 **접촉 → settle**. ±8% 로 실현 가능함이 실측됐다
- `K_est_max = 400` 은 어느 쪽을 택하든 폐기 대상이다

**"적응 제거" 를 지지하는 근거**: 적응의 존재 이유는 단단한 물체의 진동 억제였는데, **도달
가능한 최대 강성(강체)에서 적응 없이 진동이 없었다.** 그리고 적응이 고를 수 있는 폭은 목표 짝
(β=0.03)에서 `gain_scale` 0.749–1.000 뿐이다 — 배포 상수 0.769 와 겹친다.

**결정은 §6.9 가 기록한다** — 적응 제거를 채택하되 코드가 아니라 **계획을 폐기**하는 형태로
집행했다 (pin 영구화).

#### 도구 — `[SERVO ACTIVE]`

이 런에서 `--stats` 는 처음에 네 손가락 전부를 usable 로 찍었다. 기존 오염 검사(`[REF MOVED]` ·
`[SPIKE]` · `[STEPPED]`)는 **모양**만 보는데, 느린 교란을 따라가는 루프는 완벽하게 노이즈처럼
생긴 분포를 만든다 (p2p/σ 4.1–7.5 로 전부 통과). 판별은 `delta_s` 다 — 법칙이 몰지 않는 손가락은
정확히 0 이다. `[SERVO ACTIVE]` 가 이것을 잡아 usable 을 middle·ring 으로 좁힌다.

`s` 를 차분하면 안 된다: 유지 구간이 파지 여러 번이면 `s` 는 매번 접근이 끝난 자리에서 다시
시작하므로 이어붙인 폭은 **구간 사이 계단**이 지배하고, 이 런에서 실제로 네 손가락 전부가
servoed 로 찍혔다. `delta_s` 는 구성상 tick 당이라 그 계단을 싣지 않는다.

### 6.9 종결 (2026-08-14) — 적응 이득은 은퇴한다 (#426)

§6.6 은 "적응을 끈다" 를 **임시 조치**로 적었고 §6.8 은 그것을 뒤집을 수 있는 관측을 냈다.
관측 결과는 뒤집지 않았다: **`K_est_max = 1.0` pin 은 최종 처분이며, 적응을 켜는 계획
(`K_est_max` 400 + `β` 0.03) 과 추정기 재설계 계획은 모두 폐기됐다.**

**근거 셋 — 어느 것도 측정된 K 값에 의존하지 않는다** (전문·수치는 §6.8):

1. **존재 이유가 반증됐다.** 적응은 단단한 물체의 진동을 억제하려고 있었는데, **도달 가능한
   최대 강성**(금속 박스 = 강체)에서 적응 없이 `gain_scale` 0.769 로 진동이 없었다 (유지 구간
   파워 2 Hz 미만에 84–97%, 두 손가락 상관 +0.90/+0.99 = 동상). 적응이 켜져 있었다면 β=0.3 에서
   0.229 를 골랐을 것이므로 **3.4배 깎았을 지점에서 안 깎은 채로 안정했다.**
2. **추정기는 물체를 재지 않는다.** `Δs = Kp·dt·e` 이므로 `K_inst = Δf/Δs ∝ 1/e` 이고 물체 항이
   식에 없다 (`spearman(|e|,|Δf|) ≈ 0` · `spearman(|e|,|Δs|) ≈ +0.98`). 상수로 고칠 수 있는
   문제가 아니다.
3. **목표 짝이 no-op 이다.** 실측 상한 `K_total ≈ 11` 에서 β=0.03 이 고를 수 있는 폭은
   `gain_scale` 0.749–1.000 이고, 배포 상수 **0.769** 가 그 안에 들어간다.

**설계 범위가 이 하드웨어에 없다.** `K ∈ [10, 200]` 은 강체 손가락을 가정한 시뮬 플랜트에서 왔다.
실기는 손의 직렬 컴플라이언스(핑거패드·건·센서) 때문에 **~11 에서 포화**하고, 이것은 물체가 아니라
손의 성질이라 물체를 바꿔도 커지지 않는다 (§6.8 의 상한 논증). `K_est_max = 400` 은 물리 상한의
**약 36배**이며 폐기 대상이다.

#### 왜 코드를 지우지 않았는가

배포가 이미 pin 이므로 삭제의 **런타임 이득이 0** 인 반면, 삭제는 `GraspState.finger_stiffness_est`
ABI (E-3 Critical) 와 `grasp_diag.csv` 의 6개 컬럼군(`k_est_*` · `k_inst_raw_*` · `delta_s_*` ·
`delta_f_*` · `gain_scale_*` · `est_updated_*`) 을 함께 없앤다. 후자는 `plot_rtc_log` 의 **로그
종류 판별자**이기도 해서 (`k_est_` 접두, `io/log_type.py`), 이 결정의 근거가 된 두 실기 번들
(§6.7 의 `260813_1421` · §6.8 의 `260814_0855`) 이 도구에 인식조차 되지 않게 된다. **자기 결정의 증거를 못 읽게
만드는 대가로 얻는 것이 없다** — 채널은 남기고 계획만 폐기한다.

pin 이 소리 없이 풀리는 것은 세 겹이 막는다: `GraspParams::K_est_max` 의 **코드 default 가 1.0**
이므로 YAML 이 키를 빠뜨려도 켜지지 않고, `DeployedForcePiTuning` 이 배포 YAML 의 `β`·`K_est_max`
쌍을 고정하며, `DeployedTuningIsPinnedAndNoiseImmune` 이 추정치가 seed 를 못 벗어남을 고정한다.

#### 이 문서를 어떻게 읽는가

| 절 | 지위 |
|---|---|
| §3.2 · §4.1 · §4.2 · §5 Step 6 · §6.3 · §6.4 의 `β=0.03` 분석 | **설계·역사 기록.** 적응이 켜진 상태를 가정하며 실기 거동이 아니다 |
| §6.6 "조치: 적응을 끈다" | 그 조치가 **최종**이 됐다 |
| §6.6 의 기각된 완화책 3종 · §6.8 의 LS 차분 평가 | 유효 — 되살리려는 시도가 다시 나올 때 쓸 반증 자료 |
| §8 runbook | 이 질문에는 더 쓰지 않는다 (§8.2 참조). 다른 계측에는 그대로 유효 |

**되살리려면 무엇이 뒤집혀야 하는가** — 근거 1·2·3 중 하나가 아니라 **하드웨어**다: 손의 직렬
컴플라이언스가 한 자릿수 이상 올라가(핑거패드·건·센서 교체) `K_total` 이 O(100) 에 닿고, 그
강성에서 상수 이득이 실제로 진동해야 한다. 그때에도 추정기는 차분이 아니라 **레벨 회귀**
(`f = K(s−s₀)`, 창은 접촉→settle, 실측 ±8%) 여야 한다 — 근거 2 는 하드웨어와 무관하기 때문이다.
...
> **이 runbook 은 제 질문에 답했다 (§6.7 · §6.8) 이고 그 답이 §6.9 로 종결됐다.** 아래 절차는
> 이제 **다른 계측을 위한 재사용 자산**이다 — σ·강성·진동을 grasp_diag.csv 로 재는 방법 자체는
> 유효하지만, "적응을 켤 것인가" 를 다시 묻기 위해 돌리는 것은 §6.9 가 배제한다.
...
| ~~**S3**~~ | ~~벤치, `K_est_max` 상향 + `beta` 동반 인하~~ **폐기 (§6.9)** — 적응을 켜는 계획 자체가 사라졌다. 아래 각주가 이 시나리오가 무의미해진 두 이유를 남긴다 |

> **S1 은 2026-08-14 런이 답했다 (§6.8)** — σ 7 mN (무부하) ~ 37 mN (부하 HF), 추정기가 실제로
> 먹는 `σ(Δf)` = 3.2–5.6 mN. **S3 는 무의미해졌다** — `K_est_max` 를 400 으로 올려도 수렴 지점
> SNR 이 0.24 라 `k_est` 가 물체 강성으로 수렴할 수 없다.
>
> **S2 도 답했다** — 금속 박스(강체)에서 진동이 없었다. 그리고 같은 런이 **`K ≈ 11` 이 물체가
> 아니라 손의 강성이며 K_total 의 상한**임을 보였으므로, "더 단단한 물체로 다시" 라는 시나리오는
> **존재하지 않는다**. S3 도 같은 이유로 무의미하다.

**S2 를 S1 보다 먼저 본다** — 진동 유무가 나머지 전부의 범위를 정했기 때문이다. 실제로 그렇게
갈렸다 (§6.9).

**`force_pi_grasp` 값을 실기에서 바꾸는 경로** (S3 는 폐기됐지만 이 메커니즘은 남는다) —
**ROS 파라미터가 아니라 YAML + 컨트롤러 재로드**다. `grasp_controller_type` 만 파라미터로 노출돼
있고 `force_pi_grasp` 블록의 값(게인·임계 전부)은 노출되지 않는다. `set_params()` 는 C++ API 이며
ROS 표면이 없으므로 실기에서 이 값을 바꾸는 경로는 `on_configure` 재실행뿐이다 (§7 3번):
YAML 편집 → (`--no-symlink` 빌드였다면 재빌드, §7 2번) → 컨트롤러 재로드.

반영 여부는 추측하지 말고 **CSV 로 확인한다** — 아래 §8.4 의 `beta` / `K_est_max` 줄이 그 run 의
실효값을 그대로 찍는다. 값이 안 바뀌었으면 그 세션은 배포 설정을 한 번 더 돌린 것에 불과하다.

> **`K_est_max` 를 올리지 말 것 (§6.9).** 적응은 은퇴했고, 그 값을 400 으로 푸는 것은 물리 상한의
> 36배를 상한으로 주는 것이다. 배포 pin 을 벗어난 상태로 잰 데이터는 §6.3 표와도, 실기 거동과도
> 비교 대상이 아니다.
...
| **`sigma= … mN`** (핑거별) | **§6.6 표의 σ 열.** 실측은 §6.7 (9–13 mN) · §6.8 이 냈다 |
| `[SERVO ACTIVE]` · `[REF MOVED]` · `[SPIKE]` · `[STEPPED]` | 그 σ 가 **센서 노이즈 바닥이 아닌** 사유. 붙은 손가락은 `NOT a noise floor` 목록으로 빠진다 — 그 숫자를 σ 로 인용하면 안 된다. `[SERVO ACTIVE]` 는 법칙이 그 손가락의 `s` 를 몰고 있었다는 뜻이고, 모양 검사 셋과 달리 **분포가 완벽히 노이즈처럼 생겨도 발화**한다 (§6.8) |
| `→ noise floor usable from:` | **σ 로 인용해도 되는 손가락 목록.** 비어 있으면 이 런에는 노이즈 바닥이 없다 |
| `k_est: … [CONSTANT]` | `[CONSTANT]` 면 pin 이 살아 있다 — **배포 기대값이며 §6.9 이후로는 이것이 붙지 않는 런이 이상 신호다** |
...
**셋 다 §6.7–6.8 이 종결했고, 그 결과가 §6.9 다.** 1 은 σ 실측, 2 는 강체에서 진동 부재, 3 은
`k_est` 가 pin 을 안 벗어나고(#424 AC 충족) 풀어도 물체 강성으로 수렴하지 않음.

**이 runbook 을 이 질문에 다시 쓰지 않는다** — §6.9 가 세 근거 모두 K 값에 의존하지 않음을 보였고,
결론을 뒤집는 것은 더 단단한 물체가 아니라 **다른 손**이다. 이 절차가 다음에 값을 하는 곳은 손
하드웨어가 바뀌었을 때의 컴플라이언스 재측정, 그리고 §6.9 가 남긴 레벨 회귀(`f = K(s−s₀)`) 로
그 새 상한을 재는 일이다.
