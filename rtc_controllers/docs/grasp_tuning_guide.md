...
현재 코드가 갖는 방어는 `K_est_max` (배포 400) **하나뿐이며, 이것은 상한이지 해법이 아니다** —
`gain_scale` 을 0.077 아래로 못 내려가게 막아 정지(stall)를 유계로 만들 뿐, 위 표의 느려짐 자체는
남는다. 무노이즈 거동은 이 상한에 닿지 않으므로 §6.3 표는 그대로 유효하다.

**결론**: 적응 이득 스케줄링을 실기에서 켜려면 tick 차분을 그대로 둔 채 상수를 조정하는 것으로는
안 되고, 추정기 자체 (여기 조건을 반영한 가변 윈도우, RLS/칼만 등 노이즈 모델을 갖는 추정기)를
다시 설계해야 한다. 그 전까지 실기에서는 `K_est_max` 를 낮춰 (예: 배포 강성 상한 근처) 사실상
고정 이득에 가깝게 운용하는 편이 안전하다. 관측 수단은 #424 가 붙인다.
