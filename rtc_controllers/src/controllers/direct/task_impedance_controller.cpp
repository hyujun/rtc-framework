...
void TaskImpedanceController::LoadConfig(const YAML::Node& cfg) {
  ...
  MaybeSelectSubModel();
  // The schema itself lives in params/task_impedance_params.hpp (#236 S7c-2,
  // G2). What stays here is glue that needs the MODEL or the SeqLock: the
  // submodel switch above, the sensor-frame lookup, the wrench reconfigure, and
  // the commit order those two impose.
  auto g = gains_lock_.Load();
  params::TaskImpedanceConfig parsed;
  if (!cfg) {
    // NUM-6 floor-only path. The parser floors, applies §6.4, then applies the
    // §6.1 guard — and the floored gains are committed EVEN WHEN that guard
    // throws, because this is the one path where nothing else rewrites the POD
    // and get_gains() must not keep reporting a gain Compute() refuses to run.
    // The parser cannot do the commit itself (it does not own the SeqLock),
    // hence the rethrow. Nothing else in the schema was read on this path, so
    // there is no frame to look up and no wrench to reconfigure.
    try {
      params::ParseTaskImpedanceParams(cfg, g, selection_, parsed);
    } catch (...) {
      gains_lock_.Store(g);
      throw;
    }
  ...
  const pinocchio::FrameIndex sensor_id = LookupSensorFrame(parsed.sensor_frame);
  wrench_.Configure(parsed.wrench, 1.0 / GetDefaultDt());  // validates, then commits

  // Commit. Nothing below throws, so a rejected reconfigure leaves the live
  // gains and the live wrench path exactly as they were.
  sensor_frame_name_ = parsed.sensor_frame;
  sensor_frame_id_ = sensor_id;
  wrench_enabled_ = parsed.wrench_enabled;
  formulation_ = parsed.formulation;
  gains_lock_.Store(g);
  command_type_ = parsed.command_type;
}
