...
void CascadedComplianceController::LoadConfig(const YAML::Node& cfg) {
  ...
  MaybeSelectSubModel();
  // The schema itself lives in params/cascaded_compliance_params.hpp (#236
  // S7c-2, G2). What stays here is glue that needs the MODEL or the SeqLock:
  // the submodel switch above, the sensor-frame lookup, the wrench reconfigure,
  // and the commit order those two impose.
  auto g = gains_lock_.Load();
  params::CascadedComplianceConfig parsed;
  params::ParseCascadedComplianceParams(cfg, g, parsed);
  if (!cfg) {
    // NUM-6 floor-only path — nothing else in the schema was read, so there is
    // no frame to look up and no wrench to reconfigure.
    gains_lock_.Store(g);
    return;
  }
