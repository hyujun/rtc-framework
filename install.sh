...
print_summary() {
  ...
  case "$MODE" in
    sim)
      ...
      echo ""
      echo "  # Hand simulation is built-in (fake_response in integrated_bringup/config/ur5e_p1a/mujoco_simulator.yaml)"
    ...
    robot)
      ...
      echo "  ros2 launch integrated_bringup robot_ur5e_p1a.launch.py robot_ip:=192.168.1.10"
      echo ""
      echo "  # Fake hardware (no physical robot — for testing)"
      echo "  ros2 launch integrated_bringup robot_ur5e_p1a.launch.py use_fake_hardware:=true"
      echo ""
    ...
    full)
      ...
      echo "  ros2 launch integrated_bringup sim_ur5e_p1a.launch.py"
      echo ""
  ...
  echo "  AGENTS.md                — agent constitution (invariants, workflow, escalation)"
  echo "  CLAUDE.md                — Claude Code entry (@AGENTS.md import + hooks/rules)"
