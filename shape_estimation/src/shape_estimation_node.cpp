...
namespace {
auto node_log() {
  return ::rtc::shape::logging::NodeLogger();
}
}  // namespace

ShapeEstimationNode::ShapeEstimationNode() : LifecycleNode("shape_estimation_node") {
  // Lifecycle design: constructor is intentionally empty.
  // All resource allocation happens in on_configure().
}

ShapeEstimationNode::CallbackReturn ShapeEstimationNode::on_configure(
  ...
  prot_config.residual_threshold = get_parameter("protuberance.residual_threshold").as_double();
  prot_config.min_cluster_points =
      static_cast<uint32_t>(get_parameter("protuberance.min_cluster_points").as_int());
  prot_config.cluster_radius = get_parameter("protuberance.cluster_radius").as_double();
  prot_config.gap_distance_jump = get_parameter("protuberance.gap_distance_jump").as_double();
  prot_config.min_gap_invalid_count =
      static_cast<uint32_t>(get_parameter("protuberance.min_gap_invalid_count").as_int());
  ...
  clear_srv_ = create_service<std_srvs::srv::Trigger>(
      ...
      [this](const std::shared_ptr<std_srvs::srv::Trigger::Request> req,
             std::shared_ptr<std_srvs::srv::Trigger::Response> res) { ClearCallback(req, res); });

  // ── Timer (시각화 publish 주기) ────────────────────────────────────────────
  const auto viz_period = std::chrono::milliseconds(static_cast<int64_t>(1000.0 / viz_rate_hz));
  ...
  RCLCPP_INFO(node_log(),
              ...
              cloud_config.expiry_duration_sec, min_points_for_fitting_, publish_rate_hz_,
              viz_rate_hz);
  return CallbackReturn::SUCCESS;
}

ShapeEstimationNode::CallbackReturn ShapeEstimationNode::on_activate(
    const rclcpp_lifecycle::State& state) {
...
ShapeEstimationNode::CallbackReturn ShapeEstimationNode::on_deactivate(
    const rclcpp_lifecycle::State& state) {
  // 탐색 중이면 중단
  if (action_active_) {
...
ShapeEstimationNode::CallbackReturn ShapeEstimationNode::on_cleanup(
    const rclcpp_lifecycle::State& /*state*/) {
...
ShapeEstimationNode::CallbackReturn ShapeEstimationNode::on_shutdown(
    const rclcpp_lifecycle::State& state) {
...
ShapeEstimationNode::CallbackReturn ShapeEstimationNode::on_error(
    const rclcpp_lifecycle::State& /*state*/) {
...
void ShapeEstimationNode::DeclareParameters() {
  ...
  // 센서 가중치 파라미터
  declare_parameter("exploration.finger_weights", std::vector<double>{0.2, 0.4, 0.4});
...
void ShapeEstimationNode::SnapshotCallback(rtc_msgs::msg::ToFSnapshot::SharedPtr msg) {
  ...
  if (valid_count == 0) {
    RCLCPP_WARN_THROTTLE(node_log(), *get_clock(), ::rtc::shape::logging::kThrottleSlowMs,
  ...
  RCLCPP_DEBUG_THROTTLE(node_log(), *get_clock(), ::rtc::shape::logging::kThrottleSlowMs,
                        "SnapshotCallback: valid_sensors=%d, cloud_size=%d", valid_count,
                        voxel_cloud_.Size());
...
ShapeEstimate ShapeEstimationNode::EstimateShape(const std::vector<PointWithNormal>& points) {
  // Fast classification (O(1))
  ShapeEstimate fast_result =
      fast_classifier_.Classify(latest_snapshot_.local_curvatures, latest_snapshot_.curvature_valid,
  ...
  if (n_points >= min_points_for_fitting_) {
    ...
    if (fitted_result.type != ShapeType::kUnknown &&
      ...
      RCLCPP_DEBUG_THROTTLE(node_log(), *get_clock(), ::rtc::shape::logging::kThrottleFastMs,
                            "EstimateShape: fitted=%s(%.2f) 선택 (fast=%s(%.2f), points=%d)",
                            ShapeTypeToString(fitted_result.type).data(), fitted_result.confidence,
...
void ShapeEstimationNode::VizTimerCallback() {
  ...
  if (has_snapshot_) {
    ...
    if (latest_estimate_.type != ShapeType::kUnknown) {
      latest_protuberance_result_ =
          protuberance_detector_.Detect(latest_estimate_, points, snapshot_history_.Snapshots());
    } else {
      latest_protuberance_result_ = ProtuberanceResult{};
    }
    ...
    estimate_pub_->publish(
        ToMsg(latest_estimate_, latest_snapshot_, latest_protuberance_result_, header));

    // 빔 + 곡률 시각화
    tof_beams_pub_->publish(viz::BuildBeamMarkers(latest_snapshot_, ros_stamp, frame_id_));
...
void ShapeEstimationNode::InitExploration() {
  ...
  action_server_ = rclcpp_action::create_server<ExploreShape>(
      this, "/shape/explore",
      [this](const rclcpp_action::GoalUUID& uuid, std::shared_ptr<const ExploreShape::Goal> goal) {
        return HandleGoal(uuid, goal);
      },
      [this](const std::shared_ptr<GoalHandleExploreShape> goal_handle) {
        return HandleCancel(goal_handle);
      },
      [this](std::shared_ptr<GoalHandleExploreShape> goal_handle) { HandleAccepted(goal_handle); });
  ...
  sub_estop_ = create_subscription<std_msgs::msg::Bool>(
      ...
      [this](std_msgs::msg::Bool::SharedPtr msg) { EstopCallback(std::move(msg)); });
  ...
  explore_timer_ = create_wall_timer(explore_period, [this]() { ExploreLoopCallback(); });
}

// ── Action Server 핸들러 ────────────────────────────────────────────────────

rclcpp_action::GoalResponse ShapeEstimationNode::HandleGoal(
    const rclcpp_action::GoalUUID& /*uuid*/, std::shared_ptr<const ExploreShape::Goal> /*goal*/) {
  if (action_active_) {
...
void ShapeEstimationNode::HandleAccepted(std::shared_ptr<GoalHandleExploreShape> goal_handle) {
  ...
  if (goal->use_current_object_pose && has_object_position_) {
    ...
    object_pos = {goal->object_position.x, goal->object_position.y, goal->object_position.z};
  }
  ...
  if (!switch_controller_client_->service_is_ready()) {
    if (!switch_controller_client_->wait_for_service(std::chrono::seconds(2))) {
      RCLCPP_ERROR(get_logger(), "/rtc_cm/switch_controller unavailable — exploration aborted");
      return;
    }
  }
  ...
  const auto wait_dur = std::chrono::milliseconds(controller_switch_delay_ms_ + 2000);
  if (fut.wait_for(wait_dur) != std::future_status::ready) {
    RCLCPP_ERROR(get_logger(), "switch_controller srv timed out (>%lldms) — exploration aborted",
                 static_cast<long long>(wait_dur.count()));
    return;
  }
  const auto resp = fut.get();
  if (!resp->ok) {
    RCLCPP_ERROR(get_logger(), "switch_controller srv rejected '%s': %s", controller_name_.c_str(),
                 resp->message.c_str());
    return;
  }
  ...
  delayed_start_timer_ = create_wall_timer(std::chrono::milliseconds(1), [this, object_pos]() {
...
void ShapeEstimationNode::StartExploration(const std::array<double, 3>& object_position) {
  // Point cloud 초기화
  voxel_cloud_.Clear();
  ...
  RCLCPP_INFO(node_log(), "탐색 시작: 물체=[%.3f, %.3f, %.3f]", object_position[0],
...
void ShapeEstimationNode::ExploreLoopCallback() {
  ...
  auto result = motion_generator_.Step(latest_snapshot_, latest_estimate_, current_pose, dt);
  ...
  // 시각화
  {
    ...
    ros_stamp.nanosec = static_cast<uint32_t>(stamp_now.nanoseconds() % 1'000'000'000LL);

    explore_status_pub_->publish(
        viz::BuildExploreStatusMarkers(result.phase, motion_generator_.Stats(), latest_estimate_,
                                       current_pose, ros_stamp, frame_id_));
    explore_status_pub_->publish(viz::BuildTargetGoalMarker(result.goal, ros_stamp, frame_id_));
  }
  ...
  if (result.phase == ExplorePhase::kSucceeded) {
    ...
    StopExploration();
  } else if (result.phase == ExplorePhase::kFailed || result.phase == ExplorePhase::kAborted) {
...
void ShapeEstimationNode::SendActionResult(bool success, const std::string& message) {
  if (!active_goal_handle_)
    return;
...
void ShapeEstimationNode::PublishActionFeedback(ExplorePhase phase, const std::string& status_msg) {
  if (!active_goal_handle_)
    return;
...
void ShapeEstimationNode::ObjectPoseCallback(geometry_msgs::msg::PoseStamped::SharedPtr msg) {
  latest_object_position_ = {msg->pose.position.x, msg->pose.position.y, msg->pose.position.z};
...
ExplorationConfig ShapeEstimationNode::LoadExplorationConfig() {
  ...
  config.tilt_steps = static_cast<uint32_t>(get_parameter("exploration.tilt_steps").as_int());
...
void ShapeEstimationNode::RewireControllerTopics(const std::string& ctrl_name) {
  ...
  RCLCPP_INFO(node_log(), "rewired controller-owned topics to '%s'", ctrl_name.c_str());
}
