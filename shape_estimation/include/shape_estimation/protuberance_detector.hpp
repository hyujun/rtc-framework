...
class ProtuberanceDetector {
 public:
  explicit ProtuberanceDetector(const ProtuberanceConfig& config = ProtuberanceConfig{});

  // 메인 인터페이스: 피팅된 primitive + 포인트 + 최근 snapshot → 돌출 탐지
  [[nodiscard]] ProtuberanceResult Detect(const ShapeEstimate& fitted_primitive,
 ...
 private:
  ...
  // 양수 = 내부 (정상), 음수 = 외부 (돌출)
  [[nodiscard]] static double SignedDistanceFromPrimitive(const Eigen::Vector3d& point,
  ...
  [[nodiscard]] std::vector<GapEvent> DetectGaps(const std::vector<ToFSnapshot>& snapshots) const;

  // ── Step 4: 클러스터 + Gap 통합 → Protuberance 생성 ──────────────────────
  [[nodiscard]] Protuberance BuildProtuberance(const std::vector<uint32_t>& cluster_indices,
                                               const std::vector<PointResidual>& residuals,
                                               const ShapeEstimate& primitive,
                                               const std::vector<GapEvent>& all_gaps) const;

  // Primitive 표면의 바깥 방향 법선
  [[nodiscard]] static Eigen::Vector3d PrimitiveOutwardNormal(const Eigen::Vector3d& point,
                                                              const ShapeEstimate& primitive);

  // 클러스터의 표면 투영 크기 계산
  [[nodiscard]] static double ComputeSurfaceExtent(const std::vector<uint32_t>& cluster_indices,
                                                   const std::vector<PointResidual>& residuals);

  // 신뢰도 계산
  [[nodiscard]] double ComputeConfidence(uint32_t num_points, double protrusion_depth,
                                         bool has_gap) const;
};
