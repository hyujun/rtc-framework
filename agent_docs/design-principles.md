...
바로 위 규칙이 controller-owned 토픽이 **어디에 사는가**를 정한다면, 이 규칙은 **누가 그것을 만드는가**를 정한다: `rtc_controllers` 는 **제어 법칙(알고리즘)만** 소유한다. 자기 노드도, publisher 도, subscription 도 만들지 않으며 **`RTControllerInterface` 를 상속하지도 않는다.** ROS 배선은 전적으로 integration 패키지(`integrated_bringup/src/support/owned_topics.cpp` 의 `CreateOwnedTopics()`)가 소유한다.
...
### 완료 상태의 검증 기준 (수는 박제하지 않는다 — AP-DOC-1)

**`rtc_controllers` 는 순수하다.** 이 패키지에는 `RTControllerInterface` 구체 구현이 없고 `package.xml` 도 `rtc_controller_interface` 를 의존하지 않는다 — `grep -rn "public RTControllerInterface" rtc_controllers/include` 와 `grep -n rtc_controller_interface rtc_controllers/package.xml` 이 **둘 다 비어야** 한다.
...
- 수렴 이력(슬라이스 S1–S7 · DLS 축 #282 · 태스크 속도 축 #314)과 이 규칙의 결정·개정 로그는 issue #236 코멘트에 있다 — 여기 복제하지 않는다.
