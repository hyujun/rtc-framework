...
| DemoComplianceController | Position | Cartesian + Hand | **#469 S2 시점에는 `DemoTaskController` 의 rename-only 사본**이고 그 이상이 아니다 — §7 task-space admittance 는 S3 에서 결합된다. 그때까지의 계약은 "같은 입력에 같은 출력" 하나뿐이고 `integrated_bringup/test/test_compliance_task_equivalence.cpp` 가 그것만 검사한다 (이 패키지의 per-controller 스위트는 전부 출하 이름을 손으로 나열하므로 새 컨트롤러는 그 밖에서는 실행되지 않는다). 복사인 이유·상속과 추출을 기각한 근거는 `demo_compliance_controller.hpp` 배너. S2 가 실제로 추가한 설정면은 **`external_wrench.source`** 하나 (기본값 없이 필수, 오늘 유일한 합법값 `pull_estimator` — 미구현 `momentum_observer` 는 오타와 같은 경로로 configure 거부) 이고, p1b 는 `pull_estimator.use_baseline_subtraction: true` 를 **이 컨트롤러에서만** override 한다 (D-A8: shared 블록은 측정용 설정이라 건드리지 않는다). S3 준비 단계에서 두 가지가 더 갈라졌다 — (a) task 게인의 **철자**가 §7 스키마를 따른다 (`ik_kp_pos` / `ik_kp_rot` / `nullspace_kp`, 형제는 `kp_translation` / `kp_rotation` / `null_kp`): `ParseTaskAdmittanceParams` 가 이 컨트롤러의 노드를 읽으므로 두 철자를 다 두면 파싱되고 아무도 안 읽는 키가 생긴다 (D-A13, 값은 여전히 형제와 같아야 하고 등가성 테스트가 쌍으로 고정). ROS 파라미터·GUI 행 이름도 같이 옮겨갔다. (b) 같은 파서가 configure 에서 돌므로 §7 스키마 위반 (`desired_inertia` ≤ 0, `external_wrench.enabled: false`, `command_type: torque`) 이 bring-up 을 거부한다 — 마지막 항목은 **좁힌 것**이다 (바인딩 자체 리더는 torque 를 받아들였다) |
