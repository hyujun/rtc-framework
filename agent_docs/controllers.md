...
| DemoInferenceController | Position | Joint (policy-driven) | ONNX 정책 바인딩. 입력 텐서의 **이름·순서·폭·정규화·모델 경로가 전부 YAML** 이라 모델 교체에 C++ 재컴파일이 없다 (스키마 SSoT `rtc_controllers/params/policy_io_params.hpp`, 마샬링 코어 `rtc_controllers/inference/policy_io.hpp`). 스키마는 텐서 단위 (`inputs:`/`outputs:`) 이고 이름이 필수라 엔진이 이름으로 바인딩한다 — 바인딩 자체는 아직 입력 텐서 1개만 packing 한다 (#511 P4 가 N-텐서로 일반화). 정책은 `decimation` tick 마다 1회 돌고 그 사이 action 을 hold — sim 이 lock-step 이라 500 Hz / 10 = 20 ms sim-time/step 이고, 이 방식이 `control_rate` 를 낮추는 대안보다 나은 이유는 후자가 `ControllerState::dt` 를 실제 진행 sim-time 과 어긋나게 만들기 때문 (rate 파생 법칙이 전부 10배 스케일). 출력은 head 2개 — 팔은 **absolute** joint target, 손은 스칼라 1개로 YAML `hand_posture.open`↔`close` 를 보간. 실패(입력 unreadable · `Run()` false · 비유한 출력)는 **전 device 를 측정 q 로 hold** (부분 적용 없음); 스칼라 범위 이탈만 hold 가 아니라 clamp + 1회 경고 (정규화 불일치 신호). 엔진은 생성자 주입 — stub `OnnxEngine::Init()` 이 조용한 no-op 이라 `is_initialized()` 게이트가 configure 를 막고, 테스트는 fake engine 으로 모델 없이 success path 를 돈다 |
