...
|--------|------|----------------|
| RT controller manager | `integrated_bringup/config/ur5e_robot.yaml` / `ur5e_sim.yaml` | `control_rate`, `initial_controller`, `devices`, `urdf`, `device_timeout_*` |
...
| MuJoCo simulator (UR5e robot 오버레이) | `integrated_bringup/config/mujoco_simulator.yaml` | `model_path`, `robot_response.groups` (`ur5e`, `hand`), joint names, command/state 토픽 |
...
| Digital twin (UR5e overlay) | `integrated_bringup/config/digital_twin_ur5e.yaml` | `robot_description_*`, `source_*.topic`, `sensor_viz.sensor_topic`, `sensor_viz.fingertip_names`, `tcp_viz.source_topic` |
