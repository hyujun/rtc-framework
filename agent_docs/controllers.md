...
| DemoCatchingController | Position | Joint (arm hold) + Hand (step) | dynamic_catching S4.0 골격. 팔은 활성 시점 자세를 latch 해 유지하고, 손은 **무성형 계단** position 목표를 받는다 (`diagnostic.hand_step: true` 일 때만) — S4.2 가 `T_close,e2e` 를 재기 위한 진단용이다. `mujoco_native` backend 가 아니면 configure 를 거부한다 (**sim 전용**, E-8 승인 전). `ur5e_p1b`·`iiwa7_leap` 만 YAML 을 출하하므로 `_REQUIRING_CONFIG` 등록이다 |
