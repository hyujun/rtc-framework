...
| DemoInferenceController | Position | Joint (policy-driven) | ONNX 정책 바인딩. 입력 텐서의 **이름·순서·폭·정규화·모델 경로가 전부 YAML** 이라 모델 교체에 C++ 재컴파일이 없다 (스키마 SSoT `rtc_controllers/params/policy_io_params.hpp`, 마샬링 코어 `rtc_controllers/inference/policy_io.hpp`). 스키마는 텐서 단위 (`inputs:`/`outputs:`) 이고 이름이 필수라 엔진이 이름으로 바인딩하며, 입력 텐서 N개를 각자 버퍼에 packing 한다 (텐서마다 offset 공간·affine lane 이 독립). 정책은 `decimation` tick 마다 1회 돌고 그 사이 action 을 hold — sim 이 lock-step 이라 500 Hz / 10 = 20 ms sim-time/step 이고, 이 방식이 `control_rate` 를 낮추는 대안보다 나은 이유는 후자가 `ControllerState::dt` 를 실제 진행 sim-time 과 어긋나게 만들기 때문 (rate 파생 법칙이 전부 10배 스케일). 출력은 `output_features` 의 `{device, role}` 로 **선언 바인딩**된다 (suffix 추정 폐기 — 두 device 가 같은 역할을 선언하면 예전 루프는 마지막 것이 둘 다 구동했다, #511 B-2). role 은 `joint_target` (그 device 폭만큼의 absolute joint target) 과 `posture_scalar` (스칼라 1개로 YAML `hand_posture.open`↔`close` 보간, 손 전용) 둘이고, 손이 `joint_target` 을 택하면 `hand_posture` 블록 자체가 불요다. 팔은 `joint_target` 필수, 손은 두 역할 중 정확히 하나. **recurrent 는 선택** — 입력의 `source: recurrent` 를 출력의 `feeds:` 가 먹이며, 되먹임은 엔진 버퍼→엔진 버퍼라 은닉 폭이 얼마든 컨트롤러 메모리를 안 늘린다. state 는 액션 전체가 수락됐을 때만 전진하고, 비유한 state 는 얼리지 않고 0 리셋하며 (얼리면 이후 전 step NaN → 조용한 영구 hold), 긴 hold 후 리셋은 `reset_after_hold_sec` (기본 0.1 s, `0`=모든 hold, 음수=안 함; activation 리셋은 무조건). 실패(입력 unreadable · `Run()` false · 비유한 출력)는 **전 device 를 측정 q 로 hold** (부분 적용 없음); 스칼라 범위 이탈만 hold 가 아니라 clamp + 1회 경고 (정규화 불일치 신호). 엔진은 생성자 주입 — stub `OnnxEngine::Init()` 이 조용한 no-op 이라 `is_initialized()` 게이트가 configure 를 막고, 테스트는 fake engine 으로 모델 없이 success path 를 돈다 |
