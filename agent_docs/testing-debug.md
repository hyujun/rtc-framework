...
| `repo_scripts/` | `test_rt_common` + `test_install_deps` (ONNX tarball digest 검증) + `test_install_python` (venv base · CMake Python = 배포판 python) + shell unit test. CI 는 `docs-validate.yml` 이 매 PR 돌린다 — `test/test_*.sh` · 도메인·fixture 검사기 · 추적되는 `*.sh` 전체 shellcheck (Stop hook 이 셸 파일을 통째로 검사해도 되는 전제). 도메인·fixture 검사기는 Stop hook 도 돌린다 (트리거·범위: hook 헤더 Phase 1d) — CI 에서만 돌 때는 다른 패키지와의 충돌이 PR 에서야 드러났다 (#513, #571) | `check_rt_setup.sh --summary` |
