...
| `repo_scripts/` | `test_rt_common` + `test_install_deps` (ONNX tarball digest 검증) + `test_install_python` (venv base · CMake Python = 배포판 python) + shell unit test. CI 는 `docs-validate.yml` 이 매 PR 돌린다 — `test/test_*.sh` · 도메인·fixture 검사기 · 추적되는 `*.sh` 전체 shellcheck (Stop hook 이 셸 파일을 통째로 검사해도 되는 전제) | `check_rt_setup.sh --summary` |
