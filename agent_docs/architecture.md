...
MPC solve-timing observability: `RTControllerInterface::GetMpcSolveStats()` returns `std::optional<rtc::MpcSolveStats>` (default `nullopt`; `DemoWbcController` overrides). `RtControllerNode` polls at 1 Hz on `cb_group_aux_`, appends to `<session>/controller/mpc_solve_timing.csv` (9 columns, `t_wall_ns,count,window,last_ns,min_ns,p50_ns,p99_ns,max_ns,mean_ns`), logs `p50/p99/max` every 10 s via `RCLCPP_INFO`. Underlying ring is `MPCSolutionManager::GetSolveStats()` (256-sample window ≈ 12 s @ 20 Hz).
