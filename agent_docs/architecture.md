...
MPC solve-timing observability has two parallel paths sharing one source (`MPCSolutionManager::PublishSolution`):
1. **Per-tick raw stream** — `MPCSolutionManager` pushes a `rtc::MpcSolveSample {t_wall_ns, count, solve_ns}` onto a lock-free SPSC (`SpscQueue<MpcSolveSample, 128>`) on every solve. `RTControllerInterface::DrainMpcSolveSamples()` (default no-op; `DemoWbcController` overrides) lets a non-RT consumer drain in FIFO order. The same 1 Hz aux timer that spawns in `on_activate` (when `mpc_enabled_`) drains the SPSC and appends one CSV row per MPC tick to `<session>/controllers/<config_key>/mpc_solve_timing.csv` (3 columns, `t_wall_ns,count,solve_ns`). Writer: `rtc_mpc/logging/mpc_solve_timing_logger.hpp`. Drop count: `MPCSolutionManager::SolveSampleDropCount()`.
2. **Windowed aggregate** — `RTControllerInterface::GetMpcSolveStats()` returns `std::optional<rtc::MpcSolveStats>` from `MPCSolutionManager::GetSolveStats()` (256-sample sliding window ≈ 12 s @ 20 Hz). Used only for the periodic `RCLCPP_INFO` summary (every 10 s) — no longer written to disk; post-process the per-tick CSV for any percentile/window.
