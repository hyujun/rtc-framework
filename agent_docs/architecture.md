...
- **Core 0 reserved** for OS / DDS / IRQ only — ≥ 6-core 모든 tier 에서 nrt_logging / nrt_callback / nrt_publish 가 Core 0 와 분리 (v4.1). layout v5 (#349) 부터 그 세 CFS lane 은 전용 코어 대신 **Core 2 (aux slot, `rt_callback` 과 동거)** 에 얹힌다 — 실기 실측 duty 는 Core 2 ≈1.5% + nrt 합계 ≈5.6% → ≈7%. 반환된 슬롯은 system cpuset 으로 돌아가고 cset shield 가 RT cluster 로 좁혀진다 (NUC13 `2-9,12-13` → `2-9`, 이어서 #380 이 mpc_worker 슬롯을 회수해 **`2-7`**). 4-core fallback 은 이미 nrt 가 OS slot 을 공유하므로 **불변**
