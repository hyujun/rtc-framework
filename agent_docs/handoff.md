...
| 같은 task · ownership 유지, context 압박만 | **compact** (Claude 메커니즘은 [CLAUDE.md](../CLAUDE.md) §Claude Code). durable 결정·evidence 손실 위험이 있을 때만 artifact 생성/갱신 |
...
- [ ] Evidence 는 실제 실행 결과만 — `done` 을 쓰기 전에 build/test 로 검증한다 (self-eval 은 신뢰 불가; [../AGENTS.md](../AGENTS.md) §5.5)
...
- **진행 중 plan** → 각 에이전트의 private 저장소. Claude → `~/.claude/plans/<slug>.md` ([../AGENTS.md](../AGENTS.md) §6.5 Sprint Contract spec 과 동일 파일에 누적), Codex → 자기 home. repo 에 커밋하지 않는다.
...
- **정리(retention)** → 완료된 private plan 은 그 내용이 git log / issue / memory 로 복원 가능하거나 보존할 가치가 없으면 **삭제**한다 ([../AGENTS.md](../AGENTS.md) §11). 복원 불가한데 보존 가치가 있는 결정 기록이 남아 있으면 issue 코멘트로 옮긴 뒤 삭제한다.
