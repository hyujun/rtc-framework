...
- **Include grouping**: `.clang-format` `IncludeBlocks: Regroup` + `IncludeCategories` 가 4 Priority 로 분류 — **P1** 본 저장소 프로젝트 헤더 (`"rtc_*"`/`"ur5e_*"`/`"shape_estimation*"`/`"integrated_bringup*"`/`"udp_hand_driver*"`/`"robot_descriptions*"`, quote/angle 둘 다), **P2** ROS 2 (`<rclcpp/*>`, `<std_msgs/*>` 등), **P3** third-party (Eigen, pinocchio, fmt, MuJoCo, BehaviorTree 등), **P4** stdlib (`<algorithm>` 등). 같은 Priority 안 blank line 은 clang-format 이 강제 collapse — own header (`"foo.hpp"`) 와 `<rtc_base/...>` 가 같은 P1 이라 둘 사이 빈 줄은 의도된 제거다 (`/review` 가 "include grouping 부족" 으로 지적하면 거절 정당). 분리가 필요하면 `.clang-format` 패턴 추가가 정답이지 manual blank line 아님. `#ifdef ... #endif` 사이에 끼면 clang-format 이 grouping 못 해 blank line 살아 있음.
