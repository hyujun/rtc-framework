...
- **Lifecycle**: 핵심 C++ 노드는 `rclcpp_lifecycle::LifecycleNode`. Empty constructor; `on_configure` (Tier 1) / `on_activate` (Tier 2). Safety publishers은 standalone `rclcpp::create_publisher` 사용. 어떤 노드가 LifecycleNode 인지는 [architecture.md](architecture.md) 참조 — 개수 박제 금지 ([anti-patterns.md](anti-patterns.md) AP-DOC-1)
    - **Tier 0 예외 — 프로세스 스레드 레이아웃**: `main()` 이 spin 하기 전에 알아야 하는 것, 즉 어떤 lifecycle transition 보다도 먼저 확정돼야 하는 것만 생성자에 둘 수 있다. 구체적으로 ① executor 에 넘길 callback group (on_cleanup→on_configure 사이클에서 같은 객체여야 하므로 configure 에서 만들 수 없다) 과 ② 그 스레드를 어디에 pin 할지 정하는 파라미터 declare 뿐이다. **리소스 (publisher / subscription / timer / 디바이스 핸들) 는 여전히 금지** — 예외의 근거는 "생성자가 편해서" 가 아니라 "configure 가 오기 전에 이미 필요해서" 이고, 후자가 아니면 Tier 1 이다. 현재 유일한 사례는 `UdpHandNode` (`cb_group_aux_` + `aux_cpu_slot`·`use_cpu_affinity`, issue #345). 이 예외를 쓰는 노드는 생성자에 *왜* 그 항목이 configure 를 기다릴 수 없는지 한 줄로 남긴다
...
- **Merge method** — **기본은 no-ff merge commit** (`gh pr merge --merge`, 사용자 확정 2026-08-16): 머지 커밋이 PR 단위를 히스토리에 남기고, 브랜치 커밋 본문(PROC-6 spec-change 근거 같은 감사 기록)이 `main` 에 그대로 보존된다. **squash 는 커밋 본문을 PR 본문으로 덮어쓰므로** rebase 와 함께 사용자가 명시 지시할 때만 쓴다 (선례: #442 rebase · #450 no-ff; #464 까지는 squash 관례였고 `(#N)` 접미사가 그 흔적). `delete_branch_on_merge=false` 이므로 `gh pr merge --delete-branch` 로 로컬·원격 브랜치를 함께 정리한다.
