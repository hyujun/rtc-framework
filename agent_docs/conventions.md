...
- **Formatting SSoT**: C++ 는 [`.clang-format`](../.clang-format) (Google base, ColumnLimit 100, PointerAlignment Left), Python 은 [`pyproject.toml`](../pyproject.toml) (ruff, line 99). PostToolUse hook ([.claude/hooks/format-code.sh](../.claude/hooks/format-code.sh)) 이 모든 Edit 마다 자동 적용. **`ament_uncrustify` 사용 금지** — ROS 2 / Eclipse-CDT 표준 스타일 (Allman braces, `T *p`, `T &r`, `template <T>`) 이 Google base 와 호환 불가능하므로 두 lint 가 서로 영원히 충돌한다. 워크스페이스 정책 (`bdedac7`, 2026-05-16): `ament_lint_common` meta + `ament_lint_auto` 패턴 자체를 사용하지 않는다 — `set(ament_cmake_uncrustify_FOUND TRUE)` 스킵 트릭은 *작동하지 않는다*. 새 패키지는 개별 lint depend 만 선언한다: `<test_depend>ament_cmake_cppcheck</test_depend>`, `<test_depend>ament_cmake_lint_cmake</test_depend>`, `<test_depend>ament_cmake_xmllint</test_depend>`. CMakeLists 에는 `find_package(ament_cmake_cppcheck REQUIRED)` 등 + `ament_cppcheck()` / `ament_lint_cmake()` / `ament_xmllint()` 개별 호출.
