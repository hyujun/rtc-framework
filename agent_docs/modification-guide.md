...
1. Header in `rtc_controllers/include/rtc_controllers/{direct|indirect}/` -- inherit `RTControllerInterface`, implement `Compute()`, `SetDeviceTarget()`, `InitializeHoldPosition()`, `Name()` (all `noexcept`). **Layering rule** ([design-principles.md](design-principles.md) §Backend / Controller Layering): controllers read only the `DeviceStateCache` slots they consume — derived quantities (`force_magnitude`, `in_contact`, slip, ...) are computed in the controller, never pulled from the backend, and never added to `DeviceStateCache`.
