...
4. YAML in `rtc_controllers/config/controllers/` -- must include `topics:` section. Tag non-RT external topics (target / gui_position / grasp_state / tof_snapshot / ...) with `ownership: controller` and use **relative paths** (`ur5e/joint_goal`, not `/ur5e/joint_goal`). Manager-owned entries keep absolute paths and may omit `ownership` (defaults to `manager`). See `ur5e_bringup/support/owned_topics.hpp + src/support/owned_topics.cpp` for the standard create/activate/publish helper the 3 demos share.
