...
4. YAML in `rtc_controllers/examples/controllers/` (reference only) or `<robot>_bringup/config/controllers/` (production) -- must include `topics:` section. Tag non-RT external topics (target / gui_position / grasp_state / tof_snapshot / ...) with `ownership: controller` and use **relative paths** (`<robot>/joint_goal`, not `/<robot>/joint_goal`). Manager-owned entries keep absolute paths and may omit `ownership` (defaults to `manager`). See `integrated_bringup/support/owned_topics.hpp + src/support/owned_topics.cpp` for the standard create/activate/publish helper the 3 demos share.
