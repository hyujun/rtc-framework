...
namespace {
auto logger() {
  return ::rtc_bt::logging::CondLogger("check_shape_type");
}
...
BT::PortsList CheckShapeType::providedPorts() {
  return {
      BT::InputPort<shape_estimation_msgs::msg::ShapeEstimate>(
          "estimate", "Shape estimate from WaitShapeResult"),
...
BT::NodeStatus CheckShapeType::tick() {
  ...
  if (!estimate) {
    RCLCPP_ERROR(logger(), "missing estimate port: %s", estimate.error().c_str());
  ...
  RCLCPP_INFO(logger(), "type=%s (%u), confidence=%.3f, points=%u", name.c_str(), est.shape_type,
              est.confidence, est.num_points_used);
  ...
  if (expected) {
    ...
    if (expected_lower != name) {
      RCLCPP_WARN(logger(), "shape mismatch (expected=%s, got=%s, confidence=%.3f)",
