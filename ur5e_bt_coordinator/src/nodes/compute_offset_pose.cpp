...
namespace {
auto logger() {
  return ::rtc_bt::logging::ActionLogger("compute_offset_pose");
}
...
BT::NodeStatus ComputeOffsetPose::tick() {
  ...
  if (mode == "add") {
    ...
    const Eigen::Quaterniond q_res =
        (mode == "quat_body") ? (q_cur * q_off).normalized() : (q_off * q_cur).normalized();
  ...
  RCLCPP_DEBUG(logger(),
               ...
               mode.c_str(), dx, dy, dz, dr, dp, dy_rot, result.x, result.y, result.z, result.roll,
