...
namespace {
auto logger() {
  return ::rtc_bt::logging::ActionLogger("flex_extend_finger");
}
}  // namespace
...
BT::NodeStatus FlexExtendFinger::onStart() {
  ...
  flex_duration_ =
      EstimateHandTrajectoryDuration(current, flex_target_, joint_indices_, speed_, max_vel_);
  ...
  RCLCPP_INFO(logger(), "finger=%s flex_duration=%.3fs (speed=%.2f)", finger_name_.c_str(),
              flex_duration_, speed_);
...
BT::NodeStatus FlexExtendFinger::onRunning() {
  ...
  if (phase_ == Phase::kFlex && elapsed >= flex_duration_) {
    ...
    extend_duration_ =
        EstimateHandTrajectoryDuration(current, home_target_, joint_indices_, speed_, max_vel_);

    ApplyPartialHandTarget(*bridge_, home_target_, joint_indices_);

    RCLCPP_INFO(logger(), "finger=%s extend phase (%.3fs)", finger_name_.c_str(), extend_duration_);
