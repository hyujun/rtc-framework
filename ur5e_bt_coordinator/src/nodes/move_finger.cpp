...
BT::NodeStatus MoveFinger::onStart() {
  ...
  if (!finger_name) {
    throw BT::RuntimeError("MoveFinger: missing finger_name: ",
                           finger_name.error());
  }
  ...
  joint_indices_ =
      LookupOrThrow(kFingerJointIndices, finger_name.value(), "MoveFinger");
  ...
  RCLCPP_INFO(
      logger(), "finger=%s pose=%s estimated_duration=%.3fs (speed=%.2f)",
...
void MoveFinger::onHalted() { RCLCPP_INFO(logger(), "halted"); }
