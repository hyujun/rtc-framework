...
namespace {
auto logger() {
  return ::rtc_bt::logging::CondLogger("is_grasp_phase");
}
...
BT::NodeStatus IsGraspPhase::tick() {
  ...
  const char* current_str =
      (current_name != kPhaseNames.end()) ? current_name->second.c_str() : "Unknown";

  RCLCPP_DEBUG(logger(), "current=%s(%d) target=%s match=%s", current_str, gs.grasp_phase,
