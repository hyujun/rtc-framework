...
namespace {
auto logger() {
  return ::rtc_bt::logging::ActionLogger("wait_shape_result");
}
...
BT::NodeStatus WaitShapeResult::onStart() {
  ...
  RCLCPP_INFO(logger(), "waiting (confidence>=%.2f, timeout=%.1fs)", confidence_threshold_,
              timeout_s_);
...
BT::NodeStatus WaitShapeResult::onRunning() {
  ...
  if (bridge_->GetShapeEstimate(estimate)) {
    if (estimate.confidence >= confidence_threshold_ && estimate.shape_type != 0 /* UNKNOWN */) {
      setOutput("estimate", estimate);
      RCLCPP_INFO(logger(), "success: %s (confidence=%.3f, points=%u, elapsed=%.2fs)",
                  ShapeTypeName(estimate.shape_type), estimate.confidence, estimate.num_points_used,
    ...
    RCLCPP_DEBUG_THROTTLE(logger(), steady_clock, ::rtc_bt::logging::kThrottleFastMs,
                          "pending: %s (confidence=%.3f, points=%u)",
                          ShapeTypeName(estimate.shape_type), estimate.confidence,
                          estimate.num_points_used);
  }
  ...
  if (ElapsedSeconds(start_time_) > timeout_s_) {
    ...
    if (bridge_->GetShapeEstimate(estimate)) {
      setOutput("estimate", estimate);
      RCLCPP_WARN(logger(), "timeout with partial: %s (confidence=%.3f)",
                  ShapeTypeName(estimate.shape_type), estimate.confidence);
    } else {
      RCLCPP_WARN(logger(), "timeout with no estimate");
    }
