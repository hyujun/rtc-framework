...
namespace {
auto logger() {
  return ::rtc_bt::logging::ActionLogger("track_trajectory");
}
}  // namespace
...
BT::NodeStatus TrackTrajectory::onStart() {
  ...
  RCLCPP_INFO(logger(), "%zu waypoints tol=%.4f timeout=%.1fs", waypoints_.size(), pos_tol_,
              timeout_s_);
...
BT::NodeStatus TrackTrajectory::onRunning() {
  if (ElapsedSeconds(start_time_) > timeout_s_) {
    ...
    double err =
        current_idx_ < waypoints_.size() ? cur.PositionDistanceTo(waypoints_[current_idx_]) : -1.0;
    RCLCPP_WARN(logger(), "timeout (%.1fs) at waypoint %zu/%zu (last_err=%.4f tol=%.4f)",
  ...
  if (err < pos_tol_) {
    ...
    if (current_idx_ >= waypoints_.size()) {
      RCLCPP_INFO(logger(), "all %zu waypoints reached (elapsed=%.2fs)", waypoints_.size(),
                  ElapsedSeconds(start_time_));
      return BT::NodeStatus::SUCCESS;
    }
    RCLCPP_DEBUG(logger(), "waypoint %zu/%zu reached (err=%.4f)", current_idx_, waypoints_.size(),
                 err);
...
void TrackTrajectory::onHalted() {
  RCLCPP_INFO(logger(), "halted at waypoint %zu/%zu (elapsed=%.2fs)", current_idx_,
