...
Per-fingertip force magnitude와 aggregate grasp detection 결과가 포함되어 있어
...
| Node | Description |
|------|-------------|
| `IsForceAbove` | Fingertip force가 threshold 초과 (500Hz 사전 계산 활용, sustained 판정 지원) |
| `IsGrasped` | 물체 파지 상태 확인 (500Hz 사전 계산된 grasp_detected 활용) |
...
| `rtc_msgs` | GuiPosition, GraspState |
| `tf2` | 쿼터니언 → RPY 변환 |
...
│   ├── bt_types.hpp                 # Pose6D, CachedGraspState, BT conversions
