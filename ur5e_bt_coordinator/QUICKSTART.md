...
`config/poses.yaml`에서 재컴파일 없이 포즈를 수정할 수 있습니다:
...
| 증상 | 원인 | 해결 |
|------|------|------|
| `[Watchdog] /ur5e/gui_position: no messages` | RT 컨트롤러 미실행 | `rt_controller` 노드 먼저 시작 |
...
| `E-STOP active` | 비상 정지 활성화 | E-STOP 해제 후 자동 재개 |
