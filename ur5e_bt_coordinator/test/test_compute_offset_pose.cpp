...
class ComputeOffsetPoseTest : public ::testing::Test {
 protected:
  void SetUp() override { factory_.registerNodeType<ComputeOffsetPose>("ComputeOffsetPose"); }
...
TEST_F(ComputeOffsetPoseTest, QuatBodyAndWorldDifferForNonCommuting) {
  ...
  auto body_tree = CreateTree(R"(<ComputeOffsetPose input_pose=")" + base +
                              R"(" offset_roll="0.4" offset_pitch="0.0" offset_yaw="0.0"
              rotation_mode="quat_body"
              output_pose="{out}"/>)");
  auto world_tree = CreateTree(R"(<ComputeOffsetPose input_pose=")" + base +
                               R"(" offset_roll="0.4" offset_pitch="0.0" offset_yaw="0.0"
