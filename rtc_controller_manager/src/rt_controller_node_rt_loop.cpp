...
void RtControllerNode::ControlLoop()
{
  ...
  // Push per-tick timing sample (independent stream — generic
  // ThreadTimingProducer<CmTimingPayload>). Drained by DrainLog into
  // timing_log.csv. Always-on; cheap and not gated on enable_logging_
  // (the device CSVs are gated separately below).
  static_cast<void>(cm_timing_producer_.Push(rtc::CmTimingPayload{
...
void RtControllerNode::DrainLog()
{
  ...
  // Per-tick timing CSV: drain SPSC into timing_log.csv. Same generic
  // pattern the controller LifecycleNode aux thread uses for
  // mpc_solve_timing.csv — see rtc_base/timing/thread_timing_*.hpp.
  if (cm_timing_logger_.IsOpen()) {
    cm_timing_producer_.Drain(
      [this](const rtc::CmTimingSample & s) {cm_timing_logger_.Log(s);});
  }
