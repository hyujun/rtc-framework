    ...
    auto publish_entry = [&](const urtc::PublishTopicEntry& pt, std::size_t group_idx) {
      ...
      switch (pt.role) {
        ...
        case urtc::PublishRole::kGuiPosition: {
          ...
          const auto n_actual =
              std::min(static_cast<std::size_t>(gc.actual_num_channels), m.joint_positions.size());
        ...
        case urtc::PublishRole::kRobotTarget: {
          ...
          std::copy(snap.task_goals[group_idx].begin(), snap.task_goals[group_idx].end(),
                    m.task_target.begin());
          it->second.publisher->publish(m);
          return;
        }
        ...
        case urtc::PublishRole::kToFSnapshot: {
          ...
          const int num_poses = std::min(ts.num_fingers, static_cast<int>(m.tip_poses.size()));
