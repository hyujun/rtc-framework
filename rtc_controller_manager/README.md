...
3. **컨트롤러 bring-up은 3-pass 구조** (`on_configure` 시점에 `device_name_configs_` 가 이미 채워져 있도록 보장):
   * **Pass 1 — 인스턴스화 + PreConfigure**: 팩토리 생성 → `SetSystemModelConfig()` → `SetSharedModelBuilder(shared)` → 전용 `rclcpp_lifecycle::LifecycleNode`(이름 = `config_key`, 네임스페이스 = `/<config_key>`, `NodeOptions().use_global_arguments(false)`) 생성 → `SetTargetReceivedNotifier(...)` 주입 → `PreConfigure(node, yaml)` 호출 (base가 `LoadConfig(yaml)` 만 실행 → `topic_config_` 채움; **RegisterLog / 리소스 할당 없음**) → `controllers_` + `controller_nodes_` + 임시 `controller_yamls` 보관
...
|-----------|-----------|-----------|---------|
| `manager` (기본) | `RtControllerNode` | HW/sim ↔ controller 경계 트래픽: `state`, `motor_state`, `sensor_state`, `joint_command`, `ros2_command` (+ digital twin republish, `/system/estop_status`, `/rtc_cm/active_controller_name`) | CM 구독 / SPSC drain → `LifecyclePublisher` |
| `controller` | 컨트롤러별 `LifecycleNode` | 컨트롤러 입력/출력: `target`, `gui_position`, `grasp_state`, `wbc_state`, `tof_snapshot` | 컨트롤러가 on_configure에서 sub/pub 생성, publish 스레드가 SPSC 소비 후 `controllers_[active]->PublishNonRtSnapshot(snap)` 호출 |

원칙: CM은 **HW/sim과 controller 사이의 boundary 토픽** + **controller-node 관리** (switch / active / E-STOP) 만 소유한다. 컨트롤러가 생산하거나 소비하는 의미적 데이터(목표·GUI·grasp·wbc·tof) 는 controller-owned 로 분류해 컨트롤러 LifecycleNode가 직접 sub/pub. CM은 YAML `ownership: controller` entry에 대해 sub/pub 생성과 publish 디스패치를 건너뜁니다. Controller-owned target 구독 콜백은 `NotifyTargetReceived()`를 호출해 CM의 `target_received_` 게이트를 한 번 전환합니다.
...
3. `BuildDeviceSnapshot(idx)` — `state_received_` true이면 SeqLock에서 캡처, 아니면 empty
4. target controller `on_activate(prev_state, snapshot)` 호출 (D-A5: hold-init은 base가 처리)
5. `active_controller_idx_.store(idx, release)` — RT loop dispatch atomic 전환
6. RT tick 1회 대기 (`sleep_for(1.5 × dt)`, OQ-2 = sleep_for) — F-3 race benign
7. previous controller `on_deactivate(prev_state)` 호출
8. `/{robot_ns}/active_controller_name` 퍼블리시 (TRANSIENT_LOCAL, rewire trigger 영구 유지)
9. response `ok=true, message="ok"` (또는 "switched -> name")
...
모두 `cb_group_aux_` (RT path와 분리), `rmw_qos_profile_services_default`.

### 게인 채널 (per-controller ROS 2 parameter)

Phase A~E 마이그레이션 (2026-04-26) 이후 게인 채널은 **CM 소유가 아니다**. 각 컨트롤러 LifecycleNode (`/<config_key>`) 가 자체 `declare_parameter` + `add_on_set_parameters_callback`로 노출하며, parameter 콜백은 `aux_executor` 위에서 실행되어 SeqLock writer로 mutate→Store 한다 (RT path는 `Load()` 스냅샷만 사용). `~/controller_gains` / `~/request_gains` / `~/current_gains` 토픽과 `RTControllerInterface::UpdateGainsFromMsg`/`GetCurrentGains` 가상 메서드는 이 단계에서 제거됨. 컨트롤러별 노출 항목은 [agent_docs/controllers.md](../agent_docs/controllers.md) §Gains.
...
| `/{robot_ns}/active_controller_name` | `String` | TRANSIENT_LOCAL/1 | 활성 컨트롤러 이름 (downstream BT/GUI rewire trigger) |
...
CM이 생성하는 동적 퍼블리셔는 **HW/sim ↔ controller 경계 트래픽** 두 가지뿐입니다.

| 역할 | 메시지 타입 | QoS | 설명 |
|------|------------|-----|------|
| `kJointCommand` | `JointCommand` | BEST_EFFORT/1 | 관절 커맨드 (position/torque) → HW/sim |
| `kRos2Command` | `Float64MultiArray` | RELIABLE/1 | ros2_control forward bridge → sim |

다른 publish 역할 (`kGuiPosition` / `kRobotTarget` / `kGraspState` / `kWbcState` / `kToFSnapshot`)은 모두 **controller-owned 전용**입니다. 컨트롤러 LifecycleNode가 `on_configure`에서 직접 퍼블리셔를 생성하고 `PublishNonRtSnapshot()` 안에서 발행합니다 (CM은 SPSC snapshot 운반만 담당). YAML 에서 `ownership: manager` 로 설정하더라도 CM은 퍼블리셔를 만들지 않습니다.

`kGraspState` / `kWbcState` / `kToFSnapshot`은 single-producer / single-consumer 데이터 (active controller 1개의 own publisher만 읽음) 이므로, RT loop가 `output.grasp_state` / `output.wbc_state` / `output.tof_snapshot`을 SPSC snapshot에 채울 때 **그 role 의 publish entry가 선언된 device group 슬롯 1개에만** 복사합니다. 다른 group 슬롯은 zero-initialised 로 남아 의미 혼동을 막습니다. 장기적으로는 이 세 필드를 `PublishSnapshot`에서 분리해 컨트롤러가 자체 SeqLock으로 own publisher thread 에 직접 전달하는 방향으로 갈 예정입니다.
...
| 파라미터 | 타입 | 코드 기본값 | 설명 |
|---------|------|------------|------|
| `robot_namespace` | string | `""` | 매니저 레벨 토픽 네임스페이스 (`/{robot_ns}/...`). 비어 있으면 prefix 없이 publish. 로봇별 bringup이 override (예: `integrated_bringup` → `"ur5e"`) |
...
`rtc_controller_manager` 는 **default YAML을 동봉하지 않습니다**. 모든 파라미터는 `DeclareAndLoadParameters()` 가 robot-agnostic 코드 기본값(예: `robot_namespace=""`, `initial_controller=""`, `device_timeout_names=[]`, `urdf` / `devices` 미선언)으로 declare 합니다. 실제 production 값은 `<robot>_bringup` 패키지의 YAML이 단독 소유 — 예: `integrated_bringup/config/ur5e_{sim,robot}.yaml`. 새 robot 도입 시 자체 YAML을 만들어 launch에서 주입하세요.

UR5e bringup의 예시 YAML 구조 (`integrated_bringup/config/ur5e_sim.yaml`):

```yaml
/**:
  ros__parameters:
    robot_namespace: "ur5e"               # robot bringup이 명시 주입
    control_rate: 500.0
