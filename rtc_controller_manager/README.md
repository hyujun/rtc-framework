...
| `controller` | 컨트롤러별 `LifecycleNode` | `target`, `gui_position`, `grasp_state`, `wbc_state`, `tof_snapshot` 등 외부 GUI/BT용 non-RT 트래픽 | 컨트롤러가 on_configure에서 sub/pub 생성, publish 스레드가 SPSC 소비 후 `controllers_[active]->PublishNonRtSnapshot(snap)` 호출 |
...
| `kGraspState` | `GraspState` | RELIABLE/10 | 그래스프 감지 상태 (Force-PI grasp 컨트롤러) |
| `kWbcState` | `WbcState` | RELIABLE/10 | TSID-based WBC 컨트롤러 상태 (controller-owned only) |
