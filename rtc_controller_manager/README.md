...
2. 각 컨트롤러: 팩토리로 인스턴스 생성 -> `SetSystemModelConfig()` -> 전용 `rclcpp_lifecycle::LifecycleNode`(이름 = `config_key`, 네임스페이스 = `/<config_key>`, `NodeOptions().use_global_arguments(false)`) 생성 -> `SetTargetReceivedNotifier(...)` 주입 -> `on_configure(unconfigured, node, yaml)` 호출 (base 기본 구현이 `LoadConfig(yaml)`를 내부에서 try/catch로 실행) -> `controllers_` + `controller_nodes_` 벡터에 추가
...
5. `main()`에서 모든 `controller_nodes_`를 `aux_executor`에 attach — 컨트롤러가 자체 소유한 sub/pub 콜백이 non-RT 스레드에서 처리됨
6. CM `on_activate` -> 각 컨트롤러 `on_activate(active)` 호출(LifecyclePublisher 활성화 후) -> RT 루프 + publish 스레드 시작
7. CM `on_deactivate` -> RT/publish 스레드 정지 후 각 컨트롤러 `on_deactivate(inactive)` 호출
8. CM `on_cleanup` 진입 시 각 컨트롤러에 `on_cleanup(inactive)` 호출 후 `controller_nodes_`/`controllers_` 해제
