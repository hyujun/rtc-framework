...
| `controller` | 컨트롤러별 `LifecycleNode` | 컨트롤러 입력/출력: `target`, `gui_position`, `grasp_state`, `wbc_state`, `tof_snapshot`, `robot_transforms` | 컨트롤러가 on_configure에서 sub/pub 생성, publish 스레드가 SPSC 소비 후 `controllers_[active]->PublishNonRtSnapshot(snap)` 호출 |
