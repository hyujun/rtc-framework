...
E-STOP 안전 시스템, 전략 패턴 기반 다중 제어기(PD/Pinocchio/CLIK/OSC), MuJoCo 3.x 물리 시뮬레이터, UDP 핸드 인터페이스, CSV 데이터 로깅, GUI 도구를 포함한 완전한 실시간 제어 솔루션입니다.
...
| [`ur5e_rt_controller`](ur5e_rt_controller/) | 500Hz 실시간 위치 제어기 (PD/Pinocchio/CLIK/OSC) | ament_cmake |
| [`ur5e_hand_udp`](ur5e_hand_udp/) | 11-DOF 커스텀 핸드 UDP 브리지 | ament_cmake |
...
- **전략 패턴 제어기**: PDController, PController, PinocchioController, ClikController, OperationalSpaceController
...
- **UDP 핸드 통합**: 11-DOF 커스텀 핸드 데이터 수신/송신
...
[RtControllerNode]  ←  /target_joint_positions
    │  Strategy Pattern: PD / Pinocchio / CLIK / OSC
    │  4 Executor 스레드 (RT Core 2 / Sensor Core 3 / Log Core 4 / Aux Core 5)
    ▼
/forward_position_controller/commands → [UR 드라이버 / MuJoCo]

[커스텀 핸드] ←UDP→ [hand_udp_receiver / sender] ←ROS2→ [RtControllerNode]
```
