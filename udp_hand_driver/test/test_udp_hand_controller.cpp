// Unit tests for hand_controller.hpp — UdpHandController (fake_hand mode).
//
...
class FakeHandControllerTest : public ::testing::Test {
 protected:
  void SetUp() override {
    controller_ = std::make_unique<UdpHandController>("127.0.0.1",  // target_ip (unused in fake mode)
                                                   55151,        // target_port
...
TEST(HandControllerConfig, BulkMode) {
  auto ctrl =
      std::make_unique<UdpHandController>("127.0.0.1", 55151, rtc::kUdpRecvConfig, 10.0, false, 1, 4, true,
...
TEST(HandControllerConfig, NumFingertips_ClampedToNames) {
  ...
  auto ctrl = std::make_unique<UdpHandController>("127.0.0.1", 55151, rtc::kUdpRecvConfig, 10.0, false, 1,
