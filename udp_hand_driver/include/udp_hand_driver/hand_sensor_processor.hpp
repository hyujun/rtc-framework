...
class HandSensorProcessor {
 public:
  ...
  void Init() noexcept {
    ...
    const auto logger = ::udp_hand_driver::logging::SensorLogger();
    ...
    if (!filter_reinited_ && sensor_rate_estimator_.warmed_up()) {
      ...
      if (baro_filter_active_) {
        try {
          ...
          RCLCPP_WARN_THROTTLE(::udp_hand_driver::logging::SensorLogger(), filter_warn_clock_,
 ...
 private:
  ...
  void ThrottledDriftWarning() noexcept {
    ...
    RCLCPP_WARN_THROTTLE(::udp_hand_driver::logging::SensorLogger(), drift_warn_clock_,
