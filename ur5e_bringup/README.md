...
#### Gains Layout (`~/controller_gains` topic, 9 values)
...
- 7-entry 메시지와 **backwards-compatible**: 앞 7개 인덱스 동작이 동일하게 보존됨. 짧은 메시지는 기존 MPC 상태를 건드리지 않음.
...
- **`engine: "handler"` (옵트인)** — `HandlerMPCThread`가 기동되어 `rtc_mpc::MPCFactory`로 `LightContactMPC` 또는 `ContactRichMPC`를 생성하고, `GraspPhaseManager`(아래 참조)의 `PhaseContext`에 따라 실제 Aligator ProxDDP solve를 수행한다. 핸들러 factory / 모델 초기화가 실패하면 경고 로그와 함께 자동으로 mock 경로로 폴백해 RT 루프는 계속 진행된다. Cross-mode swap(light↔rich)은 `HandlerMPCThread` 내부에서 처리된다.
...
`DemoWbcController::GetMpcSolveStats()`는 `mpc.enabled=true`인 동안 `MPCSolutionManager::GetSolveStats()` 스냅샷을 `rtc::MpcSolveStats`로 변환해 항상 반환한다(= **count=0 sentinel**). 즉 `std::nullopt`는 "MPC 자체 비활성" 하나의 의미만 갖고, `count=0`인 유효 행은 "MPC enabled인데 solver가 publish 못함"(dim-mismatch / solver error / 워밍업)을 구분한다. `DemoWbcController` 자체 LifecycleNode의 aux 1 Hz 타이머(`on_activate` 시 `mpc_enabled_`인 경우만 spawn)가 이를 폴링해 `<session>/controllers/demo_wbc_controller/mpc_solve_timing.csv`로 기록 — Stage B 레이아웃 변경 전후 p50/p99 분포 비교의 baseline. 위 phase-independent state write 덕분에 grasp 명령이 오지 않은 kIdle 상태에서도 20 Hz로 row가 쌓이며, 실패 상태도 `count=0` 행으로 눈에 보인다.
...
grasp 측 phase별 OCP 설정(`ocp_type`, `PhaseCostConfig`, `ContactPlan`)은 `config/controllers/mpc/phase_config.yaml`에서 로드된다. Factory 설정(`RobotModelHandler.Init` 입력 + solver 튜닝 + friction_mu)은 `config/controllers/mpc/light_contact.yaml` / `config/controllers/mpc/contact_rich.yaml`에서 로드되며, 두 YAML의 `mpc.model:` 블록은 구조적으로 동일해야 한다(cross-mode swap이 같은 `RobotModelHandler`를 공유하므로 contact_frames 불일치는 index-based 활성화를 깬다).
...
# 3. grasp_cmd=1 → 전체 FSM 시작 (9-entry 메시지; 뒤 2개로 MPC 제어)
...
# 5. Release: grasp_cmd=2 (7-entry 메시지도 그대로 호환)
ros2 topic pub /ur5e/controller_gains std_msgs/Float64MultiArray \
...
- 메시지 본문에 클래스 이름을 박아넣지 않는다. 서브-로거 이름이 곧 식별자다 (`ur5e_bringup.demo_joint_controller`). 네이밍 규약은 [agent_docs/conventions.md](../agent_docs/conventions.md) "Logging" 섹션 참조.
...
| 서브-로거 | 사용처 |
|-----------|--------|
| `ur5e_bringup.demo_joint_controller` | `DemoJointController` (joint-space 데모 컨트롤러, 500 Hz 핫패스) |
| `ur5e_bringup.demo_task_controller` | `DemoTaskController` (task-space 데모 컨트롤러, 500 Hz 핫패스) |
| `ur5e_bringup.demo_wbc_controller` | `DemoWbcController` (WBC + MPC 데모 컨트롤러, 500 Hz 핫패스) |
| `ur5e_bringup.demo_shared_config` | `demo_shared_config` YAML 로더 (init-time, non-RT) |
...
ros2 service call /ur5e_rt_controller/set_logger_levels rcl_interfaces/srv/SetLoggerLevels \
  "{levels: [{name: 'ur5e_bringup.demo_joint_controller', level: 10}]}"

# 양쪽 데모 컨트롤러 동시에 끄기
ros2 service call /ur5e_rt_controller/set_logger_levels rcl_interfaces/srv/SetLoggerLevels \
  "{levels: [{name: 'ur5e_bringup.demo_joint_controller', level: 50}, {name: 'ur5e_bringup.demo_task_controller', level: 50}]}"
```
...
7. RT 컨트롤러 노드 launch (실행 파일 = ROS 노드 이름 = `ur5e_rt_controller` — 정렬됨)
8. UR 드라이버 CPU 핀닝 -> Core 0-1 (3초 지연)
9. RT 컨트롤러 DDS 스레드 핀닝 -> Core 0-1 (5초 지연, 비-SCHED_FIFO 스레드만)

**Lifecycle:** hand_udp_node, ur5e_rt_controller 모두 LifecycleNode 기반. 런치 시 자동 configure → activate.
...
4. RT 컨트롤러 노드 launch (실행 파일 = ROS 노드 이름 = `ur5e_rt_controller` — 정렬됨, params: `ur5e_sim.yaml` + `mujoco_simulator.yaml`)
5. MuJoCo 시뮬레이터 코어 핀닝 (2초 지연, 8코어 이상일 때만)

**Lifecycle 순서:** 런치 시 mujoco_simulator → configure → activate 완료 후 ur5e_rt_controller → configure → activate 순차 활성화.

`mujoco_simulator.yaml`은 `ur5e_bringup/config/`에 위치하며 (UR5e 전용 robot_response 그룹/조인트/토픽), agnostic 기본값은 `rtc_mujoco_sim/config/mujoco_default.yaml`에서 로드된 후 위에 오버레이됩니다.
...
| **Phase config (7a)** | `config/controllers/mpc/phase_config.yaml` 추가 — 8개 phase × `PhaseCostConfig` + `ocp_type`(`light_contact`/`contact_rich`) + `active_contact_indices`. YAML anchor로 close/open posture 및 placement weight 중복 제거. `transition` 블록에 `approach_tolerance`/`pregrasp_tolerance`/`force_threshold`/`max_failures` 수록. |
| **MPC factory YAMLs (7b)** | `config/controllers/mpc/light_contact.yaml`/`mpc/contact_rich.yaml` 추가 — `rtc::mpc::MPCFactory::Create`가 소비하는 `mpc.ocp_type`/`mpc.model`/`mpc.solver`/`mpc.limits` 블록. 두 파일의 `mpc.model` 블록(end_effector_frame + contact_frames) 구조 일치 규약으로 cross-mode swap 무결성 보장. friction_mu는 contact_rich 전용. |
