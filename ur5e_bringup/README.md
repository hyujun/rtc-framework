...
│   │   └── demo_wbc_controller.hpp     <- TSID whole-body + MPC 통합 (Phase 5/7)
│   └── phase/
│       ├── grasp_target.hpp            <- Phase 7a — grasp 목표 pose 구조체 + 외부 명령 enum
│       └── grasp_phase_manager.hpp     <- Phase 7a — 8-state grasp FSM (rtc_mpc::PhaseManagerBase 구현)
├── src/
...
│   └── phase/
│       └── grasp_phase_manager.cpp     <- Phase 7a FSM 구현 (YAML 로더 + 전이 로직)
├── config/
...
│       ├── phase_config.yaml           <- Phase 7a — GraspPhaseManager 8-phase 설정
│       ├── mpc_kinodynamics.yaml       <- Phase 7b — rtc_mpc LightContactOCP factory config
│       └── mpc_fulldynamics.yaml       <- Phase 7b — rtc_mpc ContactRichOCP factory config
├── launch/
...
#### Gains Layout (`~/controller_gains` topic, 9 values — Phase 5)
...
- 7-entry Phase 4 메시지와 **backwards-compatible**: 앞 7개 인덱스 동작이 동일하게 보존됨. 짧은 메시지는 기존 MPC 상태를 건드리지 않음.
- `mpc_enable` (index 7): 런타임 토글. `UpdateGainsFromMsg`는 빌드 타임 `mpc_enabled_`(YAML에서 결정)와 AND 결합하므로 YAML `mpc.enabled: false`이면 런타임 1은 무시됨.
- `riccati_gain_scale` (index 8): 0..1로 자동 clamp. `RiccatiFeedback`이 `scale · K · Δx`에 적용.
...
- `tsid.wqp.solver`: `max_iter` / `eps_abs` (ProxSuite 설정, rtc_tsid가 직접 읽음)
...
- **`mpc` (Phase 5)**: `enabled`, `max_stale_solutions`(기본 5), `riccati.{enabled, gain_scale, accel_only, max_delta_x_norm}`. `enabled: false`가 기본이라 Phase 4 동작이 bit-identical로 보존됨.

#### MPC 통합 동작 (Phase 5 + 7b)

`mpc.enabled: true`일 때 `InitializeHoldPosition`에서 MPC 스레드가 기동되며, `mpc.engine` 값에 따라 두 경로 중 하나를 선택한다:

- **`engine: "mock"` (Phase 5, 기본값)** — `MockMPCThread`가 기동되어 `rtc::SelectThreadConfigs().mpc`이 지정한 코어/우선순위로 20 Hz solve 루프를 돌린다. 현재 state에서 target까지의 선형 trajectory + identity Riccati gain을 공급하는 placeholder. Phase 4 self-hold 동작과 bit-identical.
- **`engine: "handler"` (Phase 7b, 옵트인)** — `HandlerMPCThread`가 기동되어 `rtc_mpc::MPCFactory`로 `LightContactMPC` 또는 `ContactRichMPC`를 생성하고, `GraspPhaseManager`(Phase 7a, 아래 참조)의 `PhaseContext`에 따라 실제 Aligator ProxDDP solve를 수행한다. 핸들러 factory / 모델 초기화가 실패하면 경고 로그와 함께 자동으로 mock 경로로 폴백해 RT 루프는 계속 진행된다. Cross-mode swap(light↔rich)은 `HandlerMPCThread` 내부에서 처리된다.

매 RT tick에서 `ComputeControl`이 `UpdatePhase` 직후 phase-independent하게 `ExtractFullState` + `mpc_manager_.WriteState(q, v, now_ns)`를 실행해 MPC 스레드가 항상 최신 `(q, v)`를 보도록 한다 — `HandlerMPCThread::Solve`의 dim-mismatch gate(`state.nq != model_->nq()`)를 통과시키기 위함이며, 그렇지 않으면 TSID를 돌지 않는 `kIdle/kApproach/kRetreat/kRelease` 구간에서 solver가 계속 false를 반환해 `mpc_solve_timing.csv`가 header만 남게 된다. TSID 활성 phase(`kPreGrasp/kClosure/kHold`)에서는 `ComputeTSIDPosition`이 최신 solution을 cubic-Hermite 보간한 `q_ref, v_ref, a_ff`를 TSID task reference로 주입하고, Riccati 피드백(`K·Δx`)은 `control_ref_.a_des`의 상위 `nv`개 원소에 가산된다. MPC가 solution을 공급하지 못하거나 `stale_count >= max_stale_solutions`면 자동으로 Phase 4 self-hold reference로 fallback한다.

`DemoWbcController::GetMpcSolveStats()`는 `mpc.enabled=true`인 동안 `MPCSolutionManager::GetSolveStats()` 스냅샷을 `rtc::MpcSolveStats`로 변환해 항상 반환한다(= **count=0 sentinel**). 즉 `std::nullopt`는 "MPC 자체 비활성" 하나의 의미만 갖고, `count=0`인 유효 행은 "MPC enabled인데 solver가 publish 못함"(dim-mismatch / solver error / 워밍업)을 구분한다. `RtControllerNode`의 aux 1 Hz 타이머가 이를 폴링해 `<session>/controller/mpc_solve_timing.csv`로 기록 — Stage B 레이아웃 변경 전후 p50/p99 분포 비교의 baseline. 위 phase-independent state write 덕분에 grasp 명령이 오지 않은 kIdle 상태에서도 20 Hz로 row가 쌓이며, 실패 상태도 `count=0` 행으로 눈에 보인다.
...
grasp 측 phase별 OCP 설정(`ocp_type`, `PhaseCostConfig`, `ContactPlan`)은 `config/controllers/phase_config.yaml`에서 로드된다. Factory 설정(`RobotModelHandler.Init` 입력 + solver 튜닝 + friction_mu)은 `mpc_kinodynamics.yaml` / `mpc_fulldynamics.yaml`에서 로드되며, 두 YAML의 `mpc.model:` 블록은 구조적으로 동일해야 한다(cross-mode swap이 같은 `RobotModelHandler`를 공유하므로 contact_frames 불일치는 index-based 활성화를 깬다).

#### 사용법 (시뮬레이션)

```bash
# Phase 4 동일 동작 (MPC 비활성)
ros2 launch ur5e_bringup sim.launch.py initial_controller:=demo_wbc_controller

# Phase 5: MPC 활성 (sim.launch.py는 launch arg 오버라이드 지원)
...
ros2 topic pub /ur5e/controller_gains std_msgs/Float64MultiArray \
  "{data: [1.0, 2.0, 0.5, 1.0, 100.0, 10.0, 1.0, 1.0, 1.0]}" -1

# 4. Riccati gain을 반으로 축소 (fade-in)
ros2 topic pub /ur5e/controller_gains std_msgs/Float64MultiArray \
  "{data: [1.0, 2.0, 0.5, 1.0, 100.0, 10.0, 1.0, 1.0, 0.5]}" -1

# 5. Release: grasp_cmd=2 (Phase 4 7-entry 메시지도 그대로 호환)
ros2 topic pub /ur5e/controller_gains std_msgs/Float64MultiArray \
  "{data: [2.0, 2.0, 0.5, 1.0, 100.0, 10.0, 1.0]}" -1
```

> `robot.launch.py`의 `enable_mpc` launch arg는 선언만 되어 있고 OpaqueFunction을 쓰지 않으므로 런타임 gains 토픽(index 7)으로 토글해야 한다. `sim.launch.py`는 `ctrl_overrides`에 `demo_wbc_controller.mpc.enabled`를 주입해 launch 시점에 반영한다.
...
| `enable_mpc` (Phase 5) | `""` | DemoWbcController MPC 토글. **선언만 되어 있고 robot.launch.py는 OpaqueFunction 미사용** — 실제 제어는 런타임 gains topic index 7로 수행. |
...
2. 환경 변수 설정 (`CYCLONEDDS_URI`, `RMW_IMPLEMENTATION`, `RTC_SESSION_DIR` + 하위 호환용 `UR5E_SESSION_DIR`)
...
**Lifecycle:** hand_udp_node, rt_controller 모두 LifecycleNode 기반. 런치 시 자동 configure → activate.
...
| `enable_mpc` (Phase 5) | `""` (YAML 사용) | DemoWbcController의 `mpc.enabled` YAML 키를 launch 시점에 오버라이드. `true`/`false` 명시. `initial_controller:=demo_wbc_controller`와 함께 사용. 런타임 토글은 gains topic index 7로도 가능. |
...
**Lifecycle 순서:** 런치 시 mujoco_simulator → configure → activate 완료 후 rt_controller → configure → activate 순차 활성화.
...
| `rtc_mpc` | MPC↔RT 인터페이스 (Phase 5: TripleBuffer + Hermite + Riccati + MPCThread) |
...
| 영역 | 변경 내용 |
|------|----------|
| **GraspPhaseManager (7a)** | 신규 모듈 `ur5e_bringup/phase/` 추가. `rtc::mpc::PhaseManagerBase`를 상속한 8-state grasp FSM(`idle → approach → pre_grasp → closure → hold → manipulate → retreat → release`) + `GraspTarget`/`GraspCommand` 보조 타입. 전이 가드: TCP-proximity(approach/pregrasp/retreat), 센서 합 기반 force threshold(closure→hold), 명시적 외부 명령(idle/hold/manipulate/retreat/release) + `max_failures` closure 가드. 모든 phase name이 libstdc++ SSO(≤15자) 범위 내임을 `static_assert`로 고정. 로봇-agnostic: Panda 기반 fixture로 13-case gtest 통과. |
| **Phase config (7a)** | `config/controllers/phase_config.yaml` 추가 — 8개 phase × `PhaseCostConfig` + `ocp_type`(`light_contact`/`contact_rich`) + `active_contact_indices`. YAML anchor로 close/open posture 및 placement weight 중복 제거. `transition` 블록에 `approach_tolerance`/`pregrasp_tolerance`/`force_threshold`/`max_failures` 수록. |
| **MPC factory YAMLs (7b)** | `mpc_kinodynamics.yaml`/`mpc_fulldynamics.yaml` 추가 — `rtc::mpc::MPCFactory::Create`가 소비하는 `mpc.ocp_type`/`mpc.model`/`mpc.solver`/`mpc.limits` 블록. 두 파일의 `mpc.model` 블록(end_effector_frame + contact_frames) 구조 일치 규약으로 cross-mode swap 무결성 보장. friction_mu는 contact_rich 전용. |
...
| **TSID WQP 솔버 패스스루** | `demo_wbc_controller.yaml`의 `tsid.wqp.solver.{max_iter, eps_abs}`를 공식 키로 노출. 기존에도 `rtc_tsid::WQPFormulation::init`이 읽던 경로이지만 YAML에 미노출 상태였음. 현재 값 = 기존 기본값(20, 1e-6)로 동작 변화 없음. `eps_rel`/`verbose`는 업스트림 TODO. |

### v5.19.0 (Phase 1b)

| 영역 | 변경 내용 |
|------|----------|
| **스레드 안전 (SeqLock)** | 3개 데모 컨트롤러(`DemoJointController`, `DemoTaskController`, `DemoWbcController`) `Gains gains_` → `rtc::SeqLock<Gains> gains_lock_` 전환. RT 경로 스냅샷 포인트: DemoJoint/DemoTask는 `ComputeControl()` 진입 시, DemoWbc는 `OnPhaseEnter()` 진입 시 단일 `Load()` 수행. `UpdateVirtualTcp()` 시그니처가 `const Gains&`를 받도록 확장되어 DemoJoint/DemoTask가 동일 스냅샷을 헬퍼에 전파. Aux 스레드 writer(`LoadConfig`, `UpdateGainsFromMsg`)는 Load/mutate/Store 패턴. `SetDeviceTarget()`에서 `control_6dof` 읽기는 `gains_lock_.Load().control_6dof` 사용 (ROS executor가 aux 스레드 직렬화 보장). DemoWbc의 `grasp_cmd_`는 기존 `std::atomic<int>` 유지 (gains 스냅샷 외). |

### v5.18.0

| 영역 | 변경 내용 |
|------|----------|
| **입력 검증** | DemoJoint/DemoTask/DemoWbc 3개 컨트롤러의 `trajectory_speed` 관련 필드에 `std::max(1e-6, val)` 클램프 적용 (LoadConfig + UpdateGainsFromMsg, 총 13곳) -- 0/음수 값 입력 시 무한 궤적 duration 방지 |
