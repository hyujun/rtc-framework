...
매 RT tick에서 `ComputeControl`이 `UpdatePhase` 직후 phase-independent하게 `ExtractFullState` + `mpc_manager_.WriteState(q, v, now_ns)`를 실행해 MPC 스레드가 항상 최신 `(q, v)`를 보도록 한다 — `HandlerMPCThread::Solve`의 dim-mismatch gate(`state.nq != model_->nq()`)를 통과시키기 위함이며, 그렇지 않으면 TSID를 돌지 않는 `kIdle/kApproach/kRetreat/kRelease` 구간에서 solver가 계속 false를 반환해 `mpc_solve_timing.csv`에 새 row가 추가되지 않는다. TSID 활성 phase(`kPreGrasp/kClosure/kHold`)에서는 `ComputeTSIDPosition`이 최신 solution을 cubic-Hermite 보간한 `q_ref, v_ref, a_ff`를 TSID task reference로 주입하고, Riccati 피드백(`K·Δx`)은 `control_ref_.a_des`의 상위 `nv`개 원소에 가산된다. MPC가 solution을 공급하지 못하거나 `stale_count >= max_stale_solutions`면 자동으로 TSID self-hold reference로 fallback한다.

MPC solve-timing CSV는 **per-MPC-tick raw 샘플** 형식이다: `<session>/controllers/demo_wbc_controller/mpc_solve_timing.csv`에 3 cols `t_wall_ns,count,solve_ns`로 한 row가 곧 한 solve이며, MPC 주파수(`target_frequency_hz`, 기본 20 Hz)로 행이 쌓인다. 데이터 흐름은 `MPCSolutionManager::PublishSolution`이 매 solve마다 SPSC ring에 sample push → `DemoWbcController::DrainMpcSolveSamples` (override)가 자체 LifecycleNode의 aux 1 Hz 타이머에서 SPSC를 비우며 `MpcSolveTimingLogger`로 행 단위 append. p50/p99/max는 post-process로 계산. 같은 1 Hz 콜백이 `GetMpcSolveStats()`(256-sample 슬라이딩 윈도우)로 10초마다 `RCLCPP_INFO` 요약 라인을 출력 — Stage B 레이아웃 변경 전후 분포 비교의 baseline. 위 phase-independent state write 덕분에 grasp 명령이 오지 않은 kIdle 상태에서도 MPC 주파수로 row가 쌓이며, solver 실패 시에는 새 row가 멈춰 그 자체가 health 신호가 된다.
