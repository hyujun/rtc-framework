...
`DemoWbcController::GetMpcSolveStats()`는 `mpc.enabled=true`일 때 `MPCSolutionManager::GetSolveStats()` 스냅샷을 `rtc::MpcSolveStats`로 변환해 반환한다(아직 한 번도 solve가 돌지 않았으면 `std::nullopt`). `RtControllerNode`의 aux 1 Hz 타이머가 이를 폴링해 `<session>/controller/mpc_solve_timing.csv`로 기록 — Stage B 레이아웃 변경 전후 p50/p99 분포 비교의 baseline. 위 phase-independent state write 덕분에 grasp 명령이 오지 않은 kIdle 상태에서도 20 Hz로 row가 쌓인다.
