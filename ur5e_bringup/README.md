...
| 영역 | 변경 내용 |
|------|----------|
| **Hand 상수 의존 이전** | `ur5e_description/ur5e_constants.hpp` 삭제에 따라 demo 컨트롤러 헤더가 `ur5e_hand_driver/hand_constants.hpp`를 include. `package.xml`에서 `<exec_depend>ur5e_hand_driver</exec_depend>` → `<depend>` 승격, `CMakeLists.txt`에 `find_package(ur5e_hand_driver)` + `ament_target_dependencies` 추가. |
