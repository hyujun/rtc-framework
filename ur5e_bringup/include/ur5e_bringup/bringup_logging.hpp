...
// every log call without measurable overhead. Demo controllers may still cache
// the result in a member to avoid repeating the lookup on every cycle.

inline rclcpp::Logger DemoJointLogger() {
  return rclcpp::get_logger("bringup.demo_joint");
}
