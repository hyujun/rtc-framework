...
struct RegisteredLogHandles {
  std::map<std::string, rtc::LogHandle<ur5e::DeviceStateLogPod>> state;
  std::map<std::string, rtc::LogHandle<ur5e::DeviceSensorLogPod>> sensor;
};
...
[[nodiscard]] LogRegistrationResult RegisterControllerLogs(
  ...
  for (const auto& entry : entries) {
    ...
    if (entry.msg_type == "rtc_msgs/DeviceStateLog") {
      ...
      auto handle = ctx.log_set.RegisterLog<ur5e::DeviceStateLogPod>(
          entry.instance,
          [joint_names, motor_names](std::ostream& os) {
            ur5e::WriteDeviceStateLogHeader(os, joint_names, motor_names);
          },
          [](std::ostream& os, const ur5e::DeviceStateLogPod& pod) {
      ...
      auto handle = ctx.log_set.RegisterLog<ur5e::DeviceSensorLogPod>(
          entry.instance,
          [sensor_names](std::ostream& os) { ur5e::WriteDeviceSensorLogHeader(os, sensor_names); },
          [](std::ostream& os, const ur5e::DeviceSensorLogPod& pod) {
