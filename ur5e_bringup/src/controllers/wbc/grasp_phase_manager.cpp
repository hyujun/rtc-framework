...
GraspPhaseManager::GraspPhaseManager(const rtc::mpc::RobotModelHandler& model) noexcept
    : model_(model) {}
...
GraspPhaseInitError GraspPhaseManager::LoadPhases(
  ...
  for (int i = 0; i < kNumGraspPhases; ++i) {
    ...
    for (int id : active_ids) {
      cp.active_frame_ids.push_back(frames[static_cast<std::size_t>(id)].frame_id);
    }
    ...
    const auto cost_err = rtc::mpc::PhaseCostConfig::LoadFromYaml(cost_node, model_, cost);
...
GraspPhaseInitError GraspPhaseManager::Load(const YAML::Node& cfg) noexcept {
  ...
  if (const auto err = LoadTransition(cfg["transition"], t); err != GraspPhaseInitError::kNoError) {
    return err;
  }

  std::array<PhaseSlot, kNumGraspPhases> slots{};
  if (const auto err = LoadPhases(cfg["phases"], slots); err != GraspPhaseInitError::kNoError) {
    return err;
  }
...
void GraspPhaseManager::Init(const YAML::Node& cfg) {
  ...
  if (err != GraspPhaseInitError::kNoError) {
    throw std::runtime_error("GraspPhaseManager::Init failed, error code " +
...
void GraspPhaseManager::SetTaskTarget(const YAML::Node& target) {
  if (!target.IsDefined() || !target.IsMap()) {
    std::fprintf(stderr, "[grasp_phase] SetTaskTarget: expected YAML map; ignored\n");
    return;
  }
  ...
  try {
    ...
    if (!tr.IsDefined() || !tr.IsSequence() || tr.size() != 3) {
      std::fprintf(stderr,
                   "[grasp_phase] SetTaskTarget: grasp_translation "
                   "must be 3-vector; ignored\n");
      return;
    }
    Eigen::Vector3d p{tr[0].as<double>(), tr[1].as<double>(), tr[2].as<double>()};
    ...
    if (rot.IsDefined()) {
      if (!rot.IsSequence() || rot.size() != 4) {
        std::fprintf(stderr,
                     "[grasp_phase] SetTaskTarget: grasp_rotation "
                     "must be 4-vector (wxyz); ignored\n");
        return;
      }
      q = Eigen::Quaterniond(rot[0].as<double>(), rot[1].as<double>(), rot[2].as<double>(),
                             rot[3].as<double>());
      q.normalize();
    }
    ...
    if (off.IsDefined()) {
      if (!off.IsSequence() || off.size() != 3) {
        std::fprintf(stderr,
                     "[grasp_phase] SetTaskTarget: "
                     "pregrasp_offset_local must be 3-vector; "
                     "ignored\n");
        return;
      }
      offset_local = Eigen::Vector3d{off[0].as<double>(), off[1].as<double>(), off[2].as<double>()};
    }
    ...
    std::fprintf(stderr, "[grasp_phase] SetTaskTarget: YAML parse failed; ignored\n");
    return;
  }
...
int GraspPhaseManager::EvaluateTransition(int current_id, const pinocchio::SE3& tcp,
...
rtc::mpc::PhaseContext GraspPhaseManager::BuildContext(int id, bool changed) const {
...
rtc::mpc::PhaseContext GraspPhaseManager::Update(const Eigen::VectorXd& /*q*/,
                                                 ...
                                                 const Eigen::VectorXd& sensor,
                                                 const pinocchio::SE3& tcp, double /*t*/) {
  if (!initialised_) {
  ...
  const int forced = forced_phase_id_.exchange(kNoForcedPhase, std::memory_order_acq_rel);
  int next_id = prev_id;
  if (forced != kNoForcedPhase && forced >= 0 && forced < kNumGraspPhases) {
    ...
    if (next_id != prev_id && cmd != GraspCommand::kNone) {
      command_.store(static_cast<int>(GraspCommand::kNone), std::memory_order_release);
    }
  }
