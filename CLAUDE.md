...
**4·5·6 자동화**: [.claude/hooks/verify-changes.sh](.claude/hooks/verify-changes.sh) Stop hook 이 turn 종료 시 자동 실행하고 hard failure 시 `exit 2` 로 다음 turn 까지 차단한다 (loop 방지는 `stop_hook_active` 가드; stop cycle 당 1회 발화). 변경 패키지만 빌드·테스트하며 **build/test 의 timeout·launch 실패는 "미검증"으로 차단** (silent pass 아님 — bound 초과 test 는 hook 의 timeout 상향). 변경 집합은 `git diff <watermark>` **∪ untracked** 이며 — watermark 는 hook 이 **마지막으로 통과한 커밋**(`.git/rtc-verify-base`, 커밋 안 됨)이지 `HEAD` 가 아니다: HEAD 기준이면 **turn 안에서 commit 한 순간 변경 집합이 비어 아무것도 검증되지 않았고**, 이 저장소에서는 그게 정상적인 turn 모양이라 게이트가 대부분의 turn 에 조용히 부재했다 (2026-09-09 실측: in-turn commit 한 D10 위반이 hook 을 통과해 CI 에서만 잡혔다). 차단(exit 2)은 watermark 를 전진시키지 않고, watermark 가 없거나 HEAD 의 조상이 아니면 (rebase·브랜치 전환) `HEAD` 로 폴백해 이전 동작과 동일해진다. 그리고 build/test 는 **위치 축**으로 좁힌다 — untracked 라도 **설치 산출물이 사는 디렉토리** (`<pkg>/src/`·`<pkg>/include/`·`<pkg>/test/`·`<pkg>/launch/`·`<pkg>/scripts/`·`<pkg>/<pkg>/`) 아래 소스는 빌드·테스트 대상이고, 그 밖의 untracked scratch 만 제외된다 — **allowlist 이지 "패키지 아래 아무 디렉토리" 가 아니다** (`config/` 는 YAML 축이 이미 parse 하므로 제외). tracked-ness 축은 신규 헤더를 검증 없이 통과시켜 폐기됐고, 그 교체가 빠뜨린 `test/` 는 #358, `launch/`·`scripts/` 는 #360 에서 편입됐다 — 후자는 DIRECTORY 설치라 **CMake 편집 없이 install 트리에 들어가는데** 같은 파일을 *수정*하면 빌드되고 *추가*하면 안 되는 비대칭이었다. 편입이 그 `.py` 를 파싱·컴파일해주지는 않는다 (얻는 것은 install 트리 정합 + 소유 패키지 test 재실행). **hook 주석이 SSoT.** README co-update 는 public surface (header/launch/config/파일 add·del/dep) 변경 시 **non-blocking checklist** (내부 리팩터·bug fix 는 미요구); CMake/`package.xml` co-update 는 blocking. 변경된 `.md` 는 `validate_docs.py --files` (변경 파일 한정), 변경된 YAML 은 parse 검사를 받는다 — 전체 코퍼스 스캔은 CI 몫이다. Doxygen 은 에이전트가 직접 검증. Pure-format commit (clang-format / ruff round-trip 동치) 은 ARCH grep + doc 단계만 skip, build/test 는 그대로.
