...
void ControllerRegistry::Register(ControllerEntry entry) {
  ...
  const auto it = std::find_if(entries_.begin(), entries_.end(), [&](const ControllerEntry& e) {
    return e.config_key == entry.config_key;
  });
  if (it != entries_.end()) {
    RCLCPP_WARN(rclcpp::get_logger("rtc_controller_interface"),
                ...
                "TUs.",
                entry.config_key.c_str(), entry.config_package.c_str(), it->config_package.c_str());
  }
