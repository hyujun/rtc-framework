...
class StubRegistryController : public rtc::RTControllerInterface {
 public:
  explicit StubRegistryController(const std::string& urdf_path) : urdf_path_(urdf_path) {}
...
TEST(ControllerRegistryTest, RegisterAndGetEntries) {
  ...
  reg.Register(
      {"test_stub_ctrl", "indirect/", "rtc_controller_interface",
       [](const std::string& urdf) { return std::make_unique<StubRegistryController>(urdf); }});
...
TEST(ControllerRegistryTest, FactoryCreatesController) {
  ...
  reg.Register({"test_factory_ctrl", "direct/", "test_pkg", [](const std::string& urdf) {
...
TEST(ControllerRegistryTest, MultipleRegistrationsAccumulate) {
  ...
  for (int i = 0; i < 3; ++i) {
    reg.Register(
        {"accum_" + std::to_string(i), "indirect/", "test_pkg",
         [](const std::string& urdf) { return std::make_unique<StubRegistryController>(urdf); }});
  }
...
TEST(ControllerRegistryTest, EntryFieldsPreserved) {
  ...
  reg.Register({"fields_test_ctrl", "direct/", "my_bringup_pkg", [](const std::string& urdf) {
                  return std::make_unique<StubRegistryController>(urdf);
                }});
...
TEST(ControllerRegistryTest, FactoryReceivesUrdfPath) {
  ...
  reg.Register({"urdf_path_test", "indirect/", "test_pkg", [](const std::string& urdf) {
                  return std::make_unique<StubRegistryController>(urdf);
                }});
