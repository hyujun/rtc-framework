...
TEST_F(ChainExtractorSerialTest, InvalidLinkThrows) {
  EXPECT_THROW(extractor_->ExtractSubModel("bad", "base_link", "nonexistent"), std::out_of_range);
}
...
TEST_F(ChainExtractorTreeTest, InvalidTipThrows) {
  ...
  EXPECT_THROW(extractor_->ExtractTreeModel("bad", "palm_link", tips), std::out_of_range);
}
