...
TEST_F(UrdfAnalyzerMimicTest, MimicBecomesPassive) {
  ...
  EXPECT_EQ(analyzer_->GetJointRole("finger_right_joint"), rub::JointRole::kPassive);
  EXPECT_EQ(analyzer_->GetPassiveSubtype("finger_right_joint"), rub::PassiveSubtype::kMimic);
  // finger_left_joint은 그대로 active
  EXPECT_EQ(analyzer_->GetJointRole("finger_left_joint"), rub::JointRole::kActive);
}
...
TEST(UrdfAnalyzerXmlTest, EmptyXmlThrows) {
  EXPECT_THROW(rub::UrdfAnalyzer("", rub::UrdfAnalyzer::FromXmlTag{}), std::runtime_error);
}

TEST(UrdfAnalyzerXmlTest, MalformedXmlThrows) {
  EXPECT_THROW(rub::UrdfAnalyzer("<robot><broken", rub::UrdfAnalyzer::FromXmlTag{}),
               std::runtime_error);
}
