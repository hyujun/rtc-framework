...
namespace {
auto logger() {
  return ::rtc::urdf::logging::XacroLogger();
}
}  // namespace
...
#pragma GCC diagnostic ignored "-Wignored-attributes"
  std::unique_ptr<FILE, decltype(&pclose)> pipe(popen(cmd.c_str(), "r"),
                                                pclose);  // NOLINT(cert-env33-c)
#pragma GCC diagnostic pop

  if (!pipe) {
    RCLCPP_ERROR(logger(), "popen 실행 실패 — 명령: %s", cmd.c_str());
    throw std::runtime_error("ProcessXacro: popen 실행 실패 — 명령: " + cmd);
  }
  ...
  if (exit_code != 0) {
    ...
    RCLCPP_ERROR(logger(), "xacro 실행 실패 (exit code %d)\n%s", exit_code, output.c_str());
    throw std::runtime_error("ProcessXacro: xacro 실행 실패 (exit code " +
                             std::to_string(exit_code) + ")\n" + output);
  }

  if (output.empty()) {
    RCLCPP_ERROR(logger(), "xacro 출력이 비어 있습니다: %s", file_path.c_str());
    throw std::runtime_error("ProcessXacro: xacro 출력이 비어 있습니다: " + file_path);
  }

  RCLCPP_DEBUG(logger(), "xacro 전처리 완료: %s (%zu bytes)", file_path.c_str(), output.size());
  return output;
}
