...
ClosedChainModel BuildClosedChainModelFromExtendedUrdf(std::string_view urdf_path,
  ...
  // (3) RigidConstraintModel 생성 (§4b joint-frame placement)
  out.constraints = BuildRigidConstraints(out.model, spec.closures);

  // (4) actuated joint 이름 → JointIndex (actuation metadata, 구속과 분리)
  out.actuated_joint_ids.reserve(spec.actuated_joints.size());
  for (const auto& jname : spec.actuated_joints) {
    if (!out.model.existJointName(jname)) {
    ...
    out.actuated_joint_ids.push_back(out.model.getJointId(jname));
  }

  // (5) q_ref: neutral → loop-consistent 사영
  pinocchio::Data data(out.model);
  const Eigen::VectorXd q0 = pinocchio::neutral(out.model);
  const ProjectionResult proj = ProjectToConstraint(out.model, data, out.constraints, q0);
  out.q_ref = proj.q;
  ...
  // (6) q_ref 특이성 검사: converged 여도 대칭 조립형상(예: neutral 근방 평면 4-bar)은
  ...
  if (!out.constraints.empty()) {
    const JacobianReport rep =
        AnalyzeConstraintJacobian(out.model, data, out.constraints, out.q_ref);
