...
inline void ClampSymmetric(std::array<double, kMaxDeviceChannels>& commands, int n,
                           std::span<const double> limits, double default_limit) noexcept {
...
inline void ClampRange(std::array<double, kMaxDeviceChannels>& commands, int n,
                       ...
                       double default_lower, double default_upper) noexcept {
  for (std::size_t i = 0; i < static_cast<std::size_t>(n); ++i) {
