...
// Each config corresponds to a publish role (kDeviceStateLog or
// kDeviceSensorLog) in the active controller's YAML topic configuration.
...
// Writes control data to role-based CSV files in a non-RT thread.
//
...
//
// Per-tick *timing* CSV is no longer this class's responsibility — the
// generic infra in rtc_base/timing/{thread_timing_*, cm_timing_sample}.hpp
// owns that path now (CmTimingProducer + ThreadTimingCsvLogger<CmTimingPayload>
// → timing_log.csv). DataLogger handles only the device-state / device-sensor
// CSVs, which are payload-rich and unique to this class.
//
...
public:
    ...
    num_inference_values_(num_inference_values)
  {
    ...
    for (std::size_t i = 0; i < device_configs_.size(); ++i) {
      ...
      if (cfg.num_channels > kMaxDeviceChannels) {
        cfg.num_channels = kMaxDeviceChannels;
      }
      if (cfg.num_sensor_channels > kMaxSensorChannels) {
        cfg.num_sensor_channels = kMaxSensorChannels;
      }
...
private:
  ...
  {
    if (dev_idx < device_configs_.size() &&
      i < device_configs_[dev_idx].joint_names.size())
    {
  ...
  {
    if (dev_idx < device_configs_.size() &&
      i < device_configs_[dev_idx].motor_names.size())
    {
  ...
  {
    if (dev_idx < device_configs_.size() &&
      i < device_configs_[dev_idx].sensor_names.size())
    {
  ...
  void WriteStateLogRow(std::size_t idx, const rtc::LogEntry & e)
  {
    ...
    for (int i = 0; i < nc; ++i) {
      f << ','
        << (i < na ? d.actual_positions[static_cast<std::size_t>(i)] : 0.0);
    }
    for (int i = 0; i < nc; ++i) {
      f << ','
        << (i < na ? d.actual_velocities[static_cast<std::size_t>(i)] : 0.0);
    }
    for (int i = 0; i < nc; ++i) {
      f << ',' << (i < na ? d.efforts[static_cast<std::size_t>(i)] : 0.0);
    }

    // Command
    for (int i = 0; i < nc; ++i) {
      f << ',' << (i < na ? d.commands[static_cast<std::size_t>(i)] : 0.0);
    }
    ...
    for (int i = 0; i < nc; ++i) {
      f << ','
        << (i < na ? d.goal_positions[static_cast<std::size_t>(i)] : 0.0);
    }
    for (const auto v : e.task_goal_positions) {
      f << ',' << v;
    }
    ...
    for (int i = 0; i < nc; ++i) {
      f << ','
        << (i < na ? d.trajectory_positions[static_cast<std::size_t>(i)] : 0.0);
    }
    for (int i = 0; i < nc; ++i) {
      f << ','
        << (i < na ? d.trajectory_velocities[static_cast<std::size_t>(i)] :
      0.0);
    }

    // Task-space trajectory reference
    for (const auto v : e.trajectory_task_positions) {
      f << ',' << v;
    }
    for (const auto v : e.trajectory_task_velocities) {
      f << ',' << v;
    }

    // Task-space FK
    for (const auto v : e.actual_task_positions) {
      f << ',' << v;
    }
    ...
    for (int i = 0; i < nmc; ++i) {
      f << ','
        << (i < nma ? d.motor_positions[static_cast<std::size_t>(i)] : 0.0);
    }
    for (int i = 0; i < nmc; ++i) {
      f << ','
        << (i < nma ? d.motor_velocities[static_cast<std::size_t>(i)] : 0.0);
    }
    for (int i = 0; i < nmc; ++i) {
      f << ','
        << (i < nma ? d.motor_efforts[static_cast<std::size_t>(i)] : 0.0);
    }
  ...
  void WriteSensorLogHeader(std::size_t idx)
  {
    ...
    if (has_sensors && num_inference_values_ > 0) {
      const char *const kFtComp[] = {"contact", "fx", "fy", "fz",
        "ux", "uy", "uz"};
      constexpr int kFtCompCount = 7;
      int ft_idx = 0;
      for (const auto & sn : cfg.sensor_names) {
        for (int c = 0; c < kFtCompCount && ft_idx < num_inference_values_;
          ++c, ++ft_idx)
        {
          f << ",ft_" << sn << "_" << kFtComp[c];
        }
      }
      for (; ft_idx < num_inference_values_; ++ft_idx) {
        f << ",inference_" << ft_idx;
      }
    } else {
  ...
  void WriteSensorLogRow(std::size_t idx, const rtc::LogEntry & e)
  {
    ...
    for (int i = 0; i < ns; ++i) {
      f << ','
        << (i < ns_actual ? d.sensor_data_raw[static_cast<std::size_t>(i)] :
      0.0f);
    }
    // Filtered sensor data
    for (int i = 0; i < ns; ++i) {
      f << ','
        << (i < ns_actual ? d.sensor_data[static_cast<std::size_t>(i)] : 0.0f);
    }
    ...
    for (int i = 0; i < ni; ++i) {
      f << ','
        << (i < ni_actual ? e.inference_output[static_cast<std::size_t>(i)] :
      0.0f);
    }
