...
- 캐시 라인 정렬 (`kSeqLockCacheLineSize = 64`)
...
| 로봇 | `goal_positions[]`, `actual_positions[]`, `actual_velocities[]`, `actual_torques[]`, `actual_task_positions[]`, `robot_commands[]`, `command_type` |
| 궤적 | `trajectory_positions[]`, `trajectory_velocities[]` |
| 디바이스 | `device_goal[]`, `device_actual[]`, `sensor_data[]`, `sensor_data_raw[]`, `inference_output[]`, `device_commands[]` |

#### PublishSnapshot 주요 필드

| 카테고리 | 필드 |
|---------|------|
| 커맨드 | `robot_commands[]`, `command_type`, `device_commands[]` |
| 상태 | `actual_positions[]`, `actual_velocities[]`, `actual_task_positions[]` |
| 타겟 | `goal_positions[]`, `actual_target_positions[]`, `target_velocities[]` |
| 메타 | `stamp_ns`, `active_controller_idx`, `ur5e_enabled`, `device_enabled`  |
...
| `robot_log.csv` | `timestamp, goal_pos_*, actual_pos_*, actual_vel_*, actual_torque_*, task_pos_*, command_*, command_type, traj_pos_*, traj_vel_*` |
| `device_log.csv` | `timestamp, device_valid, device_goal_*, device_actual_*, device_vel_*, sensor_raw_*, sensor_*, inference_valid, inference_*, device_cmd_*` |

**API:**

| 메서드 | 설명 |
|--------|------|
| `DataLogger(timing_path, robot_path, device_path, ...)` | CSV 파일 3개 열기 + 헤더 작성 |
