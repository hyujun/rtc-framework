...
| 카테고리 | 필드 |
|---------|------|
| 디바이스별 (`GroupCommandSlot[4]`) | `num_channels`, `actual_num_channels`, `commands[]`, `goal_positions[]`, `target_positions[]`, `target_velocities[]`, `trajectory_positions[]`, `trajectory_velocities[]`, `actual_positions[]`, `actual_velocities[]`, `efforts[]`, `motor_positions[]`, `motor_velocities[]`, `motor_efforts[]`, `sensor_data[]`, `sensor_data_raw[]`, `inference_output[]`, `goal_type`, `grasp_state`, `wbc_state`, `tof_snapshot`, **TF source poses** (`arm_tip_pose` + valid, `virtual_tcp_pose` + valid, `fingertip_poses[8]` + valid — `kRobotTransforms` publish role 용) |
