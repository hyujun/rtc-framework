...
| 영역 | 변경 내용 |
|------|----------|
| **GoalType enum** | `GoalType` 열거형 추가 (`kJoint`, `kTask`) + `GoalTypeToString()` constexpr 변환 |
| **PublishRole 재설계** | `kTaskPosition`, `kTrajectoryState`, `kControllerState` 제거 → `kGuiPosition`, `kRobotTarget`, `kDeviceStateLog`, `kDeviceSensorLog`, `kDigitalTwinState` 추가 |
| **SubscribeRole 확장** | `kMotorState` 역할 추가 (모터 공간 JointState 구독) |
| **DeviceState 확장** | 모터 공간 필드 (`motor_positions`, `motor_velocities`, `motor_efforts`, `num_motor_channels`) + 추론 필드 (`inference_data`, `inference_enable`, `num_inference_fingertips`) 추가 |
| **DeviceOutput 확장** | `trajectory_positions`, `trajectory_velocities` (궤적 레퍼런스) + `goal_type` 필드 추가 |
| **ControllerOutput 확장** | `task_goal_positions`, `trajectory_task_positions`, `trajectory_task_velocities` 필드 추가 |
| **DeviceNameConfig 확장** | `joint_command_names`, `motor_state_names`, `safe_position` 필드 추가 |
| **TopicConfig 재설계** | `std::map` → `std::vector<std::pair>` (YAML 삽입 순서 보존, 디바이스 인덱스 일관성) |
| **kFTValuesPerFingertip** | 13 → 7 변경 (3-head 모델: contact(1)+F(3)+u(3)) |
| **기본 이름 벡터** | `kDefaultRobotJointNames`, `kDefaultHandMotorNames`, `kDefaultFingertipNames` 추가 |
| **기본 제한값** | `kDefaultMaxJointVelocity=2.0`, `kDefaultMaxJointTorque=150.0` 추가 |
...
| **캐시 라인 상수 통합** | `kCacheLineSize`를 `types.hpp`에서 단일 정의 → SeqLock, SPSC 버퍼에서 중복 제거 |
...
| **`[[likely]]/[[unlikely]]`** | SPSC 버퍼 `Push()` hot path에 분기 힌트 추가 |
...
| 구조체 | 설명 |
|--------|------|
| `DeviceState` | 가변 채널 디바이스 상태 — `positions[64]`, `velocities[64]`, `efforts[64]`, 모터 공간 (`motor_positions[64]`, `motor_velocities[64]`, `motor_efforts[64]`), 센서 (`sensor_data[128]`, `sensor_data_raw[128]`), 추론 (`inference_data[64]`, `inference_enable[8]`) |
...
| `ControllerOutput` | `devices[4]` (DeviceOutput 배열) + `actual_task_positions[6]`, `task_goal_positions[6]`, `trajectory_task_positions[6]`, `trajectory_task_velocities[6]`, `valid`, `command_type` |
| `FingertipFTState` | 핑거팁 힘/토크 추론 결과 — `ft_data[56]` (`kFTValuesPerFingertip=7 × kMaxFingertips=8`), `per_fingertip_valid[8]` |

**설정 타입:**

| 구조체 | 설명 |
|--------|------|
| `DeviceNameConfig` | 디바이스 이름, 관절/센서/모터 이름 매핑, 커맨드 이름, URDF 설정(`DeviceUrdfConfig`), 관절 한계(`DeviceJointLimits`), 안전 위치(`safe_position`) |
...
| `GoalType` | `kJoint`, `kTask` | 목표 공간 타입 |
| `SubscribeRole` | `kState`, `kMotorState`, `kSensorState`, `kTarget` | 구독 역할 |
| `PublishRole` | `kJointCommand`, `kRos2Command`, `kGuiPosition`, `kRobotTarget`, `kDeviceStateLog`, `kDeviceSensorLog`, `kDigitalTwinState` | 퍼블리시 역할 |
...
| 디바이스별 (`DeviceLogSlot[4]`) | `num_channels`, `goal_positions[]`, `actual_positions[]`, `actual_velocities[]`, `efforts[]`, `commands[]`, `target_positions[]`, `target_velocities[]`, `trajectory_positions[]`, `trajectory_velocities[]`, `motor_positions[]`, `motor_velocities[]`, `motor_efforts[]`, `sensor_data[128]`, `sensor_data_raw[128]`, `valid` |
...
| 카테고리 | 필드 |
|---------|------|
| 디바이스별 (`GroupCommandSlot[4]`) | `num_channels`, `commands[]`, `goal_positions[]`, `target_positions[]`, `target_velocities[]`, `trajectory_positions[]`, `trajectory_velocities[]`, `actual_positions[]`, `actual_velocities[]`, `motor_positions[]`, `motor_velocities[]`, `motor_efforts[]`, `sensor_data[]`, `sensor_data_raw[]`, `inference_data[]` |
