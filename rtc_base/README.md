...
| `PublishRole` | `kJointCommand`, `kRos2Command`, `kGuiPosition`, `kRobotTarget`, `kDeviceStateLog`, `kDeviceSensorLog`, `kDigitalTwinState`, `kGraspState`, `kWbcState`, `kToFSnapshot` | 퍼블리시 역할 |
...
- 서브클래스에서 히스토그램 범위를 커스터마이징하는 템플릿 파라미터: `Buckets` (기본 20), `BucketWidthUs` (기본 100us), `BudgetUs` (기본 2000us). 서브클래스에서 백분위 해상도가 부족하면 `Buckets` × `BucketWidthUs`를 같게 유지하면서 `BucketWidthUs`를 줄이세요 (예: `ControllerTimingProfiler`는 `<200, 10, 2000>`로 10 µs 해상도, `HandTimingProfiler`는 `<250, 20, 2000>`로 20 µs 해상도)
...
| `CmTimingBuffer` (`ThreadTimingProducer<RtTickTimingPayload, 512>`) | `ThreadTimingSample<RtTickTimingPayload>` | 512 (~1초 @500Hz) | CM RT loop per-tick timing CSV |
...
| `HandUdpTimingBuffer` (`ThreadTimingProducer<RtTickTimingPayload, 512>`) | `ThreadTimingSample<RtTickTimingPayload>` | 512 (~1초 @500Hz) | hand UDP EventLoop per-tick timing CSV |
...
CM, MPC, ur5e_hand_driver의 hand UDP EventLoop가 이 패턴의 세 사용처. 향후 ONNX inference 등 새 thread는 동일 인프라를 재사용한다 — `RTControllerInterface` 변경 없음.
...
> Per-tick RT-loop timing (`t_state_us`, `t_compute_us`, `t_publish_us`, `t_total_us`, `jitter_us`)은 별도 transport — `RtTickTimingPayload` + `CmTimingBuffer` / `MpcTimingBuffer` (`timing/rt_tick_timing_sample.hpp`, CM/MPC 공유) — 로 옮겼다. `LogEntry` 는 device-state / sensor / inference CSV 만 담는다.
...
> `cm_timing_log.csv` 는 `DataLogger` 가 소유하지 않는다. CM RT loop 의 per-tick timing 은 `ThreadTimingCsvLogger<RtTickTimingPayload>` 가 직접 기록한다 (스키마: `t_wall_ns, tick_count, t_state_us, t_compute_us, t_publish_us, t_total_us, jitter_us`). MPC thread는 동일 payload로 `mpc_timing_log.csv`, ur5e_hand_driver의 EventLoop는 `hand_udp_timing_log.csv` 를 emit — `rtc_base/timing/rt_tick_timing_sample.hpp` 단일 헤더 공유.
