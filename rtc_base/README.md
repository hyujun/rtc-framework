...
> **참고:** UR5e custom hand 전용 상수 (`kNumHandMotors`, `kDefaultHandMotorNames`, `kDefaultFingertipNames`)는 `udp_hand_driver/hand_constants.hpp`로 이동되었습니다. `kNumHandJoints`, `kDefaultRobotJointNames`, `RobotState`는 사용처가 없어 삭제되었습니다.
...
> **참고:** 레거시 타입 `HandState`는 `udp_hand_driver/hand_state.hpp`로 이동되었습니다. `RobotState`는 사용처가 없어 삭제되었습니다.
