...
**코어 수별 RT 스레드 레이아웃 (Phase 5 — MPC dedicated core 포함):**
...
| **mpc_main** (FIFO 60) | Core 3¹ | Core 4² | **Core 4** | Core 9³ | **Core 9** | **Core 9** |
| **mpc_worker_0** (FIFO 55) | — | — | — | — | **Core 10** | **Core 10** |
| **mpc_worker_1** (FIFO 55) | — | — | — | — | — | **Core 11** |
| **udp_recv** (FIFO 65) | Core 2 | Core 5 | Core 5 | Core 9 | Core 11 | Core 12 |
| **logger** (OTHER -5) | Core 3 | Core 4 | Core 6 | Core 9 | Core 11 | Core 13 |
| **rt_publish** (OTHER -3) | Core 3 | Core 5 | Core 7 | Core 9 | Core 11 | Core 14 |
| **aux** (OTHER 0) | Core 3 | Core 5 | Core 7 | Core 9 | Core 11 | Core 14 |
...
> ³ 10코어는 MPC가 Core 9를 udp_recv/logging/aux와 공유 (system cpuset 제약).
> **8/12/16코어는 MPC 전용 코어를 확보**. MPC main 우선순위는 항상 sensor_io(70)보다 낮아 sensor callback이 긴 solve를 preempt.
> 12/16코어는 worker 1–2개를 추가로 제공하여 Aligator 등 parallel rollout 솔버를 지원.

**MuJoCo 시뮬레이션 코어 레이아웃:**

`GetSimCoreLayout(physical_cores)` -- `constexpr` 함수로 시뮬레이션/뷰어 스레드 코어를 할당합니다.
Phase 5에서 MPC가 Core 9–11을 점유하므로 16코어의 sim_thread_core가 Core 15로 이동했고, 10/12코어에서는 dedicated sim core를 제공하지 않습니다.

| 물리 코어 수 | sim_thread_core | viewer_core | 비고 |
|-------------|----------------|-------------|------|
| 16+ | 15 | -1 (OS) | MPC(9–11) + IO(12–14) 회피 |
| 12+ | -1 (없음) | -1 | MPC(9–10) + 공유 IO(11)로 여유 코어 없음 |
| 10+ | -1 (없음) | -1 | MPC가 Core 9 공유 |
| 8+ | -1 (없음) | -1 | sim 모드는 cset shield를 Core 2–3으로 축소 |
...
- `SelectThreadConfigs()`가 물리 코어 수에 맞는 `kMpcConfig{4,6,8,10,12,16}Core` 프리셋을 채워 반환.
