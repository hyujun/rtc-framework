...
**Stage C-1 — ClikReferenceGenerator + 공용 SE(3) 오차 (U1 통일).** Kinematics WBC (`ClikReferenceGenerator`) 와 dynamics WBC (`SE3Task`/`ObjectSE3Task`) 는 SE(3) pose 오차를 단일 공유 helper `kinematics/se3_error.hpp` 의 `ComputeTaskPoseError` 로 동일하게 계산한다 — `rtc_math` 의 **LWA-frame `BodyLog6`** = `twistLocalToWorld(log6(T⁻¹T_d))`. 이는 registered-frame `LOCAL_WORLD_ALIGNED` Jacobian `rf.J` 와 같은 프레임이며, 두 command-source 가 동일 척도로 오차를 측정한다 (A/B 비교 전제). 회전 0 이면 screw coupling 이 소멸해 위치부는 `p_d−p` 로 환원된다. Dynamics 경로는 속도 오차로 `ComputeTaskVelocityError` (= `exactPoseErrorRate`, Jlog6 정방향 — `e_pos` 의 정확한 시간미분 `ė`, 소오차서 `v_des − J·v` 로 환원) 를 추가로 쓰고, kinematics 경로는 1차 속도 법칙 `r_task = Kx ⊙ e_x` 를 유지한다 (U1: Jlog6⁻¹ 미사용). rtc_math `log3` (quaternion+atan2) 는 θ=π 에서 robust 하므로 과거의 명시적 θ→π clamp 는 제거됐다. `kinematics/clik_reference.hpp` 의 `ClikReferenceGenerator` 는 TSID 가속도 적분의 대안인 velocity-level CLIK one-step reference `(q_ref, v_ref)` 를 생성한다: L1 TCP (arm-only, 댐핑 우역행렬 `J_a♯ = J_aᵀ(J_aJ_aᵀ+μ²I)⁻¹` — GraspCache 패턴의 6×6 LDLT, SVD 미사용), L2 arm nullspace posture (projector 비형성 동치식 `v_p + J_a♯(Kx⊙e − J_a·v_p)`), L3 hand posture (projection 없음). Robot-agnostic: arm/hand velocity index set 은 controller 가 주입 (ARCH-1), `PinocchioCache` 의 registered-frame J/oMf 재사용 (별도 FK 없음), measured anchoring `q_ref = q_meas + v_ref·dt` (nq==nv reduced tree 전제), per-joint `v_limit` clamp, `Manipulability()` (LDLT pivot √det) / `TcpErrorNorm()` 진단. RT alloc 0 (모든 workspace Init() pre-alloc, `Compute()` noexcept — `test_clik_reference` 의 TU-local alloc counter 로 검증).
