...
class ConstraintBase {
 public:
  ...
  [[nodiscard]] virtual int eq_dim(const ContactState& contacts) const noexcept = 0;
  [[nodiscard]] virtual int ineq_dim(const ContactState& contacts) const noexcept = 0;

  // Equality: A·z = b — RT-safe
  // A_block: [eq_dim × n_vars], caller가 zero-init
  virtual void compute_equality(const PinocchioCache& cache, const ContactState& contacts,
  ...
  virtual void compute_inequality(const PinocchioCache& cache, const ContactState& contacts,
                                  const RobotModelInfo& robot_info, int n_vars,
