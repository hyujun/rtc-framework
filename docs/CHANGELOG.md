...
### 추가 (Added) — 궤적 생성 서브시스템 (`ur5e_rt_controller/trajectory/`)

CLIK 및 OSC 제어기에서 목표점으로의 부드러운 이동을 위한 헤더-전용 5차 다항식 궤적 생성 라이브러리가 추가되었습니다. `namespace ur5e_rt_controller::trajectory`에 위치합니다.

#### 신규 헤더 파일

| 파일 | 역할 |
|------|------|
| `trajectory/trajectory_utils.hpp` | `QuinticPolynomial` — 스칼라 5차 다항식 (위치·속도·가속도 경계 조건 만족) |
| `trajectory/task_space_trajectory.hpp` | `TaskSpaceTrajectory` — SE(3) 공간의 5차 스플라인 (Pinocchio `log6`/`exp6` 사용) |
| `trajectory/joint_space_trajectory.hpp` | `JointSpaceTrajectory<N>` — N-DOF 관절공간 5차 스플라인 (템플릿) |

#### `TaskSpaceTrajectory` 통합 (CLIK, OSC)

`ClikController` 및 `OperationalSpaceController`의 `SetRobotTarget()` 호출 시 자동으로 SE(3) 5차 궤적을 생성합니다:
- 지속 시간 자동 계산: `duration = max(0.5s, distance / 0.2m)` (평균 최대 20cm/s)
- `Compute()` 500Hz 루프에서 `traj_.compute(trajectory_time_)` 호출 → 궤적 추종
