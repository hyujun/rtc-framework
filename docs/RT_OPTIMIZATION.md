...
- **`cpu_core = -1` sentinel**: `ValidateThreadConfig` 가 통과시키고 `ApplyThreadConfig` 가 affinity step 만 skip (scheduler/priority/nice/name 은 정상 적용). 처리 함정은 sentinel 처럼 검증 계약을 **완화**할 때는 옛 계약을 박아둔 validator·applier·test 를 착수 전에 전부 찾아야 한다 — 한 곳이라도 옛 계약을 강제하면 sentinel 값이 그 게이트에서 거부된다 (강화 방향도 대칭으로 같다).
