...
| `install.sh` | `/` (루트) | ~1143 | 전체 설치 파이프라인 (의존성, RT 권한, 커널, 시스템 설정) |
...
| `setup_cpu_governor.sh` | `rtc_scripts/scripts/` | ~249 | CPU governor performance 모드 + systemd 서비스 |
| `setup_nvidia_rt.sh` | `rtc_scripts/scripts/` | ~746 | NVIDIA GPU + RT 커널 공존 설정 (7단계) |
...
| `make_logger PREFIX [STYLE]` | 로깅 프리픽스 설정. STYLE: `bracket`(기본, `[PREFIX] ...`) 또는 `emoji`(`🔧 ...`) |
...
| `check_workspace_structure DIR` | 워크스페이스 디렉토리 구조 검증 (package.xml 존재 확인) |
| `ensure_ros2_sourced` | ROS2 환경 소싱 확인 및 자동 소싱 |
| `get_base_packages` | 기본 패키지 목록 반환 (단일 진실 공급원) |
