...
| `rt_common.sh` | `repo_scripts/scripts/lib/` | ~1031 | 공유 유틸리티 라이브러리 (로깅, CPU 감지, NIC 감지, 패키지 목록, systemd 헬퍼, hybrid CPU 감지, MPC core layout SSoT) |
| `build_rt_kernel.sh` | `repo_scripts/scripts/` | ~1057 | PREEMPT_RT 커널 빌드 및 설치 (7단계) |
| `cpu_shield.sh` | `repo_scripts/scripts/` | ~339 | 동적 CPU 격리 관리 (cset shield 기반) |
| `setup_irq_affinity.sh` | `repo_scripts/scripts/` | ~187 | 하드웨어 IRQ를 OS 코어에 고정 (SMT-aware 마스크) |
| `setup_udp_optimization.sh` | `repo_scripts/scripts/` | ~230 | 네트워크/UDP 실시간 최적화 + systemd 영구 적용 |
| `setup_grub_rt.sh` | `repo_scripts/scripts/` | ~228 | GRUB RT 커널 파라미터 통합 관리 (single source of truth) |
| `setup_display_rt.sh` | `repo_scripts/scripts/` | ~235 | X11 anti-tearing + compositor 우선순위 부스트 |
| `setup_cpu_governor.sh` | `repo_scripts/scripts/` | ~250 | CPU governor performance 모드 + systemd 서비스 |
| `setup_nvidia_rt.sh` | `repo_scripts/scripts/` | ~744 | NVIDIA GPU + RT 커널 공존 설정 (7단계) |
| `check_rt_setup.sh` | `repo_scripts/scripts/` | ~1072 | RT 환경 검증 (9개 카테고리) |
| `verify_rt_runtime.sh` | `repo_scripts/scripts/` | ~1019 | 제어기 구동 중 RT 런타임 검증 (7개 카테고리) |
...
## RT 설정 스크립트 (`repo_scripts/scripts/`)
