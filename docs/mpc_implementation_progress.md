...
- [x] `sim.launch.py mpc_engine:=handler` arg wired AND actually overrides the YAML — launch-level override bridge shipped in follow-up `8d8920a` (`rtc_controller_manager::ApplyControllerParamOverrides`). Handler-mode activation verified end-to-end on the live MuJoCo simulator (2026-04-21 session, commit `6e49bc9` + followups).
- [x] `MPCSolutionManager::GetSolveStats()` percentile probe shipped (follow-up `02e119e`) — returns {count, window, last, min, max, p50, p99, mean} over a 256-sample ring. Enables p50/p99 measurement against the plan exit metrics without recompiling rtc_mpc.
- [ ] MuJoCo E2E p50/p99 exit metrics (plan §7c: KinoDynamics p50<10ms, p99<20ms; FullDynamics p50<25ms, p99<45ms) — **deferred to user perf measurement** (use `GetSolveStats` via a monitor node; auto-log wiring in DemoWbcController is a separate follow-up since polling the probe requires either `try_lock` support or a non-RT wall-timer).
...
| 2026-04-21 | Phase 7 follow-ups (commits `1cfed80` + `c47005b` + `8d8920a` + `9d6288f` + `02e119e`): (1) `sim.launch.py` + 5 other launch files get `namespace=''` for Jazzy LifecycleNode, (2) `rtc_controller_manager::ApplyControllerParamOverrides` bridges ROS param overrides into the YAML node before LoadConfig — `enable_mpc` / `mpc_engine` launch args now actually mutate the controller config (previously silently ignored since Phase 5), (3) DemoWbcController contacts=0 bug: passing `tsid_node["contacts"]` double-indexed into ContactManagerConfig::load → fixed to pass `tsid_node`, (4) `MPCSolutionManager::GetSolveStats()` percentile probe + ring buffer + 4 new gtest cases (total rtc_mpc 136/136 green) — enables consumer-side p50/p99 measurement for Phase 7c exit criteria validation. |
