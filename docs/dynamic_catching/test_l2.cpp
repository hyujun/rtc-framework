...
int main() {
  ...
  //    s=0 만 보면 D1(0)=1 외에 기저 미분 계수가 전혀 구속되지 않는다.
  {
    ...
    char b[128];
    std::snprintf(b, sizeof b, "p=%.1e v=%.1e a=%.1e (s=0 and s=1)", ep, ev, ea);
  ...
  //     이것이 없으면 D0..D5 / S0..S5 계수와 h 거듭제곱이 사실상 무검증이다.
  {
    ...
    char b[128];
    std::snprintf(b, sizeof b, "|v - dp/dt|=%.1e  |a - d2p/dt2|=%.1e", ev, ea);
  ...
  // 2) C^2: 샘플 경계 좌우에서 a 가 이어진다
  {
    ...
    char b[128];
    std::snprintf(b, sizeof b, "Hermite %.1e m/s^2 vs Taylor(1-sample) %.3e m/s^2", jump_h, jump_t);
  ...
  // 3) 순수 중력(a 상수)이면 5차 Hermite 가 참 궤적을 정확히 재현
  {
    ...
    ball::Model m;
    ball::State x;
    x << -2.5, 0.2, 0.8, 4.0, -0.2, 3.5, 0.0;
    double worst = 0.0;
    for (double t = 0.0; t < 0.9; t += kRt) {
      ball::State y = x;
      if (t > 0)
        ball::propagate(m, y, t, 1e-4, 1 << 20);
      worst = std::max(worst, (traj::sampleAt(tr, t).p - y.head<3>()).norm());
    }
    char b[128];
    std::snprintf(b, sizeof b, "max pos err = %.2e m (2차 궤적은 5차 기저에 포함)", worst);
  ...
  // 4) 항력이 있으면 보간 오차가 남는다 — 얼마나 남는지 기록
  {
    ...
    char b[128];
    std::snprintf(b, sizeof b, "pos %.2e m, accel %.2e m/s^2 (dt_cam=%.1f ms)", wp, wa, 1e3 * kCam);
  ...
  // 5) 지평 밖은 외삽 플래그
  {
    ...
    std::snprintf(b, sizeof b, "in=%d/%d out=%d/%d pre=%d/%d end=%d/%d (horizon %.4f s)",
                  int(in.before_horizon), int(in.after_horizon), int(out.before_horizon),
                  int(out.after_horizon), int(pre.before_horizon), int(pre.after_horizon),
                  int(end.before_horizon), int(end.after_horizon), tr.s[tr.n - 1].t);
    check(!in.extrapolated && out.after_horizon && !out.before_horizon && pre.before_horizon &&
              !pre.after_horizon && !end.extrapolated && end.valid,
  ...
  // 6) hint 커서: 순차 접근 결과가 이진 탐색 결과와 동일
  {
    ...
    char b[96];
    std::snprintf(b, sizeof b, "max diff = %.1e m", worst);
  ...
  // 7) 형식 검사기
  {
    ...
    char b[128];
    std::snprintf(b, sizeof b, "ok=%d dt=[%.2f,%.2f]ms horizon=%.3fs ; 위반주입 ok=%d mono=%d",
                  int(ok.ok), 1e3 * ok.dt_min, 1e3 * ok.dt_max, ok.horizon, int(bad.ok),
                  int(bad.t_monotonic));
  ...
  // 8) 회귀: 찢어진 읽기로 t 가 비단조가 되면 invalid 로 알린다 (조용히 A 를 돌려주지 않는다)
  {
    ...
    char b[96];
    std::snprintf(b, sizeof b, "h=0 -> valid=%d", int(e.valid));
