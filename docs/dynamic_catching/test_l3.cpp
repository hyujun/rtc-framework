...
ball::State ballAt(double t) {
  ...
  if (t > 0.0) ball::propagate(ball::Model{}, x, t, 1e-4, 1 << 20);
  return x;
}
...
Peak gammaRollout(double gf, double Tw) {
  ...
  for (int k = 0; k < N; ++k) {
    ...
    if (t >= kTc - Tw) { pk.a = std::max(pk.a, o.u_des.norm()); pk.v = std::max(pk.v, o.xd.norm()); }
  }
  return pk;
}
...
int main() {
  ...
  // §4.3 tMin: 고정 케이스 + verify_l3.py 용 cases.txt
  {
    ...
    for (int i = 0; i < 40; ++i) {
      ...
      if (fp) std::fprintf(fp, "%.17g %.17g %.17g %.17g %.17g %.17g\n",
                           q0, w0, q1, wm, a, plan::tMin(q0, w0, q1, wm, a));
    }
    if (fp) std::fclose(fp);
    ...
    char s[176];
    std::snprintf(s, sizeof s, "tri=%.1e trap=%.1e rev=%.1e ovr=%.1e tRest=%.1e ; cases.txt 40 rows",
                  e_tri, e_trap, e_rev, e_ovr, e_rest);
  ...
  // 회귀(C9): |w0| > w_max 는 clamp 되고 플래그가 선다 (이전에는 음수 구간을 포함한 값을 반환)
  {
    ...
    char s[112]; std::snprintf(s, sizeof s, "w0=6>pi -> clamped=%d t=%.4f ; w0=3 -> clamped=%d",
  ...
  // (clamp 가 음수를 0 으로 가려준다).
  {
    ...
    char s[128]; std::snprintf(s, sizeof s, "g_min: %.6f (기대 0.6) , %.6f (기대 0.75)",
                               w1.g_min, w2.g_min);
  ...
  // 5 ms 정확히는 g_min == g_max == 0 인 knife-edge 라 여유를 두고 양옆에서 본다.
  {
    ...
    std::snprintf(s, sizeof s, "4.9ms ok=%d(g_min=%.4f) ; 5.1ms ok=%d(g_min=%.4f) ; 임계 %.4f ms",
                  int(w_ok.feasible()), w_ok.g_min, int(w_bad.feasible()), w_bad.g_min, 1e3 * t_crit);
  ...
  // 회귀: 비물리적 입력(음수 v_dir_max)에 clamp 가 판정을 뒤집지 않는가
  {
    ...
    char s[144]; std::snprintf(s, sizeof s, "v_dir=-1 -> invalid=%d feasible=%d ; v_dir=+1 -> invalid=%d",
  ...
  // 회귀(C6): v_tcp_max 가 g_max 를 실제로 구속하는가
  {
    ...
    char s[112]; std::snprintf(s, sizeof s, "g_max: v_tcp=3 -> %.3f ; v_tcp=1 -> %.3f", a.g_max, b.g_max);
  ...
  // 창이 비는 임계 속력
  {
    ...
    char s[128];
    std::snprintf(s, sizeof s, "v_lim=%.3f m/s (v_dir=1.5, d=4cm, T=60ms) ; 6:%d 2:%d",
                  v_lim, int(at6), int(at2));
    check(std::abs(v_lim - (1.5 + 0.04 / 0.06)) < 1e-12 && !at6 && at2,
          "§4.5 max catchable speed", s);
  }
  ...
  for (double gf : gfs) std::printf("%13.1f", gf);
  std::printf("\n");
  for (double Tw : {0.30, 0.45, 0.60, 0.75}) {
    std::printf("  %-8.2f", Tw);
    for (double gf : gfs) { const auto p = gammaRollout(gf, Tw); std::printf("%9.1f/%.2f", p.a, p.v); }
    std::printf("\n");
  }
  {
    const auto p = gammaRollout(0.4, 0.30);
    char s[112]; std::snprintf(s, sizeof s, "(T_w=0.30, gf=0.4) -> %.1f m/s^2 (문서 33.0 +-5%%)", p.a);
