...
[[nodiscard]] inline Eval interpolate(const Sample& A, const Sample& B, double t) noexcept {
  ...
  if (!(h > 0.0)) return {A.p, A.v, A.a, false, false, false, false};
...
[[nodiscard]] inline Eval extrapolate(const Sample& S, double t, bool after) noexcept {
  ...
  return {S.p + S.v * d + 0.5 * S.a * d * d, S.v + S.a * d, S.a,
          !after, after, true, true};
}
...
[[nodiscard]] inline Eval sampleAt(const PredictedTrajectory& tr, double t, int& hint) noexcept {
  ...
  if (tr.n == 1) return extrapolate(tr.s[0], t, t > tr.s[0].t);
  if (t < tr.s[0].t)           return extrapolate(tr.s[0], t, false);
  if (t > tr.s[tr.n - 1].t)    return extrapolate(tr.s[tr.n - 1], t, true);
  if (t == tr.s[tr.n - 1].t)   return {tr.s[tr.n - 1].p, tr.s[tr.n - 1].v, tr.s[tr.n - 1].a,
                                       false, false, false, true};   // 지평 끝은 외삽이 아니다
