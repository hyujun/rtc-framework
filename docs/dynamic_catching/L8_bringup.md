...
| G8-C | ~~`iiwa7_leap`에서~~ **`ur5e_p1b` 에서** (D-S8-9, 2026-09-24) $\gamma=0$ 고정 (`planner.gamma.grid: [0.0]` overlay) 대 계획 γ ablation: 간극·상대속도·**충격량**·접촉력 분포 비교. G8-E 와 함께 **2×2 factorial** (lead {off, on} × γ {0, 계획}, 같은 seed 투척 ≈ 100 × 4 arm, 한 세션·블록 무작위) 의 lead on 에서의 γ 주효과로 판정하고 상호작용을 보고한다 (plan §4.4 S8) | `[SIM-ANY]` |
...
| G8-E | `ur5e_p1b` 에서 선행 보상 유무 비교. **2026-09-24 교체 (D-S8-1)**: sim 팔 actuator 가 시정수 ≈ 200 ms 의 1차 지연이라 (L5 §4.4 정정) fixture 없이 **sim 런타임 lead on/off** 로 비교한다 — S8-B overlay (`joint_cmd.lag.{T_arm: τ̂, lead_enable}` + `T_freeze` 0.52 · `horizon_min` 0.66 · `n_min` 15, 두 arm 동일 T_freeze). 2×2 factorial (G8-C 행) 의 계획 γ 에서의 lead 주효과. **판정 = t_c 서보 성분 ‖FK(q_meas) − FK(q_cmd)‖ 중앙값이 lead on 에서 감소 (부호 일치)**, 1차 지연의 비-순수지연분인 잔여를 보고, 성공률 차는 기록. sim G8-E 는 200 ms 1차 플랜트 위 검증이며 UR5e 이득을 예측하지 않는다. ~~substrate 없음 → fixture 전용 지연 주입 위 판정 (2026-09-22)~~ — 그 fixture 판정은 L5 §9 G5-E (S5.3 PASS) 로 남는다 | `[SIM-P1B]` |
