...
- **L8.6** (S8-A·B·C·E) `sim_ur5e_p1b` 폐루프 → 시스템 게이트 G8-A~C·G8-E·G8-D (2026-09-22 결정으로 `ur5e_p1b` 가 1차 — 종전 순서 "leap 먼저" 는 S3.5b 이전 서술. G8-C 는 p1b 에서 `planner.gamma.grid: [0.0]` overlay 로, D-S8-9). **S8-E 결과 (2026-09-26)**: G8-D **PASS** 93/200 (plan D-S8-17 재실행 반영; 사전 규칙 원 판정 83/200 FAIL 병기) — 게이트별 판정은 §9.1.
