...
int main() {
  ...
  // 1) k = 0 이면 해석해와 일치
  {
    Model m; State x; x << 0, 0, 0, 3, -1, 6, 0.0;
    const double T = 0.8;
    State y = x;
    ...
    char b[96]; std::snprintf(b, sizeof b, "err = %.2e m  (< 1e-9)", err);
    check(err < 1e-9, "1. analytic solution (k=0)", b);
  }

  // 2) 역학적 에너지 비증가: Edot = -k |v|^3
  {
    Model m; State x; x << 0, 0, 0, 4, 2, 7, 0.0229;
    ...
    char b[96]; std::snprintf(b, sizeof b, "max dE = %.2e  (<= 0 + rounding)", worst);
    check(worst < 1e-9, "2. mechanical energy non-increasing", b);
  }

  // 3) 스텝 절반 → 전역 오차 약 1/16 (4차 수렴)
  {
    Model m; State x0; x0 << 0, 0, 0, 5, 1, 8, 0.0229;
    const double T = 0.5;
    auto run = [&](double h) { State y = x0; propagate(m, y, T, h, 1 << 20); return y; };
    ...
    char b[96]; std::snprintf(b, sizeof b, "ratio = %.2f  (12-20)", ratio);
    check(ratio > 12.0 && ratio < 20.0, "3. RK4 4th-order convergence", b);
  }

  // 4) STM vs 중심 유한차분
  {
    Model m; State x0; x0 << 0.2, -0.1, 1.0, 4, 2, 7, 0.0229;
    const double T = 0.5, h = 2e-3;
    const int n = static_cast<int>(T / h + 0.5);
    auto prop = [&](State s) { for (int i = 0; i < n; ++i) s = rk4(m, s, h); return s; };
    State x = x0; Mat7 Phi = Mat7::Identity();
    for (int i = 0; i < n; ++i) rk4WithStm(m, x, Phi, h);
    Mat7 Fd; const double d = 1e-6;
    for (int j = 0; j < 7; ++j) {
      State xp = x0, xm = x0; xp(j) += d; xm(j) -= d;
      Fd.col(j) = (prop(xp) - prop(xm)) / (2.0 * d);
    }
    const double err = (Phi - Fd).cwiseAbs().maxCoeff();
    char b[96]; std::snprintf(b, sizeof b, "max|Phi - FD| = %.2e  (< 1e-6)", err);
  ...
  {
    Model m; State x; x << 0, 0, 0, 2e-4, 0.0, -1e-4, 0.05;   // |v| = 2.24e-4 << v_eps
    ...
    char b[112]; std::snprintf(b, sizeof b, "rel err = %.2e  (clamp bug -> v_eps/|v| = %.2f)",
                               rel, m.v_eps / v.norm());
  ...
  {
    Model m; State x; x << 0, 0, 0, 4, 0, 6, 0.0229;
    State a = x, b_ = x;
    const auto r1 = propagate(m, a, 0.2, 2e-3, 100);   // 정확히 100 스텝, 포화 아님
    const auto r2 = propagate(m, b_, 0.5, 2e-3, 100);  // 250 요구 → 100 으로 잘림
    char b[128];
    std::snprintf(b, sizeof b, "T=0.2:{n=%d,tr=%d}  T=0.5:{n=%d,tr=%d,h=%.1fms}",
                  r1.steps, int(r1.truncated), r2.steps, int(r2.truncated), 1e3 * r2.h_used);
