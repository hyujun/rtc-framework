...
ball::State ballAt(double t) {
  ...
  if (t > 0.0)
    ball::propagate(model(), x, t, 1e-4, 1 << 20);
  return x;
}
...
struct RunOut {
  double gap, relv, peak_a, peak_v, gamma;
};

// pred_offset: 계획이 믿은 포구점 - 참 포구점 (예측 오차 delta)
RunOut run(const Eigen::Vector3d& pred_offset, double gf, double Tw, double a_max = 1e9,
           double v_max = 1e9) {
...
int main() {
  ...
  // §4.3 재예측 점프: e' = e - (1-g) dp,  ed' = ed + gdot dp
  {
    ...
    for (int k = 0; k < 250; ++k)
      (void)ds.step(toTarget(ballAt(k * kDt)), k * kDt, kDt);
    ...
    char s[96];
    std::snprintf(s, sizeof s, "|de|=%.1e |ded|=%.1e  (< 1e-9)", e_err, ed_err);
  ...
  // §4.7 이산 안정 경계 s = omega*h < 2*sqrt(2)-2
  {
    ...
    char s[128];
    std::snprintf(s, sizeof s, "s=0.827 conv=%d, s=0.830 div=%d, bound=%.6f", int(!diverges(0.827)),
                  int(diverges(0.830)), 2 * std::sqrt(2.0) - 2);
  ...
  // §4.5 축 정렬: 회전벡터 오차의 연속성·단조성 + 정확성 + Jacobian
  {
    ...
    for (int i = 0; i <= 180; ++i) {
      ...
      if (prev >= 0.0)
        max_jump = std::max(max_jump, std::abs(n - prev));
      prev = n;
      const Eigen::Vector3d e = ref::axisAlignError(z, ad);
      const double a = e.norm();
      const Eigen::Matrix3d R =
          (a < 1e-12) ? Eigen::Matrix3d::Identity() : Eigen::Matrix3d(Eigen::AngleAxisd(a, e / a));
    ...
    std::printf("\n");
    char s[128];
    std::snprintf(s, sizeof s, "max 1deg jump = %.3f rad/s (sin 기반은 5.6)", max_jump);
  ...
  // 회귀(C3): 속도 포화 시 반환 xdd 가 실현 가속도와 일치
  {
    ...
    char s[112];
    std::snprintf(s, sizeof s, "xdd=%.3f realized=%.3f u_des=%.1f sat=%d", o.xdd.x(), realized,
                  o.u_des.x(), int(o.saturated));
  ...
  // D3: gamma 하향 — 세 분기(Applied / Frozen / Rejected)와 연속성
  {
    ...
    std::snprintf(s, sizeof s, "ramp-up(g=0.159)->%s  late(g=0.385)->%s  up->%s",
                  r1 == ref::DerateResult::Frozen
                      ? "Frozen"
                      : (r1 == ref::DerateResult::Applied ? "Applied" : "Rejected"),
                  r2 == ref::DerateResult::Applied
                      ? "Applied"
                      : (r2 == ref::DerateResult::Frozen ? "Frozen" : "Rejected"),
                  r3 == ref::DerateResult::Rejected ? "Rejected" : "?");
    check(r1 == ref::DerateResult::Frozen && r2 == ref::DerateResult::Applied &&
              r3 == ref::DerateResult::Rejected,
          "D3a derateGamma three branches", s);
  }
  ...
  // D3c: ed 점프는 시간에 **단조가 아니다** — 램프 중앙에 봉우리가 있다 (L7 §4.6)
  {
    ...
    for (double t = 0.41; t < kTc; t += 0.01) {
      ...
      if (std::abs(t - 0.45) < 5e-3)
        at_early = j;
      if (std::abs(t - 0.76) < 5e-3)
        at_late = j;
    }
    char s[176];
    std::snprintf(s, sizeof s, "peak %.3f at t=%.2f ; t=0.45:%.3f t=0.76:%.3f (단조 아님)", peak,
                  peak_t, at_early, at_late);
  ...
  // reset() 이 p_c, gamma 프로파일까지 지우는가 (재무장 오염 방지)
  {
    ...
    char s[160];
    std::snprintf(s, sizeof s, "hold err = %.1e m, gamma = %.3f", (o.x - hold).norm(), o.gamma);
    check((o.x - hold).norm() < 1e-9 && o.gamma == 0.0, "reset() clears p_c and gamma profile", s);
  }
