...
$d_{eff}$는 손바닥 접촉 전 폐쇄를 요구하면 포켓 깊이 $d$, 반발계수 $e$로 튕겨 나오기 전까지 허용하면 $d(1+1/e)$다(TBD-HAND-04). **S4.5 는 전자를 쓴다** — 접촉 시뮬레이션이 공을 손바닥에 앉힌 상태에서 재므로 반발 여유를 세지 않는다 (보수적, L6 §4.5).

**상한 (팔 속도).** 포구 자세 $q^\ast$에서 방향 $\hat v$로 낼 수 있는 최대 속력 $v_{dir,\max}$와 TCP 속도 한계 $v_{\max}$(= L4 `reference.v_max`)로 제한한다.

> **이 상한이 v1 의 구속이다 (S3.5a 지도, 2026-09-21).** 사용자 결정으로 속력 격차는 투척을 좁히는 대신 **팔 궤적으로 상대속도를 줄여** 닫는다 — 즉 위 하한 식이 쓰는 그 γ 다. 4 m·$T_f\ge1.0$ s 격자에서 $\gamma_{\min}$ 은 0.88–0.96 이고 그것은 팔이 5.7–8.0 m/s 로 후퇴해야 한다는 뜻인데, 출하 설정의 $\eta_vv_{\max}$ 는 1.50 m/s 라 $\gamma_{\max}\approx0.20$ 이다 — 창이 약 4.7 배로 비어 있다. 그러므로 이 방향의 선행 조건은 `reference.v_max` 실측이고 ($\gamma_{\max}$ 가 그 위에 세워져 있다), $v_{dir,\max}$ 는 자세마다 달라 지도가 후보별로 함께 내야 한다. 수치와 협상 구조는 plan §11.
