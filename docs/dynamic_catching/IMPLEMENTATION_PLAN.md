...
| S7 손 시퀀서·슈퍼바이저 | 대기 (브랜치 `feat/s7-hand-supervisor`, 착수는 이 docs 정정 커밋부터). **S6 sim 입력의 코드 대조 정정 (#537 S7 결정 2026-09-23)**: "COMMITTED 전이가 아직 없다" 는 이상 징후가 아니라 예정된 공백이다 — S5 driver 의 APPROACH 분기에 commit 질의가 없어(`CommitDue` 호출부 0) 정상 tick 은 절대 advance 하지 않는다. "APPROACH 중 stale → RETREAT 가 팔을 즉시 세운다" 도 정정: 현재 RETREAT 는 진입 즉시 advance 하고 `arm_qd_cmd_` 를 지우지 않아 마지막 CLIK 명령에서 1-tick 정지가 난 뒤 다음 tick 바로 ARMED 로 간다 — S7 설계의 RETREAT 는 정지 램프(`JointSpaceDecelStep`, ABORT_SAFE 경유 시 no-op) + 관절공간 `planner.wait_pose` 복귀다 (아래 §4.4 S7) | — |
| S8 sim 통합 평가 | 대기. **S6 에서 이월 (결정 ⑦, #537 코멘트 5793765020)**: 교체 채택 시 γ 램프 연속 ($\dot\gamma,\ddot\gamma$ 이어 받기) — RT 채택부·γ 프로파일 형태·계획기 rollout 동시 변경 (RT-1~10, `/code-review`). 이것 없이는 `refreshed` 가 램프 중 거부돼 첫 plan 만 쓴다 (L3 §4.7). sim 서보 지연 (`joint_cmd.lag` 가 sim 0) 이 t_c 오차를 지배하는 것도 여기서 본다 | — |
...
| σ_ℓ 모니터 소비자 | S7 착수 대상이 아니다 — `planner_cycle.cpp` 가 σ_ℓ 를 갱신은 하지만 소비자가 없다. **S8 이월** (D-8 포화 빈도 측정과 함께) |
