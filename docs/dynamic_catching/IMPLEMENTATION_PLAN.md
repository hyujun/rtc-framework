...
| `REF_SATURATED` 판정식 | `ref.saturated` 가 연속 `supervisor.sat_ticks` tick (기본 **50**, provisional — 5 는 단위 fixture 의 정상 접근 포화 10·76 tick 을 잘라 100 으로 올렸고, sim 기준 15 + 변형 10 투척 (`260923_2336`) 의 정상 연속 길이 max 44 · p99 42.5 를 보고 50 으로 내렸다, #537 결정 2026-09-24) 이면 승격 — 50 에서 sim 을 다시 돌려 정상 시행 발화 0 을 확인하고, 최종값은 S8 에서 정한다 (D-S7-4) |
