...
- 상태: **S0 완료** (2026-09-19) — S0.1~S0.9 게이트 PASS. S0.7 이 0.5 s profile 부족을 보고해 R2 채택·profile 지평이 §7.3 결정 대기다. 남은 승인은 S5·S6 착수 전 E-8·E-7. 승인이 막는 단계는 승인 전에 착수하지 않는다 (§4.1)
...
- 탄도 (항력 없음): 수평 이동 d = 4 − ρ_c, Δz = z_c − z₀ 에 대해 v₀² = g d² / (2cos²θ (d tanθ − Δz)), T_f = d / (v₀ cosθ)
...
| 항력 k | 0.0229 1/m | L2 §4 (공 사양 D-12 대기) |

| 항목 | 결과 |
|---|---|
| 도달 가능 투척 (v₀ 4–12 m/s) | v₀ ≥ 4.4–5.6 m/s, T_f 0.30–1.04 s, 포구 속력 ≥ 6.1 m/s |
| T_f ≥ 1.0 s 인 투척 (D-18, 사용자 결정 후 추가) | T_f 1.0 s 에서 앙각 42–54°, v₀ 5.0–5.4 m/s, 정점 z 2.5–2.6 m, 포구 속력 6.3–7.5 m/s, 하강각 약 60°. T_f 1.5 s 면 앙각 69–73°, v₀ 6.8–7.3 m/s, 정점 약 4 m, 포구 속력 8.0–8.8 m/s (세 포구점 기하 모서리) |
| 대표 (z₀ 1.75, ρ_c 0.5, z_c 0.6) | θ 0/20/40° → v₀ 7.2/5.3/5.0 m/s, T_f 0.48/0.70/0.91 s. 항력은 같은 v₀ 에서 T_f +4–5 % — 지평 판정에는 무시 |
| L | 0.140 s |
...
- **T_f ≥ 1.0 s 결정 (D-18, 2026-09-19) 의 효과:** T_f − T_det ≥ 0.9 s 가 sweep 최대 H_req 0.86 s 를 넘으므로, 지평만 충분하면 목표 분포의 모든 투척이 R2 까지 계획 가능하다 (먼 q* 의 H ≈ 1.0 s 는 T_f ≥ 1.1 s 부터). 필요 지평 자체는 로봇 쪽 값이라 그대로 0.46–0.86 s 이고 0.5 s profile 부족 판정도 그대로다. 대신 포구 속력 하한이 6.1 → 6.3 m/s 로 오르고 T_f 와 함께 커진다 — S4.4 부담은 커진다
- **권장**: sim profile 지평 0.8 s (17 점 @ 0.05 s, 설정은 사용자 — D-15), 그리고 대기 자세를 겨냥점 가까이 두어 T_move 를 줄인다. R2 채택과 profile 은 §7.3 결정 대기
- 포구 속력 ≥ 6.1 m/s 는 CATCHING_MASTER §4.1 의 우려(6 m/s 에 T_close,tot ≤ 8.9 ms 필요)를 4 m 투척에 대해 확인한다 — 거리를 줄여도 크게 내려가지 않으므로 S4.4 go/no-go 의 핵심 입력이다
...
| S0.7 지평 조건 | 필요 지평을 R1(commit) 만으로 볼지 R2(팔 이동 포함, 보수적) 로 볼지. 먼저 출발했다가 후보를 교체하는 방식을 허용하면 R2 요구가 준다 — 현재 L3 에 없음 | S3.6 |
| vision profile 지평 | S0.7 권장 0.8 s (17 점 @ 0.05 s) 로 sim profile 을 올릴지 (D-15, 설정은 사용자), 대기 자세를 겨냥점 가까이 둘지 | S3.4 |
| `kCap` | S0.7 제안 40 (provisional). 2의 거듭제곱을 원하면 64 | S1.2 |
...
**발사 조건 공간.** 발사점은 방위 φ 와 높이 z 로, 발사 속도는 크기·앙각·수평 방향으로 정한다. 수평 방향은 "발사점에서 겨냥점(aim point)을 향한 방향 + 편차" 로 두어, 사람이 로봇 쪽으로 던지되 좌우로 빗나가는 투척을 표현한다. 포구 후보는 비행시간 T_f ≥ 1.0 s 인 것만 남긴다 (D-18) — 무항력 기준 앙각 약 42–54° 이상, v₀ 약 5–5.5 m/s 이상, 정점 z 약 2.5 m 이상인 lob 이다 (S0.7 결과). 지도 도구의 초기 탐색 범위 (제안): φ ∈ [−π/2, π/2] (로봇 정면 반원), 편차 ∈ [−10°, +10°] — 탐색 범위일 뿐이며, 잡을 수 있는 범위는 지도 결과로 정한다.
...
| vision 지평이 짧으면 계획 가능한 포구 창이 준다 — S0.7 이 현 예시 profile 0.5 s 가 팔 이동 조건(R2, 0.46–0.86 s)에 부족함을 확인했다. 지평을 늘릴 주체는 외부 repo 이고 요청 채널이 없다 (P-3) | §7.3 profile 결정, S3.6 |
