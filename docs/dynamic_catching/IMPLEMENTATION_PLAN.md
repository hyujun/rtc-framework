...
| D-S8-18 (2026-09-27 사용자, #537 5847236227 R1–R4 권장 채택) | **S8-G 결정**: R1 출하 프로파일 `reference.a_max` 21 → **30** (ω 10 에서 abort 7 → 0 · 포화 17.6 → 1.1 %, 포획 동일; provisional 유지, 실기 값은 S10) · R2 **계획기 도달시간 한계 = 실행 envelope** — sim 은 `sim.yaml` 이 `robot.arm.accel_limits_path` 를 `derived_accel_limits_s8g_envelope.yaml` 로 덮고 (RT 노드 param override, `robot.yaml` 은 토크 도출 D-16 파일 유지 — 실기 envelope 은 S10) · R3 `reference.omega` 10 유지 (15·20 은 live 예측 잡음을 따라가 포획 1/4–1/5) · R4 **복제 규율**: 후속 ①·④ 의 판정 unit 은 같은 seed ≥ 2 회, 회차 간 McNemar 병기 (같은 seed·config 재현 11 대 4, p 0.039) |
...
- **S8-G sim 팔 예산 실측·재정의 (후속 ②; `[SPRINT]` 확정 2026-09-26, #537 5846760878; 분석 5846659919).** 브랜치 `feat/s8g-arm-budget`. **사전 분석 (S8-F-1 9 unit·1068 발 오프라인 재분석, `rtc_tools catching_arm_budget`)**: 팔 예산은 네 층 — **P** 플랜트 (τ̂ 0.049–0.054 s 전 관절, 토크 사용률 활성 tick max 0.61–0.83 · p99 ≤ 0.64, forcerange 도달 0 tick, 속도 캡 없음) · **R** L4 참조 (ω 10 · a_max 21 · v_max 3.5: 요구 가속 ‖u_des‖ p50 13–32 · max 54–65 m/s² 로 a_max 에서 포화, 활성 tick 의 3–25 %; ‖ẋ‖ max 2.5–3.25 로 η_v v_max 는 드물게) · **C** CLIK (속도 box = sim 정격 3.1416 rad/s 적중 tick 1–21 %; 가속은 결정 K 의 dynamic 토크 제약, `bound_conflict` 0; 실행된 관절 가속 envelope p95 14–37 rad/s²) · **B** 계획기 도달시간 box 2.03 rad/s² (실행을 묶지 않으면서 같은 운동의 도달시간을 0.60–0.89 s 로 재 `rank_reach` 93–100 % 실패; envelope 로 재면 0.15–0.25 s, §9). **t_c 간극 분해**: commit → t_c−T_arm 은 0.318 s (T_freeze − T_arm) 이고 참조 자기 잔여 e 는 80–120 mm → **13–24 mm** (임계감쇠 2차 DS 의 산술 $(1+\omega T)e^{-\omega T}$ = 0.174 — ω × 가용 시간이 지배 항, a_max 포화가 2 순위 (절벽 unit 18–24 대 LHS 13), CLIK 3–8 · 서보 3 mm 가 3 순위); 참조가 좇는 live 목표 자체가 진실에서 30–54 mm (하한), 손–공 total 41–61, d_min 31–65 → **예측 몫이 팔 몫보다 크다** (팔을 다 고쳐도 30–50 mm 남는다 — G8-B 후속). rank 실패율 (유효 plan): reach 93–100 · gamma 95–100 · rollout 98–100 (ε_term 2 mm 는 ω 10 으로 산술적으로 불가) · error_budget 100 · uncertainty 100 → 다섯 gate 가 전부 걸려 penalty 균일 = 점수 A/B 무효의 기구. **재정의**: `v_max` 는 재측정 대상이 아니다 (정격 도출값; sim 에 속도 캡이 없어 sim 이 실측해 줄 수 없고, S8-F 에서 γ 를 묶은 것은 자세 의존 v_dir,max — ① 의 영역); `a_max` 는 ω 와 짝으로 (ω, a_max) 실현 영역을 sim 에서 잰다; 계획기 도달시간 한계는 실행 envelope 로 교체 실험. 도구: `rtc_tools catching_arm_budget` (+ `--write-envelope-box`), 컨트롤러 미러 `reference.{omega,a_max,v_max}`·`planner.gamma.eta_v`·`robot.arm.{qdd_max,accel_limits_path}` (러너 `MIRROR_PARAMETERS`), overlay `s8g_w{10,15,20}_a{21,30}` (= `s8f_reach_first` + `reference` 두 잎) · `s8g_w10_a21_envbox` (`accel_limits_path` → `derived_accel_limits_s8g_envelope.yaml`, S8-F-1 9 unit envelope p95 관절별 max [20.3, 30.9, 37.0, 21.5, 14.2, 29.0]). AC: ① 도구 + 합성 positive control ② `hand_cliff` tennis 56 발 (seed 901) × 6 cell — 채택 후보 = e(t_c−T_arm) 최소 ∧ 토크 p99 ≤ 0.8 ∧ 서보 ≤ 5 mm ∧ `REF_SATURATED` abort 0, 포획 McNemar 대 S8-F-1 cliff_rf ③ envbox A/B (`rank_reach`·plan 교체·포획) ④ 문서. 출하 YAML 값 변경 없음 — 채택은 사용자 결정.
  - **S8-G 결과 (2026-09-26, #537 결과 코멘트; 브랜치 `feat/s8g-arm-budget`; 개발 PC non-RT, `hand_cliff` tennis 56 발 · seed 901 · 7 unit + S8-F-1 cliff_rf 쌍둥이).** 플랜트는 전 cell 에서 같다 (τ̂ 0.049–0.054 s, 토크 p99 ≤ 0.76, forcerange 0 tick). **(ω, a_max) 스윕** — e(t_c−T_arm) p50/p90 · `REF_SATURATED` abort · 포화 tick · 손–공 total · d_min · 포획: ω10·a21 **15.0/48.5 · 7 · 17.6 % · 44.0 · 38.4 · 11/56** (S8-F-1 쌍둥이 23.7/66.4 · 12 · 25.3 % · 43.1 · 31.3 · **4/56** — 같은 seed·같은 config 의 재현이 McNemar p 0.039 로 다르다: **56 발 unit 의 회차 분산이 S8-F-1 이 주장한 효과 크기와 같은 급**, ④·① 의 A/B 는 복제 unit 없이는 판정하지 않는다) · ω10·a30 **14.1/20.8 · 0 · 1.1 % · 41.7 · 42.5 · 10/56** (대 기준 p 1) · ω15·a21 7.8/46.1 · 8 · 16.7 % · 48.0 · 52.0 · **2/56** (p 0.004) · ω15·a30 8.9/19.4 · 0 · 3.8 % · 48.3 · 47.4 · 2/56 (p 0.012) · ω20·a21 8.6/57.9 · 13 · 27.8 % · 56.8 · 48.9 · 5/56 (p 0.18) · ω20·a30 7.4/15.3 · 0 · 5.8 % · 51.2 · 57.9 · 3/56 (p 0.039). **사전 등록한 채택 규칙 (e 최소 ∧ 토크 p99 ≤ 0.8 ∧ 서보 ≤ 5 ∧ abort 0) 은 ω20·a30 을 고르지만 그 cell 의 포획은 기준의 1/4 이다 — 규칙의 판정량이 틀렸다.** 기구: ω 를 올리면 참조 자기 잔여는 절반이 되지만 참조가 좇는 live 예측의 잡음까지 따라가 ref_vs_true 가 43–46 → 49–75 mm 로 커지고 손–공 total 이 42–44 → 48–57 mm 로 나빠진다 (ω 10 의 지연이 예측 잡음의 저역 필터였다); 이 격자에서 손–공·포획의 최적은 **ω 10**. 반면 **a_max 21 → 30 은 ω 10 에서 비용 없는 이득** — abort 7 → 0, 포화 17.6 → 1.1 %, e p90 48 → 21 mm, 포획 10 대 11 (p 1), 토크 p99 0.70. **envbox (AC-3)**: 계획기 box 를 실행 envelope 로 바꾸자 `rank_reach` 실패 95 → **14 %**, 도달 판정 통과 2 → 98 % — gate 가 정보를 갖는다; 포획 7 대 11 (p 0.42), plan 교체 0, abort 2 — 결과에는 영향 없음 (순위 penalty 가 균일했으니 예상대로). rollout gate 는 전 cell 에서 72–100 % 실패 (ε_term 2 mm 는 ω 10 × 0.32 s 로 산술적으로 불가) · uncertainty·error_budget 100 % — 다음 재정의 후보. **rig**: ω20·a21 첫 시도가 8 발째 `REF_SATURATED` abort 뒤 RETREAT 에서 팔이 전혀 안 움직여 (arm_err 0.99 rad · track_err 0.43 rad 상수, 손 effort 1.0 stalled — 물리적으로 걸림) HAND_TIMEOUT → IDLE, 러너가 복구 못 함 (`.fail1` 보존, 재시도는 56 발 완주 — 결정적 재현 아님); envbox 첫 시도는 추정기 activate 응답 유실 (S8-F-1 에 이어 2 회). **제안 (결정은 사용자)**: ① sim 프로파일 `reference.a_max` 30 (실기 값은 S10) ② 계획기 도달시간 한계 = 실행 envelope 원칙 채택 + sim 프로파일이 `derived_accel_limits_s8g_envelope.yaml` 을 가리킴 (출하 D-16 파일은 실기용으로 유지) ③ ω 10 유지 ④ S8-F 후속 ①·④ 의 판정은 복제 unit ≥ 2 로. 산출물: private `dynamic-catching-S8-tools/s8g/drv/{ab/, hn/, <cell>/}`. **결정 (2026-09-27 사용자, R1–R4 권장 채택 → D-S8-18)**: 출하 `reference.a_max` 30 (`demo_catching_controller.yaml`, `eta_jump` 램프 한계 52 → 75 mm 주석 갱신, `test_catching_clik_sweep` 의 출하 상수 30) · `sim.yaml` 이 계획기 box 를 envelope 파일로 덮음 (`test_catch_lead_overlays` 가 잎·파일·`robot.yaml` 비적용을 핀) · ω 10 · 복제 규율. S8-B~S8-F-1 의 수치는 a_max 21 · box 2.03 시절의 것이다. **PR #584 리뷰 반영 (2026-09-27)**: `sim.yaml` override 는 overlay 아래에 깔려 box 를 명시하지 않는 모든 sim overlay 에 미치므로 S8-G 격자 여섯 cell 은 측정 당시의 출하 box 를 핀해 재현성 (R4) 을 지킨다 (`s8f_*`·`catch_lead_*` 는 핀 없음 — 지금 띄우면 D-S8-18 구성); 미러는 TBD 잎을 컨트롤러가 *실행한* 기본값으로 내고 (`a_max`/`v_max` struct default, η_v 0.9) `planner.time.margin` 을 추가; 도구는 미러 부재 시 컨트롤러 YAML → `sim.yaml` → overlay 순으로 합성하고 box 초과를 관절별로 센다.
...
- ⚠️ **S4.4 판정 (2026-09-22): "2~4 배" 가 아니다.** 포구 자세에서 공 진행 방향으로 낼 수 있는 가속을 토크 한계로 직접 풀면 (`catch_speed_budget`, $|M\ddot q+h|\le\eta_\tau\tau_{\max}$, 접근축 유지, 회전자 관성 포함) 중앙값이 `ur5e_p1b` 43 · `iiwa7_leap` 79 m/s² 인데, 이 box 가 같은 자세에서 주는 값은 0.74 · 6.8 m/s² 다 — **약 50 배 · 10 배.** 회전자 관성을 MJCF 값의 10 배로 잡아도 21–23 · 35–36 m/s² 다. 이 box 를 쓰면 `ur5e_p1b` 는 어떤 투척 조건에서도 γ 창이 열리지 않는다 (§4.4 S4.4 결과)
  ...
  - **S8-G 사전 분석 (2026-09-26, #537 5846659919 — S8-F-1 9 unit·1068 발 오프라인 재분석, `rtc_tools catching_arm_budget`).** 결정 K (S6-C2) 로 CLIK 은 `accel_constraint: dynamic` (토크) 을 쓰는데 **계획기 도달시간 (L3 §4.3) 만 이 2.03 box 로 판정한다** — 실행된 관절 가속은 활성 tick 의 p95 14–37 rad/s² (관절별; max 76–250) 로 box 를 tick 의 99.8 % 에서 넘고, 같은 운동을 box 로 재면 도달시간 0.60–0.89 s (가용 lead 0.29–0.52 s) → `rank_reach` 93–100 % 실패, envelope 로 재면 0.15–0.25 s → 96 % 통과. 플랜트 토크 사용률은 max 0.61–0.83 · p99 ≤ 0.64 로 forcerange 에 닿은 tick 0. 즉 "계획기와 CLIK 이 같은 한계" 원칙은 결정 K 로 이미 깨졌고, 남은 box 는 실행을 묶지 않으면서 순위 gate 와 `w_t` 항만 무의미하게 한다. **S8-G 실험 (AC-3)**: sim overlay `s8g_w10_a21_envbox` 가 `robot.arm.accel_limits_path` 를 `derived_accel_limits_s8g_envelope.yaml` (도구 `--write-envelope-box`: unit 별 p95 의 관절별 max = [20.3, 30.9, 37.0, 21.5, 14.2, 29.0] rad/s², `provisional`, sim 전용) 로 돌려 같은 56 발에서 `rank_reach`·plan 교체·포획을 비교한다. 출하 `derived_accel_limits.yaml` 은 그대로다. **원칙 채택 (2026-09-27 D-S8-18)**: 계획기 도달시간 한계 = 실행이 실제로 내는 envelope — sim 은 `sim.yaml` 의 `robot.arm.accel_limits_path` override 로 envelope 파일을 쓰고 (`rank_reach` 95 → 14 %), `robot.yaml` 은 토크 도출 box 를 유지한다 (실기 envelope 은 S10 측정 뒤)
