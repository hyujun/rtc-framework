...
- 상태: **S0 완료**, **S1.1~S1.8 완료** (2026-09-19, PR #541), **S2 완료** (2026-09-20, PR #545~#549; **S2.3b 2026-09-21**), **S1.9 완료** (2026-09-20), **S3a 완료** (2026-09-20, PR #553), **S4a 완료** (2026-09-21, 브랜치 `feat/s4a-hand-timing` — S4.3 실기는 S10). **S3.5a 완료** (2026-09-21 — 도구 + 두 로봇 지도, PASS(provisional); §11 지도 결과). **S3.5b 완료** (2026-09-22 — `ur5e_p1b` PASS(provisional), `iiwa7_leap` 지도 빔; §4.4 S3.5b 결과). **T_det 실측 완료** (2026-09-22 — 24 비행 × 2 run; 첫 run 이 sim 공 lane 의 stamp 지터 결함을 드러내 **같은 날 고쳤다** (stamp = throttle 기준 wall 순간, `rtc_mujoco_sim`) — 재실측 stamp 축 p50 0.050 · max 0.080 s, 첫 계획 시각 max 0.22 s 로 전 비행이 지도 가정 안; `iiwa7_leap` G8-D2 평가 대상 — §4.4 T_det 실측·재실측). **S3.6 완료** (2026-09-22 — vision 요구 사양 provisional: **기구학 reachable 창 기준** (D-27) + **T_det 재실측** H_req **0.99 s** · 간격 0.05 s · `n_max` **20** ≤ `kCap` 40 (S1.2 backfill PASS) · `io.horizon_min` 0.51 s; sim profile **1.0 s / 20 점 설정** (`integrated_bringup/config/ur5e_p1b/ball_perception_sim_profile.json`, 사용자 결정); §4.4 S3.6 결과·T_det 재실측). **S5 앞 결정 6건 확정** (2026-09-22 사용자 — E-8 승인 · D-24 (a) · G5-E fixture 주입 · QP solve time 예산 · 공 lane stamp 수정 · sim profile; §7.3). **S4.4 완료** (2026-09-22 — **조건부 go**: 현 sim 씬·D-18 투척에서는 두 로봇 모두 γ 창이 비고, 낙차·가속 한계·선행시간 세 조건 아래에서만 열린다; D-16·D-18 개정은 `[제안]`; §4.4 S4.4 결과·§9·§11). S0 은 S0.1~S0.9 게이트 PASS. S0.7 이 0.5 s profile 부족을 보고해 sim profile 지평을 0.8 s 로 정했고 (D-15), 지평 요구는 R1·대기 자세는 겨냥점 근처·`kCap` 40 으로 정했다 (§7.1). 남은 승인은 S5·S6 착수 전 E-8·E-7. 승인이 막는 단계는 승인 전에 착수하지 않는다 (§4.1)
...
| S3.6 vision 사양 | 완료 (2026-09-22) — **PASS(provisional)**: `ur5e_p1b` **기구학 reachable 창** (D-27) 에서 H_req **0.99 s** (T_det **재실측** min 0.040 s — 공 lane stamp 수정 후), 간격 0.05 s, n 20 → `n_max` 20, sim profile **1.0 s / 20 점 설정** (gate 통과 창만 보면 0.84 s · n 17), `io.horizon_min` 0.51 s (R1, 올림) · `io.n_min` 11 · `prediction.dt_expected` 0.05. `io.t_stale`·`io.future_tol` 은 [제안]. `iiwa7_leap` 은 실측 선행시간에서 지도가 열려 G8-D2 평가 대상. T_det 실측·재실측 완료 (§4.4) | — §4.4 S3.6 결과·T_det 재실측 |
...
| d_eff·r_cap | G6-F: **LEAP·P1b PASS(provisional)** — 산정식·실측값·provisional 표시가 `planner.hand.*` 와 L6 §4.5 에 기록, D-3 갱신. P1b 는 탐색한 자세 기준 (2026-09-21). 투척 보정은 `NOT_EVALUATED(S7.1 후)` | 사용자 승인 2026-09-20 (LEAP) · 2026-09-21 (P1b 탐색 자세 적용) |
| GUI·plot | §13 S4 행 | — |
...
- **sim profile (D-15, 설정은 사용자): 1.05 s / 0.05 s / 21 점 / ≤ 30 Hz 를 권장한다** (현 설정은 0.8 s / 16 점). 0.8 s 를 유지하면 계획기가 보는 창 끝은 0.78 s 까지이므로 (지평 − `t_horizon_margin`), **기구학 창의 끝 (0.95 s) 쪽 후보 = 늦게 잡는 자세는 시도 대상에서 조용히 빠진다** — D-27 이 피하려는 바로 그 손실이다. 비용은 예측점 5 개 (스냅샷 복사 약 +1.0 KB/tick, `kCap` 40 안). H_req 는 T_det 에 1:1 로 민감하다 ($H_{req} = 1.028 - T_{det}$). 가장 이른 검출 (0.024 s) 기준으로 지평 H 가 덮는 t_c 는 $0.024 + H - 0.08$ 이므로 **0.95 s / 19 점은 창 끝 0.948 s 에 54 ms, 1.0 s / 20 점은 4 ms 모자라고 1.05 s / 21 점이 처음으로 덮는다**. 균일한 실기 카메라 (100 Hz) 라면 가장 이른 검출이 ≈ 0.045 s 라 1.0 s / 20 점으로 충분하다 — 1.05 s 는 sim 의 버스트까지 덮는 값이다. `sim_profile.json` 의 `prediction` 블록: `{"horizon_s": 1.05, "step_s": 0.05, "max_points": 21}` (ball_perception 은 `horizon % step == 0` 을 요구하고 producer 상한 `kMaxPredictionPoints` 는 1000 이다), `debug.max_publish_rate_hz` 30.0 (S3.4 가 이 값으로 실측했다 — `prediction/trajectory` 만 켜면 발행 0건인 결함은 §4.4 S3a 결과). 확인 CLI: `ros2 run rtc_tools vision_lane_probe <prefix>` 로 비행 몇 건을 기록한 뒤 `ros2 run rtc_tools analyze_vision_lane <prefix>` 의 발행 주기·N·지평 행이 30 Hz · 21 · 0.05…1.05 s 인지 본다. **0.95 s / 19 점은 2026-09-22 에 실기 rig 로 확인했다** — 정확히 19 점 · 지평 0.05…0.95 s · VALID 944 건 · 부분 무효 0 (아래 "T_det 실측").
...
**T_det 재실측 (2026-09-22, 공 lane stamp 수정 후) `[sim 실측 — L 은 여전히 가정값]`.** 같은 rig·같은 투척 24 발, profile **1.0 s / 0.05 s / 20 점** (설정된 파일). 공 lane stamp 간격: **p05 10.00 / p50 10.00 / p95 10.00 / max 30 ms, 25 ms 초과 3 / 9380 (0.03 %)** — 수정 전 9.2 % 였다. 수신 간격은 그대로 p05 2.7 / p95 34 ms 로 버스트가 남아 있다 — 지터가 stamp 에서 빠졌을 뿐 stepper 는 그대로다. 초과 3 건은 정확히 30 ms 로, 버스트 안에서 프로브의 `KEEP_LAST(1)` 이 놓친 샘플이다.

| 축 | min | p05 | p50 | p95 | max |
|---|---|---|---|---|---|
| `recv` (소비자 체감) | 0.041 | 0.048 | 0.064 | 0.103 | 0.113 |
| `stamp` (발행자, H_req 입력) | **0.040** | 0.050 | 0.050 | 0.070 | 0.080 |

- **24 투척 전부 검출.** 분석기가 25 번째 "비행" 하나를 미검출로 보고하는데, 그것은 던진 공이 아니라 착지 후 바닥에 멈춘 공의 36 샘플 (z 0.033 m, 0.157 s) 이 truth lane 의 0.33 s 정지 (`--flight-gap-s` 0.3 초과) 로 잘려 나온 조각이다 — 멈춘 공에 VALID 예측이 없는 것이 맞다.
- **꼬리가 사라졌다**: `stamp` max 0.690 → **0.080 s**, `recv` max 0.729 → 0.113 s. 첫 VALID 앞의 예측 수는 0 또는 1 (수정 전 최대 16). 중앙값 0.050 s 는 estimator 초기화 창 (5 점 × 10 ms) 그대로다.
- **첫 계획 시각 T_det + L 0.14 는 min 0.180 · p50 0.190 · p95 0.210 · max 0.220 s** — 24 비행 전부가 지도의 가정 0.24 s 안이다 (수정 전에는 54 %). 위 민감도 표로 읽으면 `ur5e_p1b` 는 모든 비행에서 열린 투척 ≥ 652 (0.215 s 기준), `iiwa7_leap` 은 0.19 s 에서 27 · 0.215 s 에서 11 로 **G8-D2 평가 대상이 모든 비행에서 유지**된다.
- **H_req 가 한 점 내려왔다**: 가장 이른 검출이 0.024 → 0.040 s 라 0.988 s · n 20 (위 "산식과 값"). profile 1.0 s / 20 점은 이 run 이 그대로 썼다 — 20 점 · 0.05…1.0 s · VALID 2355 · 부분 무효 0.
...
| PROC-3 | S5.4 및 S5.2e (D-24 (a) 확정) 의 `rtc_msgs`·`rtc_base` 변경 후 전체 빌드·테스트 | — |
| GUI·plot | §13 S5 행 | — |
...
| ~~sim 공 lane 지터~~ | **확정 (2026-09-22 사용자) · 구현 완료**: 두 공 토픽의 stamp 를 **throttle 이 sim 시각을 대응시키는 wall 순간**으로 (`rtc_mujoco_sim`, epoch 은 wall 그대로 — D-2·D-3 불변). 발행률 상향 (속도 추정을 더 튀게 함) · wall 타이머 (`mjData` 잠금) · `init.max_span_s` 완화 (증상만 덮음) 는 기각. 재실측: stamp 간격 25 ms 초과 9.2 % → 0.03 %, T_det max 0.69 → 0.08 s (§4.4 T_det 재실측) | ~~S7–S8 전~~ |
| ~~sim profile 설정~~ | **확정·설정 (2026-09-22 사용자, ⑤ 후 한 번)**: `integrated_bringup/config/ur5e_p1b/ball_perception_sim_profile.json` = **1.0 s / 0.05 s / 20 점 / ≤ 30 Hz**, 측정 공분산 (5 mm)² 대각 (`position_noise_stddev_m` 과 짝). 수정 전 필요했던 1.05 s / 21 점은 버스트가 만든 0.024 s 검출 때문이었다 (§4.4 S3.6 결과) | ~~S7–S8 전~~ |
...
| D-12 나머지 | 공 사양, 실기 T_close,tot 측정 시점, 성공률 floor·시행 수. S4.4 는 출하 sim 공 (ITF Type 2) 으로 판정했다 — 공 사양이 바뀌면 fly-in (L6 §4.5) 부터 다시 | S3.5b, S8 |
...
| ~~d_eff · r_cap~~ | 닫힘 (2026-09-20 사용자 승인): LEAP `d_eff` 0.080 m · `r_cap` 0.031 m (접촉 sim, provisional). P1b 는 사용자 제공 자세로 파지 불가여서 자세를 탐색으로 다시 정한 뒤 (2026-09-21) `d_eff` ≥ 0.095 m (스캔 상한) · `r_cap` 0.024 m. 투척 보정은 S7.1 후 | ~~S4.4~~ |
