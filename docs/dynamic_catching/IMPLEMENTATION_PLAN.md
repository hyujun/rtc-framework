...
| S1 순수 수치 코어 | S1.1~S1.8 완료 (2026-09-19, PR #541), S1.9 는 S2.1 후 | 이식·회귀·RT·시간 PASS, 검증기 PASS, S1.8 PASS (G7-C 임계 NOT_EVALUATED), backfill **PASS (2026-09-22, S3.6: `n_max` 19 ≤ `kCap` 40)** — §4.4 S1 결과 |
...
| S3.6 vision 사양 | 완료 (2026-09-22) — **PASS(provisional)**: `ur5e_p1b` **기구학 reachable 창** (D-27) 에서 H_req 0.93 s (T_det 0.10 가정), 간격 0.05 s, n 19 → `n_max` 19, sim profile **0.95 s / 19 점 권장** (gate 통과 창만 보면 0.71 s · n 15 · 0.8 s profile), `io.horizon_min` 0.51 s (R1, 올림) · `io.n_min` 11 · `prediction.dt_expected` 0.05. `io.t_stale`·`io.future_tol` 은 [제안]. `iiwa7_leap` 은 `NOT_EVALUATED(선행시간)`. 선행시간 T_det 실측은 병행 task (§7.3) | — §4.4 S3.6 결과 |
...
| backfill | **PASS** (2026-09-22, S3.6: `n_max` 19 ≤ `kCap` 40 — §4.4 S3.6 결과) | — |
...
- **산식과 값.** 검출 시각 T_det 0.10 s (S0.7 가정값) 의 첫 예측이 포구 창 끝까지 보여야 첫 plan 부터 그 후보를 고를 수 있다: $H_{req} = \max t_c - T_{det} + t_{horizon\_margin} + T_{margin}$ (= `prediction.t_horizon_margin` 0.05, L2 §4.6 — 계획기는 마지막 점 − 0.05 s 안에서만 고른다; + `planner.time.margin` 0.03). **기구학 창 (요구)**: 0.948 − 0.10 + 0.08 = **0.928 s** → **n = ⌈0.928 / 0.05⌉ = 19** (상자만이면 0.856 s · n 18). **gate 통과 창 (하한·참고)**: 0.732 − 0.10 + 0.08 = 0.712 s → n 15. 간격은 S1.2 실측 (0.05 s 간격 보간 오차 2.0e-11 m ≪ G2-C 1e-10 m, §4.4 S1 결과) 으로 **0.05 s** 를 유지한다 — 더 촘촘하면 점 수만 는다. ball_perception 은 `horizon % step == 0`·`horizon / step ≤ max_points` 를 강제하고 producer 상한은 1000 점이라 사양을 구속하지 않는다.
- **sim profile (D-15, 설정은 사용자): 0.95 s / 0.05 s / 19 점 / ≤ 30 Hz 를 권장한다** (현 설정은 0.8 s / 16 점). 0.8 s 를 유지하면 계획기가 보는 창 끝은 0.78 s 까지이므로 (지평 − `t_horizon_margin`), **기구학 창의 끝 (0.95 s) 쪽 후보 = 늦게 잡는 자세는 시도 대상에서 조용히 빠진다** — D-27 이 피하려는 바로 그 손실이다. 비용은 예측점 3 개 (스냅샷 복사 약 +0.6 KB/tick, `kCap` 40 안). H_req 는 T_det 에 1:1 로 민감하다 ($H_{req} = 1.028 - T_{det}$) — T_det 실측 (§7.3 병행 task) 이 0.10 s 보다 작게 나오면 그만큼 더 필요하고, 0.178 s 보다 크면 0.85 s / 17 점으로 줄어든다. `sim_profile.json` 의 `prediction` 블록: `{"horizon_s": 0.95, "step_s": 0.05, "max_points": 19}` (ball_perception 은 `horizon % step == 0` 을 요구한다), `debug.max_publish_rate_hz` 30.0 (S3.4 가 이 값으로 실측했다 — `prediction/trajectory` 만 켜면 발행 0건인 결함은 §4.4 S3a 결과). 확인 CLI: `ros2 run rtc_tools vision_lane_probe <prefix>` 로 비행 몇 건을 기록한 뒤 `ros2 run rtc_tools analyze_vision_lane <prefix>` 의 발행 주기·N·지평 행이 30 Hz · 19 · 0.05…0.95 s 인지 본다.
- **런타임 값 (provisional — 기록처는 L1 §6 · L2 §6).** `io.horizon_min` = R1 하한 $T_{freeze} + L$ = ($T_{close,tot}$ 0.2815 + T_arm 0.05 + T_margin 0.03) + L 0.14 = 0.5015 → **0.51 s** (10 ms 로 **올림** — 내림하면 R1 을 1.5 ms 미달하는 궤적이 통과한다; L 0.14 s 는 S0.7 가정값); `io.n_min` = ⌈0.51 / 0.05⌉ = **11** (0.05 s 간격에서 0.55 s); `prediction.dt_expected` **0.05 s**; **`n_max` = 19** — 권장 profile 의 점 수와 같게 둔다 (런타임 상한이 profile 을 거부하면 안 된다). 상한이므로 현 0.8 s / 16 점 profile 도 그대로 통과한다. **19 ≤ `kCap` 40 → S1.2 backfill PASS** (§4.3 S1 행). `io.t_stale` **[제안] 0.10 s**·`io.future_tol` **[제안] 1e-3 s** 는 S3.4 실측 (발행 30 Hz, 드롭 30 % 에서 p95 15 Hz, 유령 트랙 침묵 ≤ 34 ms 뒤 정지, 같은 호스트 wall clock) 기반의 제안이고 **확정은 S5.2** (L1 §6 근거 칸).
- **한계.** ① T_det·L 은 가정값 — H_req 와 `io.horizon_min` 둘 다 이에 걸려 있다 (T_det 실측 후 재계산, L 은 S5–S6 후). ② 목표 분포 자체가 provisional 셋 (fly-in `d_eff`·토크 층·다시 고른 대기 자세) 위의 값이다 (§4.4 S3.5b 결과). ③ R2 (정지 출발) 는 요구가 아니다 — 대기 자세가 상자 기준 투척의 q\* 라 첫 이동은 작다. ④ `iiwa7_leap` 은 지도가 비어 `NOT_EVALUATED(선행시간)` — 실측 선행시간에서 지도가 열리면 같은 요약 블록으로 H_req 를 다시 읽는다 (G8-D2 조건부, §1a). ⑤ 요약이 주는 것은 분포뿐이고 H_req 의 덧셈은 이 문서가 한다 — 도구에 T_det·margin 을 넣지 않은 것은 두 값이 이 문서의 결정이기 때문이다. ⑥ 기구학 창의 끝은 **이 격자·이 대기 자세·`max_reach` 1.1 m·`min_catch_height` 0.1 m** 에서의 값이다 (지도 창 0.3–1.4 s 안이라 잘리지 않았다) — 투척 격자를 넓히면 다시 읽는다. ⑦ D-27 의 "성능 게이트를 우선순위로" 는 런타임 구현이 없다 (S6) — 지금 바뀐 것은 **지평 요구를 어느 창에서 읽는가** 뿐이고, 어느 게이트가 판정으로 남는지는 S6 설계에서 정한다.
...
| 선행시간 T_det·L | S0.7 가정값 (0.10 · 0.14 s). `iiwa7_leap` 은 0.24 s 에서 0, 0.14 s 에서 65 / 2835. **결정 (2026-09-22 사용자): S3.6 을 막지 않는다** — `ur5e_p1b` 는 여유 중앙값 0.20 s 라 둔감. **T_det 만 지금 실측 가능** (S3.4 rig: 발사 stamp → 첫 유효 예측 수신), L 은 계획기가 있는 S5–S6 후에야 값이 있다. 별도 task 로 병행하고, 실측이 0.14 s 근처면 `iiwa7_leap` 지도를 `--first-plan-s` 만 바꿔 다시 돌려 S3.6 에 추가한다 (G8-D2 조건부, §1a) | S3.6 병행 (T_det) · S6 후 (L) |
...
| vision 지평이 짧으면 계획 가능한 포구 창이 준다 — S0.7 로 sim profile 을 0.8 s 로 올렸고 (D-15) 지평 요구는 연속 재계획 기준 R1 이다. 첫 목표가 작업공간 가장자리라 그 후보가 전멸하면 정지 출발(R2, 먼 q* 는 약 1.0 s)로 떨어진다. 지평을 늘릴 주체는 외부 repo 이고 요청 채널이 없다 (P-3). S3.6: gate 통과 창은 끝이 0.73 s 라 0.8 s profile 안이지만 **기구학 reachable 창은 0.95 s** 라 D-27 기준으로는 0.95 s / 19 점이 필요하다 — 0.8 s 를 유지하면 늦게 잡는 후보가 계획기에 도달하지 못한다 | S3.5a 가장자리 catchability, S8 첫 plan 측정, S3.6 |
