...
- 상태: **S0 완료**, **S1.1~S1.8 완료** (2026-09-19, PR #541), **S2 완료** (2026-09-20, PR #545~#549; **S2.3b 2026-09-21**), **S1.9 완료** (2026-09-20), **S3a 완료** (2026-09-20, PR #553), **S4a 완료** (2026-09-21, 브랜치 `feat/s4a-hand-timing` — S4.3 실기는 S10). **S3.5a 완료** (2026-09-21 — 도구 + 두 로봇 지도, PASS(provisional); §11 지도 결과). **S3.5b 완료** (2026-09-22 — `ur5e_p1b` PASS(provisional), `iiwa7_leap` 지도 빔; §4.4 S3.5b 결과). **T_det 실측 완료** (2026-09-22 — 24 비행, stamp 축 p50 0.075 s; S3.6 수치 개정, `iiwa7_leap` G8-D2 평가 대상 진입, sim 공 lane 지터 결함 보고 — §4.4 T_det 실측). **S3.6 완료** (2026-09-22 — vision 요구 사양 provisional: **기구학 reachable 창 기준** (D-27) + **T_det 실측** H_req **1.00 s** · 간격 0.05 s · `n_max` **21** ≤ `kCap` 40 (S1.2 backfill PASS) · `io.horizon_min` 0.51 s; sim profile **1.05 s / 21 점 권장** — 현 0.8 s / 16 점은 창 끝 0.78 s 까지만 담는다; §4.4 S3.6 결과·T_det 실측). **S4.4 완료** (2026-09-22 — **조건부 go**: 현 sim 씬·D-18 투척에서는 두 로봇 모두 γ 창이 비고, 낙차·가속 한계·선행시간 세 조건 아래에서만 열린다; D-16·D-18 개정은 `[제안]`; §4.4 S4.4 결과·§9·§11). S0 은 S0.1~S0.9 게이트 PASS. S0.7 이 0.5 s profile 부족을 보고해 sim profile 지평을 0.8 s 로 정했고 (D-15), 지평 요구는 R1·대기 자세는 겨냥점 근처·`kCap` 40 으로 정했다 (§7.1). 남은 승인은 S5·S6 착수 전 E-8·E-7. 승인이 막는 단계는 승인 전에 착수하지 않는다 (§4.1)
...
| S3.6 vision 사양 | 완료 (2026-09-22) — **PASS(provisional)**: `ur5e_p1b` **기구학 reachable 창** (D-27) 에서 H_req **1.00 s** (T_det **실측** p05 0.027 s), 간격 0.05 s, n 21 → `n_max` 21, sim profile **1.05 s / 21 점 권장** (gate 통과 창만 보면 0.86 s · n 18), `io.horizon_min` 0.51 s (R1, 올림) · `io.n_min` 11 · `prediction.dt_expected` 0.05. `io.t_stale`·`io.future_tol` 은 [제안]. `iiwa7_leap` 은 `NOT_EVALUATED(선행시간)`. 선행시간 T_det 실측은 병행 task (§7.3) | — §4.4 S3.6 결과 |
...
| backfill | **PASS** (2026-09-22, S3.6: `n_max` 21 ≤ `kCap` 40 — §4.4 S3.6 결과) | — |
...
- **산식과 값.** 검출 시각 T_det 0.10 s (S0.7 가정값) 의 첫 예측이 포구 창 끝까지 보여야 첫 plan 부터 그 후보를 고를 수 있다: $H_{req} = \max t_c - T_{det} + t_{horizon\_margin} + T_{margin}$ (= `prediction.t_horizon_margin` 0.05, L2 §4.6 — 계획기는 마지막 점 − 0.05 s 안에서만 고른다; + `planner.time.margin` 0.03). **기구학 창 (요구)**: T_det 는 **실측값**을 쓴다 (아래 "T_det 실측") — 구속하는 것은 **가장 이른 검출**이다 (검출이 이를수록 첫 예측의 origin 이 이르고 그만큼 더 멀리 봐야 한다): 0.948 − 0.027 (p05) + 0.08 = **1.001 s** → **n = ⌈1.001 / 0.05⌉ = 21** (90 % 상자만이면 0.929 s · n 19). **gate 통과 창 (하한·참고)**: 0.804 − 0.027 + 0.08 = 0.857 s → n 18. 종전 기록 (가정 T_det 0.10 s) 은 0.928 s · n 19 였다 — 실측이 더 이르게 검출해 요구가 **올랐다**. 간격은 S1.2 실측 (0.05 s 간격 보간 오차 2.0e-11 m ≪ G2-C 1e-10 m, §4.4 S1 결과) 으로 **0.05 s** 를 유지한다 — 더 촘촘하면 점 수만 는다. ball_perception 은 `horizon % step == 0`·`horizon / step ≤ max_points` 를 강제하고 producer 상한은 1000 점이라 사양을 구속하지 않는다.
...
- ⚠️ **꼬리가 목표 분포를 지운다.** 측정된 24 비행의 첫 계획 시각은 0.164–0.830 s 인데, **0.30 s 를 넘는 비행이 33 % · 0.45 s (지도가 비는 값) 를 넘는 비행이 8 %** 다. 즉 위 rig 결함은 "가끔 늦게 잡는다" 가 아니라 **그 시행에서 계획 가능한 후보가 아예 없다**는 뜻이고, D-27 (reachable 하면 도전) 도 후보가 계획기에 도달해야 성립한다. S7–S8 착수 전에 대응이 필요하다.
...
- D-15 sim profile 지평: 0.8 s, 간격 0.05 s, **16 점** (지평 0.05…0.80 s, t = 0 없음 — 2026-09-20 정정), ≤ 30 Hz (2026-09-19, S0.7 권장 채택). ball_perception sim profile 설정은 사용자가 하고, S3.4 가 실측으로 확인한다. S3.6 (2026-09-22) 이 목표 분포에서 같은 profile 을 다시 확인했다 (§4.4 S3.6 결과)
