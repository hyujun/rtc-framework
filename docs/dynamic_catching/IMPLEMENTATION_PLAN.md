...
- 상태: **S0 완료** (2026-09-19) — S0.1~S0.9 게이트 PASS. S0.7 이 0.5 s profile 부족을 보고해 sim profile 지평을 0.8 s 로 정했다 (D-15). R2 채택은 §7.3 결정 대기다. 남은 승인은 S5·S6 착수 전 E-8·E-7. 승인이 막는 단계는 승인 전에 착수하지 않는다 (§4.1)
...
- **권장 → 채택 (2026-09-19)**: sim profile 지평 0.8 s (17 점 @ 0.05 s, 설정은 사용자 — D-15). 0.8 s 는 R2 를 T_move + T_arm ≤ 0.64 s 까지 덮는다 — T_move 0.45 s 전부, T_move 0.60 s 는 T_arm ≤ 0.04 s 일 때. 먼 q* (H ≈ 1.0 s) 는 덮지 못하므로 대기 자세를 겨냥점 가까이 두어 T_move 를 줄이는 권장은 남는다 (§7.3). R2 채택은 §7.3 결정 대기
...
| 대기 자세 | 지평 0.8 s (D-15, 2026-09-19 결정) 로 덮지 못하는 먼 q* 를 줄이려 대기 자세를 겨냥점 가까이 둘지 (S0.7) | S3.5a |
...
| vision 지평이 짧으면 계획 가능한 포구 창이 준다 — S0.7 이 예시 profile 0.5 s 가 팔 이동 조건(R2, 0.46–0.86 s)에 부족함을 확인해 sim profile 을 0.8 s 로 정했다 (D-15). 먼 q* 는 여전히 넘는다. 지평을 늘릴 주체는 외부 repo 이고 요청 채널이 없다 (P-3) | S3.4 실측, S3.6 |
