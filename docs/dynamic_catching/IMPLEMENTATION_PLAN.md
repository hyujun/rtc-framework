...
|---|---|---|---|
| `ur5e_p1b` | 0.095 m / 0.2825 s | 0.948 → 0.960 | **6.2 → 8.0 m/s** |
| `iiwa7_leap` | 0.080 m / 0.1057 s | 0.884 → 0.909 | **5.7 → 7.6 m/s** |

출하 설정의 $\eta_vv_{\max}$ 는 두 로봇 모두 **1.50 m/s** 다 ($\Vert v\Vert_{\max}$ 에서 역산; `reference.v_max` 자체는 L4 에서 아직 TBD). 따라서 $\Vert v\Vert$ 7.4 m/s 에서 $\gamma_{\max}\approx0.20$ 이고 $\gamma_{\min}\approx0.95$ 다 — **창이 약 4.7 배로 비어 있다.** 창이 열리는 조건은

$$\min(v_{dir,\max},\ \eta_vv_{\max})\ \ge\ \Vert v\Vert-\frac{d_{eff}}{T_{close,tot}}\ =\ 7.05\ (\texttt{ur5e\_p1b})\ /\ 6.63\ (\texttt{iiwa7\_leap})\ \text{m/s}$$

이고, UR5e 의 정격 TCP 속도는 그 자릿수가 아니다. 즉 **방향은 맞지만 4 m·$T_f\ge1.0$ s 에서는 그것만으로 닫히지 않는다.** γ 는 느린 공에서 포구 가능 집합을 넓히고 그 손익분기가 정확히 $\Vert v\Vert_{\max}$ 인데, 이 격자의 공은 그 값의 3.5–4.5 배로 도착한다.

그래서 S4.4 는 **두 변수의 협상**으로 둔다 — 팔이 주는 γ 와 투척이 주는 $\Vert v\Vert$. 그리고 $\gamma_{\max}$ 가 `reference.v_max` 위에 세워져 있으므로 **그 값을 재는 것이 이 방향의 선행 조건**이다 (L4 에서 TBD, TBD-ARM-02). 여기서 $v_{dir,\max}$ (포구 자세에서 $\hat v$ 방향으로 낼 수 있는 최대 속력) 도 자세마다 다르므로, 지도 도구가 후보별로 그것을 함께 내도록 확장하는 것이 S3.5b 의 자연스러운 다음 걸음이다 — 지금 지도는 `q*` 를 내므로 그 계산의 입력은 이미 있다.
