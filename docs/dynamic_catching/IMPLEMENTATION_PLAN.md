...
| S3.6 vision 사양 | 대기 — S3.5b 의 사용자 결정 (§7.3: `d_eff` 의 뜻 · `a_dec` · 1차 로봇 · 선행시간 실측) 후 | — |
...
- **1차 목표 로봇 (결정 C 의 입력).** 이 결과로는 `ur5e_p1b` 만 목표 투척 분포를 가진다. `iiwa7_leap` 은 선행시간 실측 (T_det·L) 이 0.14 s 근처로 나오거나 q\* 의 seed 의존을 줄이지 않는 한 비어 있다. **확정은 사용자** (§7.3).
- S3.6 은 시작하지 않았다 — `[SPRINT]` 3 의 정지 조건 (`iiwa7_leap` 지도가 빔). 원자료와 러너 (`run_gate.sh`) 는 repo 에 두지 않는다; 방법은 두 도구의 인자가 SSoT 다.
...
| 1차 목표 로봇 | S4.4 권장 철회 (2026-09-22 사용자, 결정 C). **S3.5b 결과: 목표 투척 분포를 가진 것은 `ur5e_p1b` 뿐이다** (`iiwa7_leap` 은 선행시간 실측이 0.14 s 근처로 나오지 않는 한 빔) — §4.4 S3.5b 결과. Epic 기준 1 의 G8-D2 (`iiwa7_leap`) 를 어떻게 둘지와 함께 정한다 | S5 전 |
...
| `planner.hand.d_eff` 의 뜻 (TBD-HAND-04) | 출하값 (포켓 깊이 0.095 / 0.080 m) 으로는 `ur5e_p1b` gate 지도가 **비어 있다**. 열린 것은 전부 fly-in 실측 상대속도 1.0 m/s 위에 있다 (등가 $d_{eff}=v_{rel}T_{close,tot}$ = 0.2815 / 0.1047 m). 실측을 런타임 γ 창에 싣려면 `d_eff` 를 그 등가값으로 정의해야 한다 — §4.4 S3.5b 결과, L6 §4.5 | S3.6 전 |
| `supervisor.decel.a_dec` | TBD. S3.5b 는 10 m/s² (provisional) 로 돌렸고 정지점 게이트가 단독으로 후보의 40 % 를 거른다 | S3.6 전 |
| 선행시간 T_det·L | S0.7 가정값 (0.10 · 0.14 s). `iiwa7_leap` 은 0.24 s 에서 0, 0.14 s 에서 65 / 2835 — sim 실측이 필요하다 (S3.4 rig) | S3.6 전 |
