...
2. 실험 보정: 시뮬레이션과 실기에서 저속 투척으로 "폐쇄 늦음" 경계를 찾는다. **S7.1 (손 시퀀서) 이후에 한다** — S7.1 완료 (2026-09-24) 로 선행조건은 풀렸다. **sim 입력 (S8-F-1, 2026-09-26, plan §4.4 S8-F-1 결과)**: 정면 입사·r 0 에서 tennis v50 **3.6 m/s** [3.0, 4.0] (절벽 3.5 → 3/8, 4.0 → 0/8), 포획은 접촉 상대속도 ≲ 1.4 m/s 에서만 (접촉 v_rel 로지스틱 σ(0.33 − 1.16 v_rel): 1.0 → 0.30, 2.0 → 0.12) — 위 시각 발동 fly-in 의 1.0–1.25 와 같은 자리이고, 85° 급강하 lob 2.5 m/s 도 v_rel 1.44 에서 3/14; beanbag 은 v_rel 2.25 에서도 4/8 이라 tennis 경계의 절반 이상은 반발이다. 폐루프에서는 팔이 γ 를 v_max/v 로 낮춰 v_rel 이 속력과 함께 오르므로 "폐쇄 늦음" 이 아니라 상대속도가 경계를 정한다 (실기는 S10) — 시퀀서 없이 비-RT 러너의 지연으로 폐쇄 시각을 맞추면 지터 × 공 속력이 $d_{eff}$ 와 같은 자릿수다 (plan §4.4 S4a).
...
- TBD-HAND-03 지문 잡음 (실기 σ) — 부호·frame 은 닫힘 (S7.3 은 $\Vert F-b\Vert$ 만 판정, sim 잡음 0 이라 `f_min` 만 유효). **sim 에서는 `NOT_EVALUATED(sim 무잡음)`** (2026-09-24 S8 준비) — sim 지문 lane 은 잡음이 0 이라 바이어스 추정·잡음 문턱을 검증할 수 없다. 값은 S10 실측
