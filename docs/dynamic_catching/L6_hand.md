...
2. 실험 보정: 시뮬레이션과 실기에서 저속 투척으로 "폐쇄 늦음" 경계를 찾는다. **S7.1 (손 시퀀서) 이후에 한다** — S7.1 완료 (2026-09-24) 로 선행조건은 풀렸고 아직 하지 않았다 (sim 은 S8 — 손 근처 투척 탐색 S8-F 의 속력 절벽·v50(r) 지도가 이 경계의 입력이다 (plan §4.4 S8), 실기는 S10) — 시퀀서 없이 비-RT 러너의 지연으로 폐쇄 시각을 맞추면 지터 × 공 속력이 $d_{eff}$ 와 같은 자릿수다 (plan §4.4 S4a).
