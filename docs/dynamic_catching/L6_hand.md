...
P1b 는 같은 식으로 $1.5+0.095/(0.2805+0.001)=$ **1.84 m/s** 인데, 이것은 **공식값**이다 — 위 fly-in 표가 말하듯 이 손이 실제로 받은 속력은 그보다 훨씬 낮다. LEAP 을 $\eta=0.9$ 로 잡으면 1.85 m/s 다. $d_{eff}$ 가 §4.1 예시의 4 cm 보다 두 배 커서 상한이 예시의 2.17 m/s 와 같은 자리로 올라왔지만, S0.7 가정값 8.4 m/s 와는 여전히 3.7 배 차이다 — S4.4 는 목표 속도를 낮춘다.
...
- **P1b 의 폐쇄 속도** — 자세는 2026-09-21 에 다시 정했지만 (§4.5) 날아드는 공은 0.5 m/s 부터 거의 못 잡는다. S4.4 의 입력이다. P1b $d_{eff}$ 는 스캔 상한에 걸린 하한값이다
