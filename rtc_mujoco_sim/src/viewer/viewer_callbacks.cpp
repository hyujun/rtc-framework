...
static void SelectBody(GLFWwindow* w, ViewerState* s) noexcept {
  ...
  int selflexid = -1;
  int selskin = -1;
  const int selobj = mjv_select(s->model, s->vis_data, s->opt,
  ...
  switch (key) {
    ...
    case GLFW_KEY_TAB: {
      ...
      if (s->cam_mode == CameraMode::kFree) {
        cur = 0;
      } else if (s->cam_mode == CameraMode::kTracking) {
        cur = 1;
      } else {
        cur = 2 + s->fixed_cam_idx;
      }
    ...
    // ── Solver controls ──────────────────────────────────────────────────────
    case GLFW_KEY_I: {
      static constexpr int kInts[] = {mjINT_EULER, mjINT_RK4, mjINT_IMPLICIT, mjINT_IMPLICITFAST};
      static constexpr const char* kNames[] = {"Euler", "RK4", "Implicit", "ImplicitFast"};
    ...
    case GLFW_KEY_B:
      ...
      break;
    case GLFW_KEY_U:
      s->opt->flags[mjVIS_ACTUATOR] ^= 1;
      break;
    case GLFW_KEY_E:
      s->opt->flags[mjVIS_INERTIA] ^= 1;
      break;
    case GLFW_KEY_W:
      s->opt->flags[mjVIS_COM] ^= 1;
      break;
    case GLFW_KEY_L:
      s->opt->flags[mjVIS_LIGHT] ^= 1;
      break;
    case GLFW_KEY_A:
      s->opt->flags[mjVIS_TENDON] ^= 1;
      break;
    case GLFW_KEY_X:
      s->opt->flags[mjVIS_CONVEXHULL] ^= 1;
      break;
    ...
    case GLFW_KEY_F5:
      s->scn->flags[mjRND_WIREFRAME] ^= 1;
      break;
    case GLFW_KEY_F6:
      s->scn->flags[mjRND_SHADOW] ^= 1;
      break;
    case GLFW_KEY_F7:
      s->scn->flags[mjRND_SKYBOX] ^= 1;
      break;
    case GLFW_KEY_F8:
      s->scn->flags[mjRND_REFLECTION] ^= 1;
      break;
    ...
    case GLFW_KEY_F4:
      ...
      break;
    case GLFW_KEY_F9:
      s->show_sensor = !s->show_sensor;
      break;
    case GLFW_KEY_F10:
      s->show_model_info = !s->show_model_info;
      break;
