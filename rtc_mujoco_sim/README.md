...
robot-specific 오버라이드(예: UR5e 설정 — `model_path`, `robot_response.groups`, joint names, command/state 토픽, servo gains)는 robot bringup 패키지에 위치합니다 — 예시: [integrated_bringup/config/mujoco_simulator.yaml](../integrated_bringup/config/mujoco_simulator.yaml).
...
# (예: integrated_bringup/config/ur5e_sim.yaml)에서 `/**:` wildcard로 관리.
```
...
ros2 launch rtc_mujoco_sim mujoco_sim.launch.py \
    params_file:=$(ros2 pkg prefix integrated_bringup)/share/integrated_bringup/config/mujoco_simulator.yaml
