...
if [[ "$TOTAL_CORES" -lt 4 ]]; then
  ...
  warn "  Core 0: OS/NVIDIA IRQ, Core 1+: RT 제어 루프 (rt_control/rt_callback/MPC)"
...
# Layout v4.1: 모든 tier 에서 Core 0 만 OS / NVIDIA IRQ (mask = 0x1 비-SMT 기준).
# RT cluster (rt_control / rt_callback / MPC) 가 Core 1 부터 시작.
