...
#
# The primitives this judgement is built from — normalise_cpu_set,
# shield_actual_user_cpus, isolcpus_isolated_cpus, shield_isolation_method —
# now live in lib/rt_common.sh (issue #386 A). They stayed here while cpu_shield
# was their only caller; check_rt_setup.sh sources rt_common.sh and NOT this
# file, so it could not reuse them and re-derived isolation from
# /sys/devices/system/cpu/isolated alone — a file cset never writes.
