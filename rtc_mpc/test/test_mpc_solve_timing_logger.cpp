...
// emission, append-mode (no duplicate header), per-sample Log() column
// count, and no-op behaviour when Open() failed.
//
// Schema is the per-tick raw-sample format: `t_wall_ns,count,solve_ns`
// (one CSV row per MPC solve). The 9-column aggregate format used
// previously has been replaced — readers that want percentiles compute
// them in post over the raw stream.
//
