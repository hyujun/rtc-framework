...
LightStageHandles CacheStageHandles(StageModel& stage, const StageComponentKeys& keys) noexcept {
  LightStageHandles h{};
...
OCPBuildError LightContactOCP::Build(const PhaseContext& ctx, const RobotModelHandler& model,
  ...
  try {
    ...
    for (int k = 0; k < H; ++k) {
      ...
      fresh_handles.push_back(
          CacheStageHandles(stored_stage, stage_keys[static_cast<std::size_t>(k)]));
    }
