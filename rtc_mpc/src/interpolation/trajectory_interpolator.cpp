...
void TrajectoryInterpolator::Init(int nq, int nv, int n_contact_vars) {
  ...
  if (nq < 0 || nq > kMaxNq || nv < 0 || nv > kMaxNv || n_contact_vars < 0 ||
      n_contact_vars > kMaxContactVars) {
    nq_ = 0;
    nv_ = 0;
    n_contact_vars_ = 0;
    has_solution_ = false;
...
void TrajectoryInterpolator::SetSolution(const MPCSolution& sol, uint64_t receive_ns) noexcept {
  if (!sol.IsValid()) {
    has_solution_ = false;
    return;
  }
...
void TrajectoryInterpolator::Interpolate(uint64_t now_ns, Eigen::Ref<Eigen::VectorXd> q_ref,
  ...
  const int64_t delta_ns = static_cast<int64_t>(now_ns) - static_cast<int64_t>(receive_ns_);
  const double t_local = std::max<double>(0.0, static_cast<double>(delta_ns) * 1e-9);
  ...
  if (t_local >= horizon_sec) {
    ...
    for (int i = 0; i < nq; ++i) {
      q_ref(i) = sol_.q_traj[static_cast<std::size_t>(last)][static_cast<std::size_t>(i)];
    }
    for (int i = 0; i < nv; ++i) {
      v_ref(i) = sol_.v_traj[static_cast<std::size_t>(last)][static_cast<std::size_t>(i)];
      a_ff(i) = 0.0;
    }
    for (int i = 0; i < nc; ++i) {
      lambda_ref(i) =
          sol_.lambda_traj[static_cast<std::size_t>(last_u)][static_cast<std::size_t>(i)];
    }
    meta_out.valid = true;
...
void TrajectoryInterpolator::InterpolateHermite(int node_k, double alpha,
  ...
  for (int i = 0; i < n_coupled; ++i) {
    ...
    v_ref(i) = (h00p * q_k + h10p * dt * v_k + h01p * q_k1 + h11p * dt * v_k1) * inv_dt;
    // a(t) = d²q/dt² = (d²q/dα²) / Δt²
    a_ff(i) = (h00pp * q_k + h10pp * dt * v_k + h01pp * q_k1 + h11pp * dt * v_k1) * inv_dt2;
  }
  ...
  for (int i = n_coupled; i < nq; ++i) {
    const auto idx = static_cast<std::size_t>(i);
    q_ref(i) = (1.0 - alpha) * sol_.q_traj[kk][idx] + alpha * sol_.q_traj[kk1][idx];
  }
  ...
  for (int i = 0; i < nc; ++i) {
    const auto idx = static_cast<std::size_t>(i);
    lambda_ref(i) =
        one_minus_alpha * sol_.lambda_traj[kk][idx] + alpha * sol_.lambda_traj[kk1][idx];
  }
}

double TrajectoryInterpolator::RemainingHorizonSec(uint64_t now_ns) const noexcept {
  if (!has_solution_ || sol_.dt_node <= 0.0 || sol_.horizon_length <= 0) {
    return 0.0;
  }
  const int64_t delta_ns = static_cast<int64_t>(now_ns) - static_cast<int64_t>(receive_ns_);
  const double t_local = std::max<double>(0.0, static_cast<double>(delta_ns) * 1e-9);
  const double horizon_sec = static_cast<double>(sol_.horizon_length) * sol_.dt_node;
