...
ContactRichMPC::ContactRichMPC() : core_(std::make_unique<internal::MPCHandlerCore>()) {}

ContactRichMPC::~ContactRichMPC() = default;

MPCInitError ContactRichMPC::Init(const MPCSolverConfig& solver_cfg, const RobotModelHandler& model,
                                  const OCPLimits& limits,
                                  const PhaseContext& initial_ctx) noexcept {
  return core_->Init(solver_cfg, model, limits, initial_ctx, ocp_);
}

MPCSolveError ContactRichMPC::Solve(const PhaseContext& ctx, const MPCStateSnapshot& state,
...
int ContactRichMPC::nq() const noexcept {
  return core_->nq();
}

int ContactRichMPC::nv() const noexcept {
  return core_->nv();
}

int ContactRichMPC::nu() const noexcept {
  return core_->nu();
}
