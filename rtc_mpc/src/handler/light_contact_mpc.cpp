...
LightContactMPC::LightContactMPC() : core_(std::make_unique<internal::MPCHandlerCore>()) {}
...
MPCSolveError LightContactMPC::Solve(const PhaseContext& ctx, const MPCStateSnapshot& state,
...
int LightContactMPC::nq() const noexcept {
  return core_->nq();
}

int LightContactMPC::nv() const noexcept {
  return core_->nv();
}

int LightContactMPC::nu() const noexcept {
  return core_->nu();
}
