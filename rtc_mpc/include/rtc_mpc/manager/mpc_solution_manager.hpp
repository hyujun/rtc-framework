...
public:
  ...
  // pushes one rtc::MpcSolveSample onto a generic ThreadTimingProducer.
  ...
  /// @brief Direct accessor for the per-tick solve-sample producer. The
  /// caller drives drain / drop-count from a non-RT thread. Returning a
  /// reference (not a copy) is fine — the consumer side of SpscQueue is
  /// single-reader and the producer side stays in this manager's own
  /// MPC thread.
...
private:
  ...
  // PublishSolution), consumer is the non-RT drain thread (typically the
  // controller LifecycleNode's 1 Hz aux timer). Lock-free, wait-free push.
