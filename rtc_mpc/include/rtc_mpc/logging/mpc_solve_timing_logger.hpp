...
// generic — see `rtc_base/timing/thread_timing_*`. This header is a thin
// wrapper that resolves the canonical
// `<session>/controllers/<config_key>/mpc_solve_timing.csv` path and
// pre-binds the MpcTimingPayload header / row writers.
//
// CSV schema:
//   t_wall_ns,tick_count,solve_ns
// One row per MPC solve. The first two columns come from
// `ThreadTimingCsvLogger`, the third from `WriteMpcTimingRow`.
