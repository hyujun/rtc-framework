...
`robot_descriptions/objects/` 는 **이 저장소가 소유하는** 시뮬레이션 물체다 (submodule 인 `object_sim` 은 upstream 에 push 할 수 없으므로 거기에 추가하지 않는다). 레이아웃은 `object_sim` 과 같은 `<name>/object.xml` (body 이름 `object`, joint 없음 — `rtc_mujoco_sim` object_pool 이 freejoint 를 붙인다) 이라, object_pool 의 `directory` 를 `package://robot_descriptions/objects` 로 바꾸면 그대로 쓴다. 현재 둘 다 `ur5e_p1b` sim overlay 가 쓴다 (`integrated_bringup/config/ur5e_p1b/sim_overlays/`), 그리고 둘 다 **기하가 계약**이라 옮기기 전에 파일 주석을 읽어야 한다 — `pole` 은 원통 r 0.04 m · h 0.15 m 에 **원점 = 원통 중심** (정책 관측이 그 원점의 pose 다), 질량 0.2 kg 은 학습 자산 값이 아니다 (`inference_pole.yaml`). `fingertip_disk` 는 100 kg 받침 + 넥 + 손끝 높이 디스크의 한 body 로, z 오프셋이 overlay 의 스폰 z 와 짝이고 (x,y) 는 격자탐색 결과다 — 어느 하나만 바꾸면 디스크가 손끝 센서 밴드를 벗어난다 (`fingertip_grasp.yaml`). `fingertip_disk_free` 는 그 스탠드에 디스크를 **자식 body 로 분리**해 6-DOF 관절 + `gravcomp="1"` + 약한 복원 스프링을 준 판으로, 손을 따라오는 물체가 필요할 때 쓴다 (`fingertip_grasp_free.yaml`) — 두 파일의 스폰·받침·디스크 밴드는 서로 맞춰져 있어야 하고, 왜 자식 body 여야 하는지(object_pool 이 root 의 `gravcomp` 를 덮고 root 의 park 이 free 자식을 떼어놓는다)는 파일 주석에 있다.
