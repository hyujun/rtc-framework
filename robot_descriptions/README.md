...
| 영역 | 변경 내용 |
|------|----------|
| **순수 데이터화** | `include/ur5e_description/ur5e_constants.hpp` 제거. `kNumHandMotors`/`kDefaultHandMotorNames`/`kDefaultFingertipNames` → `ur5e_hand_driver`, `HandState` → `ur5e_hand_driver/hand_state.hpp`. `RobotState`/`kNumHandJoints`/`kDefaultRobotJointNames`는 사용처가 없어 삭제. `rtc_base` 의존도 제거 — 이 패키지는 URDF/MJCF/mesh만 담는다. |
