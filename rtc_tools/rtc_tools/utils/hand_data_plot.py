...
TIMING_COLS_BULK = ["t_cycle", "t_write_position", "t_read_all_motors", "t_read_all_sensors"]
TIMING_LABELS = {
    "t_cycle": "cycle_total",
    "t_write_position": "write_pos",
    "t_read_position": "read_pos",
    "t_read_velocity": "read_vel",
...
def plot_timing(df: pd.DataFrame):
    ...
    for ax, col in zip(axes, available, strict=False):
        ...
        avg = data.mean()
        ax.axhline(avg, color="r", linestyle="--", linewidth=0.8, label=f"avg={avg:.3f}ms")
    ...
    # 히스토그램 플롯
    fig2, axes2 = plt.subplots(1, len(available), figsize=(4 * len(available), 4))
    if len(available) == 1:
        axes2 = [axes2]
    ...
    for ax, col in zip(axes2, available, strict=False):
        ...
        ax.axvline(avg, color="r", linestyle="--", linewidth=1, label=f"avg={avg:.3f}")
        ax.axvline(p99, color="orange", linestyle="--", linewidth=1, label=f"p99={p99:.3f}")
    ...
    if "t_timeouts" in df.columns:
        ...
        if total_timeouts > 0:
            ...
            ax3.plot(df["timestamp"], timeout_data, linewidth=0.8, color="red", alpha=0.7)
    ...
    for col in available:
        ...
        label = TIMING_LABELS.get(col, col)
        print(
            f"  {label:<15s} {data.mean():8.3f} {data.std():8.3f} "
...
def main():
    ...
    parser.add_argument(
        "--motors", type=int, nargs="+", help="표시할 모터 인덱스 (예: 0 1 2). 기본=전체"
    )
    parser.add_argument(
        "--sensors-only", action="store_true", help="센서 데이터만 표시 (모터 생략)"
    )
    parser.add_argument(
        "--motors-only", action="store_true", help="모터 데이터만 표시 (센서 생략)"
    )
    parser.add_argument(
        "--timing-only", action="store_true", help="타이밍 데이터만 표시 (모터·센서 생략)"
    )
