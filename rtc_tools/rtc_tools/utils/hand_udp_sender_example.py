...
CMD_READ_SENSORS = [CMD_READ_SENSOR0, CMD_READ_SENSOR1, CMD_READ_SENSOR2, CMD_READ_SENSOR3]
...
def encode_motor_packet(
    cmd: int, floats: list[float] | None = None, joint_mode: int = JOINT_MODE_MOTOR
) -> bytes:
    """
    모터 패킷 인코딩 (43 bytes)
...
def encode_motor_read_request(cmd: int, joint_mode: int = JOINT_MODE_MOTOR) -> bytes:
    """
...
def encode_sensor_request(cmd: int, sensor_mode: int = SENSOR_MODE_RAW) -> bytes:
    ...
    """
    assert CMD_READ_SENSOR0 <= cmd <= CMD_READ_SENSOR3, f"개별 센서 커맨드가 아닙니다: 0x{cmd:02x}"
...
def decode_sensor_response(buf: bytes) -> tuple[int, int, list[int]]:
    ...
    # skip reserved[8..12]
    # tof[13..15] → out[8..10]
    values.extend(raw_data[BAROMETER_COUNT + RESERVED_COUNT + i] for i in range(TOF_COUNT))
...
def decode_all_motor_response(
    ...
    _id, cmd, mode = struct.unpack("<BBB", buf[:HEADER_SIZE])
    raw_data = struct.unpack(f"<{ALL_MOTOR_DATA_COUNT}I", buf[HEADER_SIZE:ALL_MOTOR_PACKET_SIZE])
...
def decode_all_sensor_response(buf: bytes) -> tuple[int, int, list[int]]:
    ...
    raw_data = struct.unpack(
        f"<{ALL_SENSOR_DATA_COUNT}i", buf[HEADER_SIZE:ALL_SENSOR_RESPONSE_SIZE]
    )
    ...
    for finger in range(NUM_FINGERTIPS):
        ...
        # tof[13..15]
        values.extend(
            raw_data[base + BAROMETER_COUNT + RESERVED_COUNT + i] for i in range(TOF_COUNT)
        )
...
class HandUDPSender:
    ...
    def __init__(
        ...
        print(
            f"Hand UDP Sender (motor: {MOTOR_PACKET_SIZE}B, sensor req: {SENSOR_REQUEST_SIZE}B → resp: {SENSOR_RESPONSE_SIZE}B)"
        )
        print(f"  Target: {target_ip}:{target_port}")
        print(
            f"  Motors: {NUM_HAND_MOTORS}, Connected sensors: {self.num_sensors}/{NUM_FINGERTIPS}"
        )

    def write_position(
        self, positions: list[float], joint_mode: int = JOINT_MODE_MOTOR
    ) -> list[float] | None:
    ...
    def read_all_motors(
        self, joint_mode: int = JOINT_MODE_MOTOR
    ...
    def read_sensor(
        self, fingertip_idx: int, sensor_mode: int = SENSOR_MODE_RAW
    ...
    def _request_motor(
        self, cmd: int, floats: list[float] | None = None, joint_mode: int = JOINT_MODE_MOTOR
    ) -> list[float] | None:
    ...
    def _request_motor_read(
        self, cmd: int, joint_mode: int = JOINT_MODE_MOTOR
    ) -> list[float] | None:
    ...
    def _request_sensor(
        self, cmd: int, sensor_mode: int = SENSOR_MODE_RAW
    ) -> tuple[list[int], int] | None:
        """
...
def _format_sensor_detail(sensors: list[int], num_sensors: int, sensor_modes: list[int]) -> str:
...
class UdpTimingStats:
    ...
    def format_stats(self) -> str:
        ...
        lines = [
            f"── UDP Timing Stats (last {n} cycles, total {self._total_cycles}, timeouts {self._timeout_count}) ──"
        ]
...
class HandDataCsvLogger:
    ...
    def __init__(
        ...
        # 타이밍 컬럼 (ms 단위)
        if bulk_mode:
            self._timing_keys = ["cycle", "write_position", "read_all_motors"]
            if num_sensors > 0:
                self._timing_keys.append("read_all_sensors")
        else:
            self._timing_keys = ["cycle", "write_position", "read_position", "read_velocity"]
            if num_sensors > 0:
...
def example_write_only(target_ip: str = "192.168.1.2"):
    ...
    try:
        ...
        while True:
            ...
            positions = [
                0.5 + 0.3 * np.sin(2 * np.pi * 0.5 * t + i * 0.2) for i in range(NUM_HAND_MOTORS)
            ]
...
def example_poll_cycle(
    target_ip: str = "192.168.1.2", num_sensors: int = NUM_FINGERTIPS, csv_dir: str | None = None
):
    ...
    if csv_dir is not None:
        csv_logger = HandDataCsvLogger(
            num_sensors=num_sensors, output_dir=csv_dir, prefix="hand_poll"
        )
    ...
    print(
        f"  WritePosition(43B↔43B) → ReadPosition(43B↔43B) → ReadVelocity(43B↔43B) → ReadSensor×{num_sensors}(3B[MODE=raw]→67B)"
    )
    print("Ctrl+C로 중지\n")
...
def example_read_only(
    ...
    if csv_dir is not None:
        csv_logger = HandDataCsvLogger(
            num_sensors=num_sensors, output_dir=csv_dir, prefix="hand_read"
        )
        print(f"  CSV 저장: {csv_logger.filepath}")
    ...
    print(
        f"  ReadPosition(43B↔43B) → ReadVelocity(43B↔43B) → ReadSensor×{num_sensors}(3B[MODE=raw]→67B)"
    )
    print("Ctrl+C로 중지\n")
...
def example_poll_cycle_bulk(
    ...
    if csv_dir is not None:
        csv_logger = HandDataCsvLogger(
            num_sensors=num_sensors, output_dir=csv_dir, prefix="hand_bulk_poll", bulk_mode=True
        )
        print(f"  CSV 저장: {csv_logger.filepath}")
...
def example_read_only_bulk(
    ...
    if csv_dir is not None:
        csv_logger = HandDataCsvLogger(
            num_sensors=num_sensors, output_dir=csv_dir, prefix="hand_bulk_read", bulk_mode=True
        )
        print(f"  CSV 저장: {csv_logger.filepath}")
...
def main(args=None):
    ...
    print(
        f"  모터 일괄 응답:  {ALL_MOTOR_PACKET_SIZE}B = [ID:1B][CMD:1B][MODE:1B][data:30×float32] (pos+vel+cur)"
    )
    ...
    print(
        f"                  → barometer[{BAROMETER_COUNT}] + reserved[{RESERVED_COUNT}](skip) + tof[{TOF_COUNT}] = {SENSOR_VALUES_PER_FINGERTIP} 유효 값"
    )
    print(
        f"  센서 일괄 응답:  {ALL_SENSOR_RESPONSE_SIZE}B = [ID:1B][CMD:1B][MODE:1B][data:64×uint32] (4핑거)"
    )
    print(
        f"  모터: {NUM_HAND_MOTORS}개, 핑거팁: 최대 {NUM_FINGERTIPS}개, 센서 값/팁: {SENSOR_VALUES_PER_FINGERTIP}개"
    )
    print()
    ...
    # 연결된 센서 수 입력
    num_sensors_str = input(
        f"연결된 센서 수 (0~{NUM_FINGERTIPS}, 기본={NUM_FINGERTIPS}): "
    ).strip()
    ...
    print(
        f"  → 센서 {num_sensors}개 사용 (cmd: {', '.join(f'0x{CMD_READ_SENSORS[i]:02x}' for i in range(num_sensors)) or 'none'})"
    )
    print()
