...
- **tick overrun 공변량**: `<session>/timing/cm_timing_log.csv` (RT 루프 tick 당 한 행) 에서 시행 창 [발사, 시행 끝] 안 `t_total_us` > 제어 주기 tick 수 (`tick_overrun_n`) 와 최대 `jitter_us` (`tick_jitter_max_us`), 요약 p50/p95/max. `t_wall_ns` 는 이름과 달리 **steady_clock** (`ThreadTimingProducer::NowNs`) 이라 러너의 `launch_wall_time` (epoch) 과 기준이 다르다 — clock lane 의 시행별 steady 오프셋으로 잇고, lane 이 없으면 `NOT_EVALUATED(no clock lane …)`, 파일이 없으면 `NOT_EVALUATED(no timing log)`. `rt_callback_timing_log.csv` 는 tick 이 아니라 device state 콜백당 한 행이라 쓰지 않는다. sim 모드 CM 은 `jitter_us` 를 0 으로 낸다 (deadline wakeup 이 아님)
...
- 통계 함수는 `catching_trials` 의 것을 그대로 쓴다 (`truth_block`·`floor_verdict`·`streak_distribution`·`impulse_correlation`·`tick_overrun_summary`·`mcnemar_exact`). `invalid_reason` 열이 없는 (검증 규칙 이전) CSV 는 거부 — 그 unit 에 `catching_trials` 를 다시 돌린다
- 테스트 `test/test_catching_pool.py` (13 케이스): 손으로 쓴 unit dir 로 절단 (unit 중간에서 목표 도달·순서 뒤집으면 다른 시행이 빠짐)·`INSUFFICIENT_N`·84/200 PASS 대 83/200 FAIL·McNemar 쌍 (한쪽 무효·한쪽에만 있는 idx 제외)·중복 `(seed, idx)` 거부·구 CSV 거부·S3.1b 합산·gate-map 블록·CLI strict JSON
