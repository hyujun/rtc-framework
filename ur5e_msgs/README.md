...
# 워크스페이스 루트에서
cd ~/ur_ws
colcon build --packages-select ur5e_msgs
source install/setup.bash
```

빌드 후 생성된 메시지는 C++ (`ur5e_msgs/msg/joint_command.hpp` 등) 및 Python (`ur5e_msgs.msg.JointCommand` 등)에서 사용 가능합니다.
...
```
ur5e_msgs   ← std_msgs (ROS2 기본 메시지)
    ↑
