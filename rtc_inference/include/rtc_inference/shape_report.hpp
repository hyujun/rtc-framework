...
// YAML that describes them. It has to be right, and "right" here means two
// things the old inline check did not do:
//
...
//      about what to edit.
//
// Nothing here knows ONNX Runtime exists. That is deliberate and not
// stylistic: there is no way to build a .onnx fixture in this repo's test
// environment (no python `onnx` module), so a comparison living inside the
// engine would be reachable only through a real model file and would ship
// untested. As a pure function over two lists of shapes it is covered
// exhaustively by `test_shape_report.cpp` instead, and what stays untested is
// only the handful of ORT calls that collect the model side.
//
...
/// One tensor, as either the model or the config describes it.
///
/// `name` is carried for DIAGNOSTICS at this stage — pairing is positional (see
/// CompareModelIo). A model-side name comes from ORT; a config-side name is
/// empty while the schema has no way to spell one.
struct TensorSpec {
  std::string name;
...
struct IoReport {
  ...
  /// True when every row is kOk. Arity disagreement always produces at least
  /// one kNotDeclared / kNotInModel row, so it is covered by this too.
...
///
/// PAIRING IS POSITIONAL: entry i of `model_*` is compared with entry i of
/// `declared_*`, which is exactly how the engine binds tensors, so the report
/// describes the binding that will actually happen rather than an idealised
/// one. The consequence is inherited from that binding and is worth stating:
/// two tensors of the SAME shape that the model exports in the other order
/// compare as kOk. Names are collected and printed so a human can see it even
/// though this function cannot.
///
...
inline std::vector<TensorComparison> CompareSide(const std::vector<TensorSpec>& model,
  ...
  for (std::size_t i = 0; i < n; ++i) {
    ...
    // the config side has none to offer at this stage.
...
inline std::string IoReport::Format(std::string_view model_path) const {
  ...
  detail::AppendSide(out, "inputs ", inputs, model_inputs, declared_inputs);
  detail::AppendSide(out, "outputs", outputs, model_outputs, declared_outputs);
  return out;
}
...
inline IoReport CompareModelIo(const std::vector<TensorSpec>& model_inputs,
  ...
  report.inputs = detail::CompareSide(model_inputs, declared_inputs);
  report.outputs = detail::CompareSide(model_outputs, declared_outputs);
  return report;
}
