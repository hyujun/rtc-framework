...
| 출력 | `{1, 1, 7}` | 7개 값: contact(1) + F(3) + u(3) (3-head 모델) |
...
| 영역 | 변경 내용 |
|------|----------|
| **3-head 모델** | 출력 형상 변경: 13 → 7 (contact(1) + F(3) + u(3)), `kFTValuesPerFingertip=7` |
| **RunModels()** | 배치 추론 최적화: IoBinding 우회 → `Session::Run()` 직접 호출 (RT 루프 권장) |
| **aarch64 지원** | CMake 수동 탐색에 `/usr/lib/aarch64-linux-gnu` 경로 추가 |
