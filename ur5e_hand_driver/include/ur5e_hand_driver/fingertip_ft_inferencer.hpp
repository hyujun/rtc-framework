...
class FingertipFTInferencer {
 public:
  ...
  [[nodiscard]] bool FeedCalibration(const std::array<int32_t, kMaxHandSensors>& /*sensor_data*/,
  ...
  [[nodiscard]] FingertipFTState Infer(const std::array<int32_t, kMaxHandSensors>& /*sensor_data*/,
                                       int /*num_fingertips*/) noexcept {
    return {};
  }
  ...
  [[nodiscard]] std::array<std::array<float, kBarometerCount>, kMaxFingertips> baseline_offset()
      const noexcept {
    return {};
  }
};
 ...
 public:
  ...
  void InitFT(const Config& config) {
    ...
    for (int f = 0; f < n; ++f) {
      ...
      if (f >= static_cast<int>(config_.model_paths.size()) ||
        ...
        RCLCPP_WARN(::ur5e_hand_driver::logging::FtLogger(), "finger[%d]: SKIPPED (empty path)", f);
        model.valid = false;
        continue;
      }

      const auto& path = config_.model_paths[static_cast<std::size_t>(f)];
      RCLCPP_INFO(::ur5e_hand_driver::logging::FtLogger(), "finger[%d]: loading \"%s\"", f,
                  path.c_str());
      ...
      model.input_tensor = Ort::Value::CreateTensor<float>(
          ...
          static_cast<std::size_t>(config_.history_length) * kFTInputSize, input_shape, 3);
    ...
    RCLCPP_INFO(::ur5e_hand_driver::logging::FtLogger(), "num_active=%d / %d", num_active_, n);
  ...
  [[nodiscard]] bool FeedCalibration(const std::array<int32_t, kMaxHandSensors>& sensor_data,
    ...
    if (calibration_count_ >= config_.calibration_samples) {
      ...
      calibrated_ = true;
      RCLCPP_INFO(::ur5e_hand_driver::logging::FtLogger(), "Calibration COMPLETE (%d samples)",
                  calibration_count_);
      return true;
    }
    return false;
  ...
  void ResetCalibration(uint16_t sample_count = 0) noexcept {
    ...
    calibration_count_ = 0;
    calibrated_ = false;
    RCLCPP_INFO(::ur5e_hand_driver::logging::FtLogger(), "Calibration RESET (target=%d samples)",
  ...
  [[nodiscard]] FingertipFTState Infer(const std::array<int32_t, kMaxHandSensors>& sensor_data,
    ...
    try {
      ...
      for (int f = 0; f < n; ++f) {
        ...
        if (H > 1) {
          std::memmove(model.input_buffer.data(), model.input_buffer.data() + kFTInputSize,
                       static_cast<std::size_t>(H - 1) * row_bytes);
        }
        std::memcpy(model.input_buffer.data() + static_cast<std::size_t>(H - 1) * kFTInputSize,
      ...
      RCLCPP_ERROR_THROTTLE(::ur5e_hand_driver::logging::FtLogger(), steady_clock,
