...
namespace rtc
{
  namespace hand_packets
  {
    ...
    {
      return cmd == Command::kWritePosition ||
             cmd == Command::kReadAllMotors ||
             cmd == Command::kReadPosition ||
             cmd == Command::kReadVelocity;
    }

    // Returns true if the command is a sensor command.
    inline constexpr bool IsSensorCommand(Command cmd) noexcept
    {
      return cmd == Command::kSetSensorMode ||
             cmd == Command::kReadAllSensors ||
...
#pragma pack(pop)
    ...
    inline SensorRequestPacket MakeMotorReadRequest(
        Command cmd, JointMode joint_mode = JointMode::kMotor) noexcept
    {
