#ifndef UR5E_HAND_DRIVER_HAND_PACKETS_HPP_
#define UR5E_HAND_DRIVER_HAND_PACKETS_HPP_
...
inline constexpr std::size_t kMotorPacketSize =
...
inline constexpr std::size_t kAllMotorResponseSize =
...
// For joint commands (0x01, 0x10, 0x11, 0x12):
//   mode 0x00 = motor position, 0x01 = joint position (same as JointMode)
// For sensor commands (0x04, 0x14-0x17, 0x19):
...
// ── Joint mode (mode field for joint commands: write/read position) ──────────
enum class JointMode : uint8_t {
  kMotor = 0x00,  // raw motor encoder position (default, backward-compatible)
  kJoint = 0x01,  // joint-space position (gear-ratio mapped by firmware)
};
...
inline constexpr Command SensorCommand(int fingertip_idx) noexcept {
  return static_cast<Command>(static_cast<uint8_t>(Command::kReadSensor0) + fingertip_idx);
}

// Returns true if the command is a joint (motor) command.
inline constexpr bool IsJointCommand(Command cmd) noexcept {
  return cmd == Command::kWritePosition || cmd == Command::kReadAllMotors ||
         cmd == Command::kReadPosition || cmd == Command::kReadVelocity;
}

// Returns true if the command is a sensor command.
inline constexpr bool IsSensorCommand(Command cmd) noexcept {
  return cmd == Command::kSetSensorMode || cmd == Command::kReadAllSensors ||
         (cmd >= Command::kReadSensor0 && cmd <= Command::kReadSensor3);
}

inline constexpr bool IsSensorCommand(uint8_t cmd) noexcept {
  return cmd == static_cast<uint8_t>(Command::kSetSensorMode) ||
         cmd == static_cast<uint8_t>(Command::kReadAllSensors) ||
         (cmd >= static_cast<uint8_t>(Command::kReadSensor0) &&
...
static_assert(sizeof(SensorRequestPacket) == kSensorRequestSize,
              "SensorRequestPacket size mismatch");
...
static_assert(sizeof(SensorResponsePacket) == kSensorResponseSize,
              "SensorResponsePacket size mismatch");
...
static_assert(sizeof(AllMotorResponsePacket) == kAllMotorResponseSize,
              "AllMotorResponsePacket size mismatch");
...
static_assert(sizeof(AllSensorResponsePacket) == kAllSensorResponseSize,
              "AllSensorResponsePacket size mismatch");
...
inline SensorRequestPacket MakeMotorReadRequest(Command cmd,
                                                JointMode joint_mode = JointMode::kMotor) noexcept {
  SensorRequestPacket pkt{};
  pkt.id = kDeviceId;
  pkt.cmd = static_cast<uint8_t>(cmd);
  pkt.mode = static_cast<uint8_t>(joint_mode);
  return pkt;
}
...
inline MotorPacket MakeWritePosition(const std::array<float, kNumHandMotors>& positions,
                                     JointMode joint_mode = JointMode::kMotor) noexcept {
  MotorPacket pkt{};
  pkt.id = kDeviceId;
...
[[nodiscard]] inline bool DecodeMotorPacket(const uint8_t* buf, std::size_t len,
                                            MotorPacket& out) noexcept {
  if (len < kMotorPacketSize)
    return false;
...
[[nodiscard]] inline bool DecodeSensorResponse(const uint8_t* buf, std::size_t len,
                                               SensorResponsePacket& out) noexcept {
  if (len < kSensorResponseSize)
    return false;
...
inline void ExtractMotorFloats(const MotorPacket& pkt,
...
[[nodiscard]] inline bool DecodeAllMotorResponse(const uint8_t* buf, std::size_t len,
                                                 AllMotorResponsePacket& out) noexcept {
  if (len < kAllMotorResponseSize)
    return false;
...
[[nodiscard]] inline bool DecodeAllSensorResponse(const uint8_t* buf, std::size_t len,
                                                  AllSensorResponsePacket& out) noexcept {
  if (len < kAllSensorResponseSize)
    return false;
...
inline void ExtractAllMotorFloats(const AllMotorResponsePacket& pkt,
                                  ...
                                  std::array<float, kMotorDataCount>& velocities,
                                  std::array<float, kMotorDataCount>& currents) noexcept {
  for (std::size_t i = 0; i < kMotorDataCount; ++i) {
    positions[i] = Uint32ToFloat(pkt.data[i]);
    velocities[i] = Uint32ToFloat(pkt.data[kMotorDataCount + i]);
    currents[i] = Uint32ToFloat(pkt.data[2 * kMotorDataCount + i]);
  }
}
...
inline void SerializePacket(const MotorPacket& pkt,
...
inline void SerializeSensorRequest(const SensorRequestPacket& pkt,
...
[[nodiscard]] inline bool DecodePacket(const uint8_t* buf, std::size_t len,
                                       MotorPacket& out) noexcept {
  return DecodeMotorPacket(buf, len, out);
}

inline void ExtractFloats(const MotorPacket& pkt,
                          std::array<float, kMotorDataCount>& out) noexcept {
...
#endif  // UR5E_HAND_DRIVER_HAND_PACKETS_HPP_
