 ...
 public:
  ...
  [[nodiscard]] Stats GetStats() const noexcept {
    ...
    if (s.ft_infer_count > 0) {
      s.ft_infer = LoadPhaseStats(ft_infer_sum_, ft_infer_min_, ft_infer_max_, s.ft_infer_count);
    }
  ...
  [[nodiscard]] std::string Summary() const noexcept {
    ...
    if (s.is_bulk_mode) {
      std::snprintf(buf, sizeof(buf),
                    ...
                    static_cast<unsigned long>(s.count), s.mean_us, s.min_us, s.max_us, s.p95_us,
                    s.p99_us, static_cast<unsigned long>(s.over_budget), over_pct, s.write.mean_us,
                    s.read_all_motor.mean_us, s.read_all_joint_motor.mean_us,
      ...
      std::snprintf(buf, sizeof(buf),
                    ...
                    static_cast<unsigned long>(s.count), s.mean_us, s.min_us, s.max_us, s.p95_us,
                    s.p99_us, static_cast<unsigned long>(s.over_budget), over_pct, s.write.mean_us,
                    s.read_pos.mean_us, s.read_joint_pos.mean_us, s.read_vel.mean_us,
                    s.read_sensor.mean_us, s.sensor_proc.mean_us);
    }

    if (s.ft_infer_count > 0) {
      const std::size_t len = std::strlen(buf);
      std::snprintf(buf + len, sizeof(buf) - len, "  ft=%.0f\xc2\xb5s", s.ft_infer.mean_us);
    }
