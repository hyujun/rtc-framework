...
// EventLoop. Inherits TimingProfilerBase<250,20,2000> for histogram/percentile
...
class HandTimingProfiler : public TimingProfilerBase<250, 20, 2000> {
public:
  using PhaseStats = typename TimingProfilerBase<250, 20, 2000>::PhaseStats;
  ...
  void Update(const PhaseTiming &t) noexcept {
    ...
    if (t.is_bulk_mode) {
      ...
      if (t.is_sensor_cycle) {
        ...
        UpdatePhase(read_all_sensor_sum_, read_all_sensor_min_,
                    read_all_sensor_max_, t.read_all_sensor_us);
      }
    } else {
  ...
  [[nodiscard]] Stats GetStats() const noexcept {
    ...
    if (s.individual_count > 0) {
      s.read_pos = LoadPhaseStats(read_pos_sum_, read_pos_min_, read_pos_max_,
                                  s.individual_count);
      s.read_joint_pos =
          LoadPhaseStats(read_joint_pos_sum_, read_joint_pos_min_,
    ...
    if (s.bulk_count > 0) {
      s.read_all_motor =
          LoadPhaseStats(read_all_motor_sum_, read_all_motor_min_,
                         read_all_motor_max_, s.bulk_count);
      s.read_all_joint_motor =
          LoadPhaseStats(read_all_joint_motor_sum_, read_all_joint_motor_min_,
    ...
    if (s.sensor_cycle_count > 0) {
      if (s.is_bulk_mode) {
        s.read_all_sensor =
            LoadPhaseStats(read_all_sensor_sum_, read_all_sensor_min_,
  ...
  [[nodiscard]] std::string Summary() const noexcept {
    ...
    if (s.is_bulk_mode) {
      ...
      std::snprintf(
          ...
          static_cast<unsigned long>(s.count), s.mean_us, s.min_us, s.max_us,
          s.p95_us, s.p99_us, static_cast<unsigned long>(s.over_budget),
          over_pct, s.write.mean_us, s.read_pos.mean_us,
          s.read_joint_pos.mean_us, s.read_vel.mean_us, s.read_sensor.mean_us,
          s.sensor_proc.mean_us);
    }

    if (s.ft_infer_count > 0) {
      const std::size_t len = std::strlen(buf);
      std::snprintf(buf + len, sizeof(buf) - len, "  ft=%.0f\xc2\xb5s",
                    s.ft_infer.mean_us);
    }
  ...
  void Reset() noexcept {
    ...
    ResetPhase(read_all_joint_motor_sum_, read_all_joint_motor_min_,
               read_all_joint_motor_max_);
    ResetPhase(read_all_sensor_sum_, read_all_sensor_min_,
               read_all_sensor_max_);
