...
| `kReadAllMotors` | `0x10` | 3B | 123B | 벌크 모터 읽기: pos[10]+vel[10]+cur[10] |
...
| `kReadAllSensors` | `0x19` | 3B | 259B | 벌크 센서 읽기: 4 핑거팁 × 16 uint32 |
...
- **내부**: `HandController` — event-driven 드라이버 (jthread, Core 5, SCHED_FIFO/65)
  ...
  - `consecutive_recv_failures_` (`std::atomic<uint64_t>`): 연속 recv 실패 횟수 (링크 건강 추적)
  ...
  - `sensor_decimation`: N cycle마다 센서 읽기 — 기본 1 (v5.11.0)
  ...
  - `communication_mode`: `"individual"` 또는 `"bulk"` 모드 선택
  - `joint_mode`: `"motor"` (0x00) 또는 `"joint"` (0x01) 모드
...
| 토픽 | 타입 | QoS | 설명 |
|------|------|-----|------|
| `/hand/joint_states` | `sensor_msgs/JointState` | BEST_EFFORT/1 | 관절 위치/속도 (name, position, velocity) |
| `/hand/motor_states` | `sensor_msgs/JointState` | BEST_EFFORT/1 | 모터 위치/속도/전류 (position, velocity, effort) |
...
| `/hand/link_status` | `std_msgs/Bool` | RELIABLE/TRANSIENT_LOCAL/1 | UDP 링크 상태 (~100 Hz 데시메이션) |

### 구독

| 토픽 | 타입 | QoS | 설명 |
|------|------|-----|------|
| `/hand/joint_command` | `rtc_msgs/JointCommand` | BEST_EFFORT/1 | RT 컨트롤러 모터 명령 (500 Hz) |
...
/**:
  ros__parameters:
    ...
    recv_timeout_ms: 0.1              # ppoll 수신 타임아웃 (ms, sub-ms 지원)
    sensor_decimation: 1              # N cycle마다 센서 읽기 (1=매번, 3=3cycle마다)
    communication_mode: "bulk"        # "individual" 또는 "bulk"
    joint_mode: "motor"               # "motor" (0x00) 또는 "joint" (0x01)

    # 센서 LPF (Bessel 4차)
    baro_lpf_enabled: true
    baro_lpf_cutoff_hz: 100.0         # 기압 센서 차단 주파수
    tof_lpf_enabled: true
    tof_lpf_cutoff_hz: 15.0           # ToF 센서 차단 주파수

    # 드리프트 감지
    drift_detection_enabled: false
    drift_threshold: 5.0              # OLS 기울기 임계값
    drift_window_size: 2500           # 윈도우 크기 (500Hz × 5초)

    # Failure Detector (50Hz non-RT jthread)
    enable_failure_detector: false    # 장애 감지기 활성화
...
| `communication_mode` | `bulk` | `"individual"` 또는 `"bulk"` |
| `recv_timeout_ms` | `0.1` | ppoll 수신 타임아웃 (ms) |
...
- **네 가지 장애 조건**:
  ...
  3. **Low rate**: EventLoop rate가 `min_rate_hz` 미만인 상태가 N회 연속 감지
  4. **Link down**: `consecutive_recv_failures` ≥ `link_fail_threshold`가 N회 연속 감지
...
| 필드 | 설명 |
|------|------|
| `write_us` | WritePosition + echo (0x01) |
| `read_pos_us` | ReadPosition (0x11) 또는 echo 데이터 |
| `read_vel_us` | ReadVelocity (0x12) |
| `read_sensor_us` | ReadSensor×4 (0x14–0x17) |
| `sensor_proc_us` | 센서 LPF + 드리프트 감지 |
| `ft_infer_us` | F/T 추론 소요시간 |
| `total_us` | 전체 사이클 |
...
|------|------|
| `write_us` | WritePosition + echo (0x01) |
| `read_all_motor_us` | ReadAllMotors (0x10) |
| `read_all_sensor_us` | ReadAllSensors (0x19) |
| `sensor_proc_us`, `ft_infer_us`, `total_us` | 동일 |
...
// 상태 콜백 등록 (EventLoop 스레드에서 호출 — printf 금지)
controller.SetCallback([](const urtc::HandState& state,
  ...
  // state.motor_currents[10]   — 모터 전류 (Bulk 모드)
  // state.sensor_data[44]      — 촉각 센서 (4 핑거팁 × 11)
  // ft_state.ft_data[]         — F/T 추론 결과 (활성화 시)
});
...
`hand_sensor_processor.hpp`에 정의된 센서 후처리 파이프라인입니다. EventLoop에서 noexcept로 실행됩니다.

| 메서드 | 설명 |
|--------|------|
| `PreFilter()` | Rate estimator tick + 지연 필터 재초기화 |
| `ApplyFilters()` | Bessel 4차 LPF in-place (barometer + ToF) |
| `DetectDrift()` | OLS 기반 원-샷 드리프트 감지 (1 Hz 스로틀 경고) |

- 실효 샘플링 레이트 = 500 Hz / `sensor_decimation`
- 필터 재초기화는 rate estimator 안정 후 수행 (정확한 샘플링 레이트 확보)
...
| 영역 | 변경 내용 |
|------|----------|
| **ROS2 인터페이스** | Float64MultiArray → JointState(관절/모터) + HandSensorState(센서) + JointCommand(명령) |
| **토픽 분리** | `/hand/joint_states` + `/hand/motor_states` + `/hand/sensor_states` + `/hand/sensor_states/monitor` + `/hand/link_status` |
| **Bulk 모드** | `communication_mode: "bulk"` 지원 — ReadAllMotors(0x10) + ReadAllSensors(0x19) |
| **JointMode** | `joint_mode: "motor"/"joint"` 선택 — 모터/관절 공간 명령 |
| **HandSensorProcessor** | 독립 클래스로 분리 — LPF, rate estimation, drift detection |
| **HandFailureDetector** | 링크 건강 검사 추가 (`check_link`, `link_fail_threshold`) |
| **HandTimingProfiler** | sensor_proc_us, ft_infer_us 단계 추가, Bulk 모드 단계 추가 |
